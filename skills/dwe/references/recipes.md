...
- `test run` is the **one conditional** entry: it deploys a throwaway copy, so it may be run unattended when that scenario's `cost_profile` (from `dwe test list --output json`) shows no `isolation_findings`, `shared_volumes: 0` and `shell_steps: 0` **and** you can account for the build/pull cost. Any hard stop set, or any doubt → hand it over. Propose it only for substantial changes. Full rule: the `dwe test run` gate in `SKILL.md`, detail in `integration-tests.md` § 1.
