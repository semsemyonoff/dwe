...
- Pick the apply command by what changed:
  ...
  - **`exports.env` only** (`workspace/defaults.yml`) → `dwe run` (or `dwe deploy run --force`). That block is in no config hash, so a plain `dwe deploy run` never re-renders `.env` — it journal-skips the implicit render step, and the built-in pipeline's always-run `up` step then re-ups against the stale file; only a custom pipeline with no always-run step returns `already up-to-date` (`render-and-vars.md` § 7).
