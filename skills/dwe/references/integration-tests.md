...
| --- | --- | --- |
| `isolation_findings` | named / `external:` volumes and networks the copy shares with the real env, and host ports interpolated from a variable this scenario's copy does not remap (`interpolated_host_port` — a variable traced to a `vars:` path is remapped automatically and never listed; fix the rest by routing the port, § 6). An entry carrying `"shared": true` is a `docker.yml` `shared: true` volume the project acknowledges — already counted by `shared_volumes` | **hard stop** if non-empty after dropping `"shared": true` entries |
...
The mutations are **mostly isolated and disposable** — own compose project, **non-shared** volumes, auto-remapped host ports, and a copy-local `.dwe/`, so they don't touch your running stack. But isolation is **not** total: `shared: true` volumes are reused verbatim (real cache/data is visible to every run), `container_name:` / named / `external:` compose resources bypass compose-project scoping, a compose host port interpolated from a variable dwe cannot trace keeps its original value, and arbitrary host side effects of host-executing deploy steps (absolute paths, `~`, bind mounts outside the project) are not sandboxed — see "Documented limitations" in `dwe docs show config/tests --lang en`. **These leaks map onto the profile's three hard-stop fields** (`shared_volumes`, `isolation_findings`, `host_steps`): when any of them is set, the gate in § 1 sends the run to the user, because a failure would no longer be confined to the copy. When they are all clear, the remaining question is only cost.
...
**The auto-remap covers `services.<name>.ports` AND a compose variable dwe can trace back to a `vars:` path — nothing else.** A host port that reaches compose through any *other* channel is not remapped:
...
- **A traced var-interpolated compose port** (`ports: ["${DB_PORT}:5432"]` where an active `exports.env` rule exports `DB_PORT` `from: vars.<path>`) → **remapped automatically**, from the same allocation batch as the modeled ports, with no scenario config at all. A scenario that needs a specific port writes `env.vars: { <path>: <number> }`, which **pins** it and disables the remap for that path (`auto` and an absent key do not pin). `<path>` is the `vars:` path the rule reads, NOT the compose variable name (`DB_PORT`). These never appear in `isolation_findings`.
- **An untraced var-interpolated compose port** — no `exports.env` rule, a rule whose `when:` is falsy, or one reading something other than `vars.<path>` / a declared service port → **not remapped**. It binds the original host port in every copy, so parallel scenarios — and a `--keep` copy left running — **collide on it**. This is the real trap; it looks like a flaky "port already allocated" that only appears under `--parallel` or alongside a kept run. The scanner warns about it (`interpolated_host_port`) but does **not** block: the run still starts. The fix is to route the port: declare it under `services.<name>.ports`, or export the variable `from: vars.<path>` through an `exports.env` rule. (`services.<name>.ports` can't take a `${var}` — it's a strict int rejected at config load — so the `exports.env` route is what a var-routed compose port needs.)

The rule of thumb: model every host port under `services.<name>.ports`; if a port must reach compose through a var instead, export that var from a `vars.<path>` so dwe can trace and remap it.
...
- **A literal host port** (`8080:8080`) in raw compose — bypasses both the remap and `ports_free`, and the scanner flags **any** literal host-port token regardless of your service config. Merely adding a `services.<name>.ports` entry while `8080:8080` stays literal in compose does NOT clear the finding or move the bind. Fix it by **replacing the literal with an interpolated token** (`${APP_PORT}`) sourced from a modeled port — a `services.<name>.ports.<x>` value surfaced via an `exports.env` rule (`from: services.<name>.ports.<x>`) — or by routing the compose interpolation through a var an `exports.env` rule exports `from: vars.<path>`. (An interpolated port is never blocking: traced to a `vars:` path it is always silent, and sourced from a modeled port it is silent in the scenarios that remap that service — enabled and not listed under `env.services.disable`, even when the service is `required`; otherwise it draws the non-blocking `interpolated_host_port` warning above.)
