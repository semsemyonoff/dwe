...
**A `ports:` entry is display-only on its own.** It feeds `dwe info` and the test-runner's port isolation, but nothing binds until an `exports.env` rule surfaces it (`{name: APP_PORT, from: services.<name>.ports.http}`) and the compose file interpolates `${APP_PORT}`. `PROJECT`, `UID`, `GID` and `COMPOSE_PROJECT_NAME` are injected into `.env` automatically — do not declare export rules for them (it is a hard error). `COMPOSE_PROJECT_NAME` carries the lowercased compose project name, so a raw `docker compose` from the project root scopes to the same project as `dwe`; change it through `docker.yml` `project_name`.
