...
- **A `ports:` entry in `service.yml` does not publish anything** — compose binds only when an `exports.env` rule surfaces it (`{name: APP_PORT, from: services.<name>.ports.http}`) and compose interpolates that var. DWE tooling still reads the declaration: `ports_free` preflight checks it and `dwe test` remaps it, so removing it silently drops both. `PROJECT`, `UID`, `GID`, `COMPOSE_PROJECT_NAME` are injected into `.env` automatically and may not be redeclared; the compose project name is set through `docker.yml` `project_name`, always lowercased.
...
- **Output streams.** The `▶ <id>` run banner goes to stderr (and is not printed with `-o json`), so stdout of `dwe cmd <id>` is the command's own output. `dwe -v cmd <id>` also echoes the exact process it spawns (`docker compose … exec|run …`, host `sh -c …`) to stderr.
