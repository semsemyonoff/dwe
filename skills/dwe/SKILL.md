...
- `isolation_findings` non-empty **after dropping entries carrying `"shared": true`** — named /
  ...
  variable this scenario's copy does not remap (`interpolated_host_port`; the fix is to route
  the port through a declared `services.<name>.ports` entry or an `exports.env` rule
  `from: vars.<path>`, which `dwe test` then remaps on its own — a variable already traced to
  a `vars:` path is never listed here). A `"shared": true` entry is a volume the
...
- `dwe deploy eject --out <path>` / `dwe reset eject --out <path>` — write the built-in default pipeline as an editable `deploy.yml` / `reset.yml`; refuses an existing target without `--force`. Without `--out` both only print to stdout, which is a read.
