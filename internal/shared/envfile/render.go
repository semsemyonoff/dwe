...
func BuildContent(cfg *config.DweConfig, baseDir string) (string, error) {
	...
	for _, name := range config.ReservedExportNames {
		...
		if name == "COMPOSE_PROJECT_NAME" {
			// Resolved HERE, in slice order, rather than up front: the
			// resolver derives the name from project.prefix/name when
			// docker.yml is absent, so an early call would report a marker in
			// project.name as a compose-project-name failure and bury the far
			// more specific PROJECT / project.name message emitted just above.
			//
			// A wrong compose project name in .env is exactly the split-brain
			// this export exists to remove, so a resolution failure stops the
			// render rather than falling back to a guessed name.
			resolved, err := config.ResolveComposeProjectName(baseDir, cfg)
			if err != nil {
