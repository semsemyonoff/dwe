...
func TestMermaidPlaceholder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.expectErr && tt.ph.Err == "" {
				t.Errorf("expected error in placeholder")
			}
			if len(tt.ph.Text) == 0 {
				t.Errorf("expected non-empty placeholder text")
			}
		})
	}
}
...
func TestExtractMermaidBlocks(t *testing.T) {
	input := []byte("Text\n```mermaid\ngraph A\n```\nMore\n```mermaid\ngraph B\n```")
	blocks := ExtractMermaidBlocks(input)
