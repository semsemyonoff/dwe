...
func TestDiffToolSelection_NothingChanged(t *testing.T) {
	...
	// kept matches current state: adminer enabled, mailpit not
	toEnable, toDisable := diffToolSelection(rows, []string{"adminer"})
	if len(toEnable) != 0 {
...
func TestDiffToolSelection_OneDisabled(t *testing.T) {
	...
	// user unchecked adminer
	toEnable, toDisable := diffToolSelection(rows, []string{})
	if len(toEnable) != 0 {
		t.Errorf("toEnable should be empty, got %v", toEnable)
	}
...
func TestDiffToolSelection_MultipleChanges(t *testing.T) {
	...
	// user enables adminer, disables redis_insight, leaves mailpit
	toEnable, toDisable := diffToolSelection(rows, []string{"adminer", "mailpit"})
	if len(toEnable) != 1 || toEnable[0] != "adminer" {
		t.Errorf("toEnable = %v, want [adminer]", toEnable)
	}
