...
// TestServiceNameCompletion_brokenSchema documents the __complete contract: a
...
func TestServiceNameCompletion_brokenSchema(t *testing.T) {
	...
	root := rootCmdForServiceCompletion(flags, "")
	completions, directive := cmdctx.ServiceNameCompletion(flags)(root, nil, "")
...
// TestServiceNameCompletion_malformedManifestDoesNotBlock documents the
// boundary between completion (config-scoped) and rendering (manifest-scoped):
...
`
	...
	flags := &cmdctx.RootFlags{}
	root := rootCmdForServiceCompletion(flags, "")
	completions, _ := cmdctx.ServiceNameCompletion(flags)(root, nil, "")
