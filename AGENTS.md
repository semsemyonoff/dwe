...
- **Anchor derivation + long-doc hint** — `parseHeadingSlugLabel` is the single derivation for every anchor surface, with the **slug from the RAW heading text and the label from the stripped one**.
  ...
  A slug also keeps the heading's OWN leading hyphens (`--parallel-n`) — trimming them made `docs show` reject a link its own docs ship — and `TestDocumentLinkAnchorsResolve` pins the surface the advertise-side tests cannot see: every `#anchor` a doc links to.
