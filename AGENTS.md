...
- `internal/config/` — `DevboxConfig` struct, layered `LoadConfig()`, `LoadDeployConfig()`, `LoadDockerConfig()`, `LoadLifecycleConfig()`, `ResolvePath()`, `ExportRule`, `ComposeConfig`, `DockerConfig`, `DeployConfig`, `LifecycleConfig`, `IDEConfig`, `InfoConfig`, `LoadInfoConfig()`, `StylesConfig`, `LoadStylesConfig()`; `BinariesConfig` (devbox/docker/shell binary overrides, read from top-level `devbox.yml` only — not layered); nil-safe accessors `DevboxBin`, `DockerBin`, `ShellBin` (always go through these, never `cfg.Binaries.*` directly); `Action` (typed step-body and `check:` struct: `Type` one of `shell|devbox|command|builtin`, `Cmd`, `With`; custom `UnmarshalYAML` rejects scalar shorthand); **strict YAML decode**: `LoadDeployConfig`, `LoadResetConfig`, `LoadLifecycleConfig` use `yaml.Decoder.KnownFields(true)` — unknown fields in deploy/reset/lifecycle YAML are hard errors; other loaders (info, styles, docker, localconfig, topology) remain lenient; `(*DevboxConfig).ComposeFiles()` returns active compose file list (base + enabled overlays, sorted); `(*DevboxConfig).ComposeFilesAll()` returns all compose files regardless of enabled state — used by `--all` commands; `DockerArgs` has `Pull []string` and `Build []string` fields (yaml: `pull`, `build`) in addition to the other per-subcommand keys
- `internal/docker/` — `Compose` struct for building and executing `docker compose` commands with policy args; `Compose.Bin` is private-by-convention — always use `Compose.BinName()` (nil/empty-safe), never read the field directly; `NewCompose(cfg, dockerCfg)` populates `Bin` via `config.DockerBin(cfg)`; `NewComposeAll(cfg, dockerCfg)` uses `ComposeFilesAll()` instead of `ComposeFiles()` — use when `--all` is in effect
...
- `internal/command/` — cobra commands with Fang integration and command groups (root summary, lifecycle, services/tools, deploy/reset, docker/compose, docs); thin adapters delegating to domain packages. `docs generate --scope cli` works without a project (outputs to cwd); `--scope all`/`--scope commands` requires a project. Note: `devbox deploy config` and `devbox deploy config-check` commands were removed — their functionality is now available only as pipeline builtins (`service_configs_copy`, `service_configs_check` in `internal/builtin/`). `devbox docker pull [--all] [services...]` and `devbox docker build [--all] [--force] [services...]` are image-management subcommands; each `RunE` delegates to a pure resolver helper (`resolvePullInvocation`, `resolveBuildInvocation`) that selects `NewCompose` vs `NewComposeAll` based on the `--all` flag and builds the extra args slice — these helpers are unit-tested directly to avoid invoking Docker in tests.
...
- **YAML loader strictness**: Deploy/reset/lifecycle loaders (`LoadDeployConfig`, `LoadResetConfig`, `LoadLifecycleConfig`) and the command-file parser (`model.ParseCommandFile`) use `yaml.Decoder.KnownFields(true)` — unknown fields produce hard errors. All other loaders (info, styles, docker, localconfig, topology) use lenient `yaml.Unmarshal`. When adding new loaders, match the strictness of the surface: strict for user-edited pipeline/command YAML; lenient for internal or non-step config.
