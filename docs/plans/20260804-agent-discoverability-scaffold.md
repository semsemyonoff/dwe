...
- [x] make the hub triplet active (`dir`, `dir_internal: /workspace`,
      ...
      outlier and gets fixed here — the accompanying comment also states the class-1
      "mount the whole hub, not just `src/`" rule Task 7 documents
- [x] make `icon` active and **add** an `info:` block with `title` (19/19 services fill
      both; the template has no `info:` block at all today, so this is an addition)
- [x] **the port question is decided: leave `ports:` and `exports.env` commented, but
      ...
      — ➕ the `defaults.yml` half also names `PROJECT`/`UID`/`GID` as auto-injected and
      not redeclarable (`config.ReservedExportNames`), the exact fact one analysed session
      reverse-engineered from another repository's `.env`. Both halves are pinned by
      `TestEmbeddedTemplates_PortPairIsDocumentedOnBothSides`
- [x] this also gates Task 5c: with `ports:` commented, the starter scenario **cannot**
...
- [x] remove the commented `render.ide.enabled: true` example (it is the default for
      `type: app`; only `false` is meaningful)
- [x] **drop the `project_name` item**: `workspace/docker.yml` is not a `.tmpl`, so it is
      ...
      — header rewritten to "overriding is per-key, not whole-file", naming the four keys
      that carry defaults; the commented `args:` example gained the missing `down:` entry
      so the list and the header agree
- [x] regenerate the golden; extend `validity_test.go` beyond `validatecfg.All()` to
      `templates.All()` and assert zero `config`+`templates` diagnostics (constraint 6)
      — the helper now collects **warnings as well as errors** (a fresh scaffold that warns
      is the noise this branch removes); `env` stays excluded because it checks the host.
      `TestEmbeddedTemplates_StarterServiceKeepsTypeAndContainerActive` was replaced by
      `TestEmbeddedTemplates_StarterServiceActiveKeys`, which pins the exact active set and
      re-asserts that `ports`/`hosts`/`required`/`depends_on` stay commented
- [x] run tests — must pass before task 5 — `make test` and `make lint` both clean; a real
      `dwe init --default` + `dwe validate` reports 3 infos and zero warnings/errors
