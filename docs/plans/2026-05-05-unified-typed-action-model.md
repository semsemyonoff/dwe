...
- [x] migrate `service_configs_copy` shorthand call sites in `internal/deploy/` and `internal/lifecycle/` to emit `type: builtin, cmd: service_configs_copy, with: {service, mode}` directly
...
- [x] update `internal/pipeline/{executor,step,plain}_test.go`, `internal/deploy/{plan,service_plan}_test.go`, `internal/reset/*_test.go`, `internal/lifecycle/*_test.go` — (struct literals and assertions updated, YAML fixture updates completed)
