...
- [ ] verify every Overview deliverable exists and is exercised by tests (enable gate,
...
- [ ] verify the existing goldens are byte-stable (rendering unchanged): the `frame_*.golden`
  and `help_default.golden` assertions still pass without regeneration
- [ ] verify the `Plugin` interface is unchanged and there are NO new importers of `tui`;
