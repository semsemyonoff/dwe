...
- [ ] update `internal/pipeline/logging.go` `OpenPipelineLog` to write to
  `.devbox/logs/<name>.log` (parent dir created with `0o755`)
- [ ] update godoc on `OpenPipelineLog`, `DevboxConfig.Log` (in
  `internal/config/devbox.go`), and the Long descriptions in
  `internal/command/deploy.go` / `reset.go` to mention the new path
- [ ] update `internal/lifecycle/phases.go` log-dir reference if any (path is
  derived inside `OpenPipelineLog`, but the comment in `phases.go` mentions
  `logs/` — update it)
- [ ] update `docs/reference/config/lifecycle.md`, `deploy.md`,
  ...
  list)
- [ ] update generated CLI docs `docs/reference/cli/devbox_deploy_run.md`
...
- [ ] update or add a note in `docs/reference/config/index.md` documenting
  `.devbox/` as the gitignored devbox artifact root
- [ ] check `internal/config/lifecycle.example.yml` (if present) and any other
  fixture/example YAML for `logs/` references
- [ ] update existing tests in `internal/pipeline/logging_test.go` and
  `internal/lifecycle/phases_test.go` (search "logs/" already references this)
- [ ] add a new test case ensuring `.devbox/logs/<name>.log` is created and
