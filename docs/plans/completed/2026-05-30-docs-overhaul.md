...
- [x] `make build` exits 0 — regenerates `content_hashes_gen.go` and `internal/core/docs/embedded/`
- [x] add a Go test that walks `BuiltinFS` under `i18n/ru/`, parses each file's `> Translated from: <relPath> @ <hash>` header, and asserts `hash == ContentHashFor(relPath)` for every file. Catches drift in CI instead of via manual spot-check (`TestRussianTranslationsAreFresh` in `internal/core/docs/lang_test.go`)
- [x] `make test` exits 0
- [x] `make lint` exits 0
- [x] `./bin/devbox docs list --lang ru` enumerates the Russian tree (all 52 mirror paths reachable; the `Lang` column hard-codes `en` per a pre-existing TODO in `collectTopics` — out of scope for this plan)
- [x] `./bin/devbox docs search --lang ru архитектура` finds `concepts/architecture.md`
- [x] `./bin/devbox docs show concepts/architecture --lang ru` returns Russian content; no staleness warning surfaces
- [x] `./bin/devbox docs show README --lang ru` returns the Russian README
- [x] `./bin/devbox docs llms-txt` lists both locales (English by default, Russian via `--lang ru`)

**Acceptance criteria for Phase 6:**

- [x] every English file under `docs/reference/` plus repo-root `README.md` has a Russian sibling at the mirrored path under `docs/i18n/ru/`
- [x] every Russian file starts with a valid `> Translated from: ... @ <hash>` header whose hash matches the current English file (enforced by `TestRussianTranslationsAreFresh`)
- [x] zero staleness warnings on a fresh `make build`
- [x] both English and Russian discovery paths exercised cleanly via the `devbox docs` commands above
