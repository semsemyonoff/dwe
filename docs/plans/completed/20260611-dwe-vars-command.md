...
- [x] CORRECTION to the prior review: do NOT gate the EXISTING `e`/`ForceParamForm` binding. `dwe commands` ModeRun already handles `e` as `ForceParamForm` and shows `e edit params` in the footer (`model.go:296-308`); gating that shared binding behind `AllowEdit=false` would REMOVE existing command-browser behavior and break its goldens — the opposite of intended. Keep the ModeRun `e`/`ForceParamForm` path fully intact and independent. (The `EditParams`/`ForceParamForm` path stays gated on `m.opts.Mode == ModeRun` — untouched; `TestModeRun_EditParamsStillForcesForm` + `TestModeEdit_EditParamsKeyInert` pin both sides.)
- [x] Add the vars edit intent ADDITIVELY: a new `ActionEdit` (and, if a distinct key/footer entry is needed, a `Mode`-scoped or `Options`-scoped binding) that does NOT alter the command-mode `e edit params` binding or footer text. The vars browser opts into edit; the command browser is byte-identical to today. Reference the additive-extension contract in the `Result` struct comment (run.go:84-97). (New `ActionEdit`/`ModeEdit` appended at the iota tail; `actionForMode(ModeEdit)→ActionEdit`; footer Enter relabels to "edit" and breadcrumb noun to "var" ONLY in ModeEdit.)
- [x] Map vars → `cmdbrowser.Item`: `ID = vars.<path>` (drives the namespace tree), `Description = effective value`, `Type = layer-source badge` (`local`/`default`), `Inspect = func(width) string` rendering the inspect view. No second browser. (`buildVarsBrowserItems` in `browser.go`; `renderVarInspectFor` reuses the `vars inspect` resolve+scan+render.)
- [x] No-arg `dwe vars` dispatch: interactive → run browser; on `ActionInspect` show overlay (already in browser); on `ActionEdit`/Enter → open the `set` huh form, write, reload, refresh items. (`runVarsBrowser` loop: `runBrowser` → `runVarsSet(path,"",false)` → reload + rebuild; cancel exits.)
- [x] Non-interactive/container guard: `!widgets.IsInteractiveFn(...) || cmdctx.NonInteractiveEnv()` → call `vars list` (mirror of `command.go`; call the EXPORTED `cmdctx.NonInteractiveEnv()` directly rather than copying `command.go`'s private wrapper). (Bare-command `RunE` routes namespace-arg / JSON / non-interactive / `NonInteractiveEnv()` to `runVarsList`; `isInteractive` is a test seam over `widgets.IsInteractiveFn`.)
- [x] Write tests: vars→Item mapping (tree namespaces, badges, value-in-description); non-interactive dispatch falls back to `list`. (`browser_test.go`: `TestBuildVarsBrowserItems`, `TestBareVars_InteractiveRunsBrowser`, `TestBareVars_NonInteractiveFallsBackToList`, `TestBareVars_NamespaceArgListsEvenInteractive`.)
- [x] HARD REGRESSION GATE: assert `dwe commands` browser output is byte-identical to today (run `cmdbrowser` golden tests + `dwe commands` browser tests). Specifically assert the command-mode footer STILL shows `e edit params` and the existing `ForceParamForm` tests still pass — the vars `ActionEdit` addition must not change anything in command mode. (`TestModeRun_FooterUnchangedByEditMode` asserts 9 entries / `e edit params` / `y skip confirm` / `enter select` / "command" noun; full `cmdbrowser` + `cli/command` suites green.)
- [x] `make embedded-docs` once, then `go test ./internal/core/ui/cmdbrowser/... ./internal/cli/vars/... ./internal/cli/command/...` (focused) — must pass before Task 10. (All three packages pass; `golangci-lint` clean.)
...
- [x] Add `"vars": true` to `bridgeAllowedTopLevel` with a comment: read subcommands always reachable; `set` is reachable but deny-by-default gated at runtime by `bridge.vars_writable` (enforced in Task 8's `set`, since `bridgeCommandAllowed` is prefix-wide and cannot see the var arg). TUI auto-disabled in container via non-interactive dispatch.
- [x] Add a `render config` nested exception in `bridgeCommandAllowed` (mirroring the existing `bridge status` exception): `render config` reachable from container, all other `render` subcommands (env/ide/ai/git) stay host-only. (`bridgeCommandAllowed` now returns true for `render config`; `applyBridgeContainerVisibility` keeps the `render` parent visible for its allowed `config` child.)
- [x] Guard `render config --harvest` in `config.go`: when `bridgeInvokedFromContainer()` is true, reject `--harvest` with a typed error — it mutates host `.dwe/generated.yml` and would be a write path that bypasses `bridge.vars_writable`. Only read-only render is container-reachable. (Uses `bridgeclient.InContainer()`; typed error `render_harvest_host_only`.)
- [x] Write tests: `bridgeCommandAllowed("dwe vars get/list/inspect")` → true; `dwe vars set ...` path is reachable (true) BUT a container `set` of a non-`vars_writable` var is denied by Task 8's runtime gate (cross-checked here or in Task 8); `dwe render config` → true while `dwe render env/ide/ai/git` → false; container `render config --harvest` → rejected; a non-allowlisted sibling top-level command stays blocked (regression guard). (`bridgepolicy_test.go` table + visibility cases for vars/render; `config_test.go` `TestNewConfigCmd_harvestRejectedInContainer` + `_renderAllowedInContainer`; container runtime-gate for non-`vars_writable` var is covered by Task 8's `set_test.go`.)
- [x] `make embedded-docs` once, then `go test ./internal/cli/...` (focused) — must pass before Task 11. (All `internal/cli/...` packages green; `golangci-lint` clean.)
...
- [x] Update `docs/internals/packages.md` + `AGENTS.md` (CLAUDE.md is the symlink) with the new load-bearing contracts: comment-preserving node writer (with node-aware `validateMergeable` + legacy port exception) is the single `local.yml` write path (vars/services/setup); shared `config.LoadLayers`/`ResolveLayeredPath` layer helper; `vars set` scalar-coercion grammar; field-aware `varsusage` scan + its caveat; `cmdbrowser` additive `ActionEdit` (existing `e`/ForceParamForm untouched); top-level `bridge.vars_writable` deny-by-default container-write allowlist (dot-boundary matcher) + `vars`/`render config` bridge reachability + the prefix-wide-allowlist runtime-gating note + the `render config --harvest` container block. Re-run `make build`. (AGENTS.md gained a new Critical Pattern bullet; packages.md updated across `project/local/`, `project/config/`, new `project/varsusage/` + `cli/vars/` entries, `cli/service/`, `cli/render/`, `workflow/setup/`, `ui/render/` vars renderers, `cmdbrowser` `ActionEdit`/`ModeEdit`, the Container command policy, and `tpl.VarPattern`. Naming corrected to the actual `config.LoadLayers`. `make build` re-synced embedded docs + content hashes; `go test ./internal/core/docs/...` green.)
- [x] Move this plan to `docs/plans/completed/20260611-dwe-vars-command.md`.
