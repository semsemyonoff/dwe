...
- [ ] put new defaults in `internal/ui/styles.go`:
...
- [ ] keep existing v1 `Style*` accessors as-is (other consumers depend on them); add a new file `internal/ui/cmdbrowser/palette.go` that constructs `charm.land/lipgloss/v2` styles from the `Color*()` strings — this keeps the v2 dependency confined to `cmdbrowser` (and any future v2 surface).
- [ ] unit tests in `internal/config/styles_test.go` (load YAML, struct-field assertions) and `internal/ui/styles_test.go`: (a) default `Color*()` returns the hardcoded fallback when no `styles.yml` loaded, (b) after `ApplyStyles(cfg)` with non-empty fields, `Color*()` returns the configured value, (c) empty field in `cfg.Colors` does NOT overwrite the default. Pure string assertions, no lipgloss types.
- [ ] `make test ./internal/config/... ./internal/ui/...` — must pass
