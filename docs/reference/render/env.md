...
- **Editing `.env` by hand** — the file is regenerated by `devbox render env --out .env` and by lifecycle hooks. Edit `devbox/defaults.yml` exports or `devbox/local.yml` overrides instead.
- **`--out` has no short form.** The flag is spelled `--out`; there is no `-o` alias. Use `devbox render env --out .env`.
