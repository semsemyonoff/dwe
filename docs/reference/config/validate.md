...
For `deploy.yml` and `reset.yml` these two Info states have a direct remedy: `dwe deploy eject --out workspace/deploy.yml --force` (or `dwe reset eject`) writes the built-in default over the inert file, turning it into a real, editable pipeline. `--force` is required here because the inert file already exists: `eject` refuses to overwrite **any** existing file without it, an authored pipeline included. Its refusal *message* keys on the same two conditions this validator uses — it says the file is inert only when it is — so the two commands always agree about which files are inert. `lifecycle.yml` has no eject equivalent — see [`deploy/index.md`](deploy/index.md).
