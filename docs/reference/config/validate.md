...
`config.template_refs` warns on a `${head.path}` reference whose head is a known merged-config root key (so it actually lives in `cfg.Raw`) but the remaining path does not resolve — almost always a typo, since `vars:` is the one root key with fully free-form content (`${vars.opechatka}` when only `vars.source.repo` was declared). It is deliberately silent on an unrecognized head (`${HOME}`, a stray dollar sign) and on the special namespaces that never live in `Raw` (`param`, `context`, `files`, `host`, `snapshot`, `args`, `generated`) — see [Two syntaxes: shorthand and full templates](../templates.md#two-syntaxes-shorthand-and-full-templates) in the Templates reference.

Its scope is every `${...}` reference the runtime actually renders, not pipeline steps alone: the templated fields of all workspace YAML (pipeline `cmd` / `with` / `check` / shell `when:`, `info.yml` `text`/`value`/`title`, command `cmd`/`argv`/`env`/`workdir`/`messages.*`, `docker.yml` `project_name`) plus the render bodies under `workspace/templates/config/**`. Only the `${...}` shorthand is checked — the Go-template form (`{{ resolve .Raw "vars.x" }}`) is not.

`config.container_name` warns when a compose service's `container_name:` diverges from the name dwe derives for it (`<project>-<service>`). The defect is divergence itself, not casing: a mismatched `container_name` is a foot-gun for raw `docker`/`docker compose` usage, scripts, and documentation that assume the derived name. Silent when the declared value already matches the derived name, or when it is an interpolated `${...}` value that cannot be compared without resolving the environment.

`config.ports_exports` warns when a service declares `services.<name>.ports.<key>` but no `exports.env` rule reads from `services.<name>.ports.<key>`. Such a port is display-only: a `local.yml` override of it will not move the actual container binding anywhere visible, and `dwe test`'s automatic host-port isolation (see [`tests.md`](tests.md)) silently does not apply to it either.
