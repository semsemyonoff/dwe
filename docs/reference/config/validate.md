...
`secrets.unresolved` is the **second exception** to the rule that only `env.*` and `checks.*` run in preflight (the first is `config.validate`). It is a readiness question — "can this machine actually deploy right now" — not a content one, so `dwe run` / `dwe restart` / `dwe stop` / `dwe deploy run` / `dwe reset run` and the deploy wizard's own pre-flight gate all stop with the same named fix (the service-scoped `dwe stop|restart <name>` bypass compose and preflight both, by design), while `dwe status`, `dwe docs`, `dwe validate` and `dwe prompt` keep working. Unresolved values are grouped **by reason**, one diagnostic per reason listing the sorted paths, so the single actionable fix is not buried under one identical row per marker. The reasons are `no_identity` (no source anywhere), `invalid_identity` (a source **is** set but holds no age key — a truncated `DWE_AGE_KEY`, a keyfile that lost its key line; the message names the source, from fixed wording, because an `age` parse error echoes the private-key bytes it choked on), `wrong_identity` (a readable identity for another recipient) and `corrupt` (a damaged payload, not a key problem). At an interactive `dwe run` / `dwe restart` / `dwe deploy` a *missing* identity is [offered](secrets.md#the-offer-inside-dwe-deploy-dwe-run-and-dwe-restart) before preflight runs, so this validator sees only what the offer never covered — a non-interactive run, or a broken source that importing a key would not repair. A *declined* offer does not reach it: the command ends at the gate with the same fix instruction.
...
Under a **per-service deploy** the probe is narrowed to the ports that run can actually bind: `dwe deploy run --service <name>` (and the deploy `dwe services enable|disable --apply` performs) checks only the named services plus the transitive `depends_on` closure of their `service.yml` declarations — what `docker compose up <name>` brings up. A foreign container holding some *other* service's port therefore no longer blocks a run that never touches it. Whole-project runs (`dwe deploy run`, `dwe run`) and `dwe validate` are unchanged: they check every enabled service. The closure follows `service.yml` `depends_on` only, so a dependency declared solely in raw compose is not counted — it surfaces as a compose bind error inside the step instead.

`config.compose_project_name` warns when the active compose `-f` chain declares a top-level `name:` that differs from the project name dwe passes via `docker compose -p`. dwe always invokes compose with `-p <resolved>` (the resolved `workspace/docker.yml` `project_name`, else the canonical `<prefix>-<name>` from `project.name`), which silently overrides any top-level `name:` in the compose files. A divergent `name:` is therefore dead config — the real project scope (container/network/volume labels) is dwe's resolved name, not the one the file appears to declare — and a foot-gun for anyone running raw `docker compose` without dwe's `-p`. Since `.env` now carries `COMPOSE_PROJECT_NAME` (see [`render env` → System variables](../render/env.md#system-variables)), raw compose run from the project root already follows dwe's name — so a divergent `name:` no longer even holds for that case, and the resources created under it become invisible to raw compose. The check models compose's real precedence: it scans the active chain (`ComposeFiles()` — enabled overlays only) in `-f` order and compares the **last** declared `name:` (later `-f` overrides earlier), so a base `name:` that a later overlay already corrects does not warn. The fix is to align the two: change the effective compose `name:` to match, set `docker.yml` `project_name` to match and drop the redundant compose `name:`, or write `name: ${COMPOSE_PROJECT_NAME}` in the compose file — that now resolves from the generated `.env`. The check stays silent when the effective name uses unresolved interpolation (e.g. `name: ${COMPOSE_PROJECT_NAME}` — cannot be compared) or when `project.name` is unset (dwe omits `-p`, so compose honours the file's own `name:`).

`config.formal_block_fields` warns when a formalized top-level config block carries an unknown nested key. The merged 3-layer config is decoded leniently (plain `yaml.Unmarshal`, not `KnownFields`), so a typo under one of these blocks — e.g. `stop: { port_release_timeot: 0 }` — is silently dropped and the field falls back to its default, quietly changing behavior. The strict-root allowlist catches unknown *top-level* keys with a hard error, but not nested ones; this check fills that gap as a non-fatal `dwe validate` warning (it does not run in preflight, so it never blocks `dwe run`). It scans all three layer files (`workspace.yml`, `workspace/defaults.yml`, `workspace/local.yml`) and anchors each finding to the offending file and line. The recognized-key set for each block is derived by reflection from the backing Go struct's YAML tags, so it cannot drift when a struct gains a field. Covered blocks (immediate children only — the check does not descend into nested structures like `runtime.spx`): `project` (`name`, `prefix`), `runtime` (`use_https`, `spx`), `exports` (`env`), `compose` (`base`, `extra`), `docs` (`mermaid`, `cache_size_mb`), `update` (`mode`), `bridge` (`vars_writable`), `stop` (`port_release_timeout`), `secrets` (`recipient`). The free-form (`vars`) and per-service (`services`) blocks are intentionally excluded.
...
`config.exports` warns on an `exports.env` rule whose `from:` or `when:` dot-path does not resolve in the merged config. These are bare dot-paths, not `${...}` references, so `config.template_refs` never sees them: `from: vars.db.passwrod` is a well-formed string that the loader accepts and [`dwe render env`](../render/env.md) renders as `DB_PASSWORD=` — an empty value that reaches every container as if it had been declared that way. An unresolvable `when:` is quieter still: it is falsy, so the rule is skipped and the variable never appears at all — and because that makes the rule dead, its `from:` is not reported a second time. The hint names the consequence of the particular rule shape — the variable renders empty, the `default:` is always used, or `dwe render env` fails on a `required:` rule. The gate is read first, as the renderer reads it: when a `when:` resolves but is falsy, the rule is skipped before `from:` is ever resolved, so the hint instead says the gate is falsy right now and the miss surfaces once it turns on. It is a warning, not an error: `from:` paired with `default:` is a legitimate optional-path pattern, and a path may live in a `local.yml` that is not on this machine. A path that resolves to a present key holding `null` is **not** reported — the key exists, so the empty render is the author's own. The finding is anchored to the layer that declares the rule (`workspace.yml`, `workspace/defaults.yml` or `workspace/local.yml`).

The `commands` domain applies the same criterion to the three dot-path fields in `workspace/commands/*.yml`: `params.<name>.default_from`, `params.<name>.options.from` and `context.<name>.from`. Each warns when the path does not resolve in the merged config — an unresolvable `default_from` leaves the parameter with no default, an unresolvable `options.from` leaves the choice list empty, and an unresolvable `context.from` renders `${context.<name>}` as an empty string inside the command body. Command files are checked as written, so daemon-expanded synthetic commands are never reported twice.

The same domain checks `hide:` on commands and `group:` blocks. Beyond template syntax, every expression is rendered against the merged config — the context runtime visibility uses — and a render failure warns `hide: expression does not evaluate: …`. Runtime evaluation is fail-open, so such an expression otherwise leaves the command visible with only a log line. The rendered result must then be something runtime can evaluate: a literal `true` / `false` / `1` / `0` or empty, a `cmd: <shell command>`, or a [`when:` predicate](conditions.md) with its arguments (`dir-exists <path>`, `generated-missing <svc> <field>`, …). Anything else — `yes`, a misspelled verb such as `dir-exist`, `file-exists` without a path — warns `hide: expression renders to "…", which is neither a boolean nor a known predicate`; at runtime it too would leave the command visible. A rendered `cmd:` or builtin predicate is never executed by `dwe validate` (only an empty `cmd:` is reported, as ``hide: expression renders to an empty `cmd:` command`` — at runtime it is an evaluation error), and the render check is skipped when the config failed to load. Only the result for the current config is checked: a template branch that config does not take is not. See [Hide condition](commands/directives.md#hide-condition).

The same domain checks the target of `service_exec`, `service_run` and `daemon` commands. Runtime passes `service:` (or `runner.service:`) straight to `docker compose exec` / `run`, so it must name a **compose service**, not a dwe service key. The check warns `service: "<name>" is not a service in any compose overlay, disabled services included` when no git-tracked compose file declares that service. Those files are `compose.base` plus every service's `compose:` and `compose_after:`, disabled services included, so a command aimed at a compose-only service of a currently disabled tool is not reported. Local overlays are not consulted: `local.yml` `compose.extra` (project-wide or per-service) and the generated bridge overlay vary per machine, and the command file is shared, so every checkout gets the same answer and a service only a local overlay defines is reported. The hint suggests the closest compose name, or, when the value is a dwe service key, the compose service that key runs as. When no tracked file declares any service, the hint says that instead. The check is skipped when the value is templated (`${param.*}`, `{{ … }}`), when the config failed to load, and when some file in the set cannot be read or parsed, or pulls in more files through a top-level `include:`. `config.compose_files` reports a file of that set that is missing or unreadable, but not one that fails to parse.
...
`config.compose_files` warns when a file listed under a service's `compose:` or `compose_after:` does not exist (or is a directory); it applies the same check to [`compose.base`](workspace.md#compose), which heads every `-f` chain. Without it a typo surfaces only as a `docker compose` error on the next `dwe run`. Paths resolve the way compose receives them — against the project root, not the service folder — and when the missing path does exist relative to the service folder the hint names that file. Every service is checked regardless of its enabled state: `--all` compose operations load a disabled service's files, and enabling it later would break the stack. A path a child inherits through `extends:` is reported once, naming every service that lists it. It is a warning, not an error, because a compose file may live in a source tree that `dwe deploy run` clones, so a fresh checkout can lack it until the first deploy. Per-developer `local.yml` `compose.extra` paths are not covered here: `LoadConfig` already rejects a missing project-wide one, or a per-service one of an enabled service.

`config.healthcheck_start_period` is an **info** note on a compose service in the active `-f` chain (enabled services only) whose healthcheck runs a test but declares no `start_period`. `dwe run` brings the stack up with `docker compose up --wait`, and without a `start_period` every probe a slow-starting service fails while booting counts against `retries` — with the defaults (`interval: 30s`, `retries: 3`) a service that needs longer than that is marked unhealthy and the whole run fails. The healthcheck is merged across the chain the way compose merges it, key by key: a `start_period` in an earlier file still counts, `!reset` drops the healthcheck and `!override` replaces it. Silent when the healthcheck is disabled (`disable: true`, `test: ["NONE"]`) or declares no `test:`; an explicit `start_period: 0s` counts as declared. A healthcheck baked into the image is not visible. It is info rather than a warning because most services start well inside `interval × retries` — the note names a failure mode, not a defect, so it never fails `--strict`.
...
`config.deploy`, `config.lifecycle` and `config.reset` apply the same three-state model to the pipeline files. An **absent** `workspace/deploy.yml` / `workspace/lifecycle.yml` reports `no <file> — built-in default pipeline is active` at `SeverityInfo`. An **all-comment or empty** file — the state `dwe init` scaffolds — reports `<file> has no active content (all comments or empty) — built-in default pipeline is active`, also at `SeverityInfo`: the file exists, but the built-in default is what actually runs, so reporting `SeverityOK` for it would claim a pipeline that is not in force. A file that **parses but carries no pipeline** is the same case in different clothes and reports at `SeverityInfo` too: `deploy.yml` / `reset.yml` with an empty or absent `phases:` reports `<file> declares no phases — built-in default pipeline is active`, and a `lifecycle.yml` missing either half reports `lifecycle.yml declares no run: section — built-in default run pipeline is active` (likewise for `stop:`, and one row per missing section). Only a file whose content actually reaches the engine earns `SeverityOK`. The plan cross-check (`plan resolution failed: …`) runs in every state, since the resolved plan is exactly what deploy executes.

For `deploy.yml` and `reset.yml` these two Info states have a direct remedy: `dwe deploy eject --out workspace/deploy.yml --force` (or `dwe reset eject --out workspace/reset.yml --force`) writes the built-in default over the inert file — without `--out` the document goes to stdout and no file is touched, turning it into a real, editable pipeline. `--force` is required here because the inert file already exists: `eject` refuses to overwrite **any** existing file without it, an authored pipeline included. Its refusal *message* keys on the same two conditions this validator uses — it says the file is inert only when it is — so the two commands always agree about which files are inert. `lifecycle.yml` has no eject equivalent — see [`deploy/index.md`](deploy/index.md).
