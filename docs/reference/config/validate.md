...
The same domain checks `hide:` on commands and `group:` blocks. Beyond template syntax, a template-only expression is rendered against the merged config — the context runtime visibility uses — and a render failure warns `hide: expression does not evaluate: …`. Runtime evaluation is fail-open, so such an expression otherwise leaves the command visible with only a log line. `cmd:` and builtin-predicate expressions are never executed or rendered by `dwe validate`, and the render check is skipped when the config failed to load. See [Hide condition](commands/directives.md#hide-condition).
