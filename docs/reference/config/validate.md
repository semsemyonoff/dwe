...
The same domain checks the target of `service_exec`, `service_run` and `daemon` commands. Runtime passes `service:` (or `runner.service:`) straight to `docker compose exec` / `run`, so it must name a **compose service**, not a dwe service key. The check warns `service: "<name>" is not a service in any compose overlay, disabled services included` when no file in the full overlay chain declares that service. Every configured overlay counts, so a command aimed at a compose-only service of a currently disabled tool is not reported. The hint suggests the closest compose name, or, when the value is a dwe service key, the compose service that key runs as. The check is skipped when the value is templated (`${param.*}`, `{{ … }}`), when the config failed to load, and when some file in the chain cannot be read or pulls in more files through a top-level `include:`. A missing compose file is reported by `config.compose_files`.
