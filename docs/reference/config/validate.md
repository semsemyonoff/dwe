...
For `deploy.yml` and `reset.yml` these two Info states have a direct remedy: `dwe deploy eject --out workspace/deploy.yml --force` (or `dwe reset eject`) writes the built-in default over the inert file, turning it into a real, editable pipeline. `eject` refuses to overwrite an *authored* file without `--force`, and its refusal keys on the same two conditions this validator uses, so the two commands always agree about which files are inert. `lifecycle.yml` has no eject equivalent — see [`deploy/index.md`](deploy/index.md).
