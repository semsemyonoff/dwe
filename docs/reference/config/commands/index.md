...
2. **Resolve context** — read each `context.<key>.from` dot-path out of the merged config.
   ...
   `dwe validate` warns (domain `commands`) when one of these dot-paths does not resolve in the merged config. The consequence differs per field: an unresolved `params.<name>.default_from` falls through to `default:`, or fails the parameter when it is `required:` with no other value; an unresolved `params.<name>.options.from` yields an **empty option list**, so a select form offers no choices; an unresolved `context.<name>.from` renders as an empty string. `dwe validate --strict` turns the warning into an error.
