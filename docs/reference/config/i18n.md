...
# Via userconfig (~/.config/workspace/config.yml or .dwe/config.yml)
# language: de
dwe commands list
# → Picks German
...
`DWE_LANGUAGE` is injected into the userconfig loader at startup; it replaces whatever `language` value lives in `~/.config/workspace/config.yml` or the project's local config. It sits **below** an explicit `--lang` flag and **above** the system `$LANG` variable:
...
The `--lang` flag accepts any 2-letter language code. If the locale is not available in your project, DWE falls back to English. The generated documentation always lives under `commands/<lang>/` (including English — there is no legacy `commands/` root in the new layout).
...
1. The DWE build process (`make build`) walks `docs/reference/` and `docs/internals/`.
...
1. When you view a translated document via `dwe docs show`, `dwe docs export`, or the TUI, DWE calls `ResolveContent(locale)`.
