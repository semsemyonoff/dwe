...
`DWE_LANGUAGE` is injected into the userconfig loader at startup; it replaces whatever `language` value lives in `~/.config/devbox/config.yml` or the project's local config. It sits **below** an explicit `--lang` flag and **above** the system `$LANG` variable:
