...
For a secret-like answer — an app key, a session secret, a Fernet key — point
the developer at `dwe vars set <path> --generate hex|base64url[:N]|uuid` in
`description:` rather than a one-liner such as
`python -c 'import secrets; …'`. The command needs nothing beyond dwe, writes the
same `local.yml` path, and refuses to overwrite an existing value without
`--force` (see [`vars.md`](vars.md#dwe-vars-set)).
