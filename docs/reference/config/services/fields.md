...
| `compose` | list | no | all | Additional compose overlay files activated when the service is enabled. Relative paths inside an overlay (bind mounts, `build.context`, `env_file`) resolve against the directory of the first file in the `-f` chain — [`compose.base`](../workspace.md#compose), the project root in the usual layout — not against the overlay's own directory: `dwe` passes no `--project-directory`, so Compose applies its multi-file rule. The listed paths themselves are relative to the project root; `dwe validate` warns ([`config.compose_files`](../validate.md#validation-domains)) when one does not exist, whether or not the service is enabled. |
| `compose_after` | list | no | all | Compose overlay files emitted after every service group (tool → infra → app) and before the generated bridge overlay and the project-wide `local.yml` `compose.extra` — same enabled gate as `compose`, one pass ordered by service name across all types, list order kept within a service. Use this for a patch that must win over an app's own overlay (whole-value keys like `command:`, `healthcheck:`, or an `environment:` entry both files set) rather than losing to it because the app group emits later. Between two `compose_after` files touching the same key the later service name wins — don't rely on it, patch disjoint keys. This fixes emission **order**, not the patched service's **presence**: a patch block for an app that is currently disabled still makes Compose reject the whole project. Not settable from an overlay layer (`workspace.yml`, `defaults.yml`, `local.yml`) — it belongs in `service.yml` like `compose`. Paths resolve the same way as `compose`. Unlike `compose`, not mirrored into the merged config — there is no `${services.<name>.compose_after}`, and no `exports.env` `from:` or command `default_from:` can read it. |
