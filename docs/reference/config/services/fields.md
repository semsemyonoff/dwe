...
| `compose` | list | no | all | Additional compose overlay files activated when the service is enabled. Relative paths inside an overlay (bind mounts, `build.context`, `env_file`) resolve against the directory of the first file in the `-f` chain — [`compose.base`](../workspace.md#compose), the project root in the usual layout — not against the overlay's own directory: `dwe` passes no `--project-directory`, so Compose applies its multi-file rule. |
