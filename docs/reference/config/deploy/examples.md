...
- **Frame-aware parser**: the `\r`-aware `lineTee` parses a child's stream into `(frame, final)` callbacks — directly for parallel sub-steps, through `frameLogWriter` for sequential step bodies. Only the latest frame is shown on the live row. In log files a non-final (`\r`-terminated) frame is *pending*: the next frame evicts it, so only the last frame of a redraw run is written and `50%\r100%\n` logs a single `100%` line rather than two. A run that ends on a bare `\r` is emitted by the writer's `Flush()` at step end. A `\r\n` that lands in two different reads does not collapse in-scan; `lineTee` itself holds the `\r`-closed frame and re-emits it in place of the empty frame the `\n` would otherwise produce, so the line survives the read boundary on both routes. This is frame collapsing, not terminal emulation: `abc\rX\n` is logged as `X` while a terminal shows `Xbc`, `foo\r\x1b[K\n` is logged as `foo` while a terminal shows a blank line, and whole-frame redraws driven by cursor-up sequences (compose's `[+] up 2/3` block) are out of scope and still land once per redraw.
...
- **Per-sub-step log files**: `.dwe/logs/parallel/<pipeline>/<group>/<sub>.log` captures the sub-step's committed lines (ANSI stripped, non-final `\r` frames dropped — unless the frame that closes the line carries no text, as for a `\r\n` split across two reads, where the held content frame is committed instead of a blank one). A tail the child left without a closing newline — `\r`-terminated or not — is not written to this file; it still reaches the global pipeline log, because the same frame also goes to `Reporter.StepOutput`, which commits the trailing tail when the sub-step finishes. The global pipeline log (`.dwe/logs/<pipeline>.log`) receives every status line and every committed child line exactly once.
