...
- **Per-sub-step log files**: `.dwe/logs/parallel/<pipeline>/<group>/<sub>.log` captures the sub-step's committed lines (ANSI stripped, non-final `\r` frames dropped). A tail the child left without a closing newline — `\r`-terminated or not — is not written to this file; it still reaches the global pipeline log, because the same frame also goes to `Reporter.StepOutput`, which commits the trailing tail when the sub-step finishes. The global pipeline log (`.dwe/logs/<pipeline>.log`) receives every status line and every committed child line exactly once.
