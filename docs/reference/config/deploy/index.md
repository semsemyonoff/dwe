...
The string leaves of `with:` are the one exception: they are rendered whenever they carry a known-head `${...}` **or** a literal `{{`, because a `with:` value has always been passed through the template engine before reaching the command. So `with: {host: '{{ resolve .Raw "vars.db.host" }}'}` still resolves, and the mixed-form restriction above applies to `with:` leaves too.
