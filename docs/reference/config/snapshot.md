...
Declarative snapshot workflows: capture the state of a DWE project (databases, indices, DWE local config, deploy state) into a named directory under `./snapshots/<name>/` and restore or roll back to it.
...
A snapshot captures a known-good state of mutable project data — typically databases, search indices, service-branch metadata, and the workspace files that pin the developer's local configuration — into a self-contained directory. Restore is a soft operation: the `restore` workflow runs and the workspace files are swapped back into place. It does **not** invoke `reset`, recreate containers, or reapply deploy steps.
...
dwe snapshot restore feature-x-wip          # back to WIP
dwe snapshot rollback                       # quick: restore the rollback_target
```
...
| `rollback_target` | string | — | Name of the snapshot used by `dwe snapshot rollback`. Must point at an existing snapshot. |
...
| `local_yml` | block | — | Override policy for `workspace/local.yml` keys (see [`local_yml.preserve_keys`](#local_ymlpreserve_keys)). |
...
- `dwe snapshot create x` → default block.
- `dwe snapshot create x --using=db-only` → `create.variants.db-only`.
- `dwe snapshot restore x` → uses `restore.variants[<manifest.variant>]` if set; falls back to the default `restore` block when the variant is missing on the restore side.
...
| `block` | Any non-empty diff aborts before any side effect on `workspace/local.yml` (exit 1, typed `ServicesMismatchError`). |
...
`workspace/local.yml` typically contains machine-specific overrides (ports, hostnames, paths) that should not travel with a snapshot. `preserve_keys` lists dot-paths whose **current** values survive restore even when the snapshot ships a `local.yml`.
...
**Create**: `captureDWEFiles` reads `workspace/local.yml`, calls `stripPreservedKeys` to remove the listed dot-paths, and writes the result into `<snap>/workspace/local.yml`. If every top-level key was preserved (resulting in an empty mapping), the file is still written so restore semantics are unambiguous.
...
`dwe snapshot pack <name>` produces a single `./snapshots/<name>.tar.gz`. No `.sha256` sidecar is written — one file per snapshot. The in-memory sha256 of the archive is shown in the success message for ad-hoc reference but is not required by `unpack`. Transport bitflips are caught by gzip CRC32 and tar structural validity; in-archive tampering of individual artifacts is caught by `unpack`'s manifest-driven verification (see below).

## `unpack`

`dwe snapshot unpack <tar-path> [--as=<name>] [--no-verify] [-y]` extracts an archive into `./snapshots/<final-name>/` using the existing distrust-safe extract contract (`filepath.IsLocal`, no symlinks, 50 GiB size cap, 100 000 entry cap) into a sibling `./snapshots/.unpack-<random>/` staging dir, then atomic-renames into place.
...
- Copies `workspace/local.yml` and `.dwe/deploy/state.yml` into `<snap>/workspace/` before running the workflow.
- Runs the selected create workflow with `${snapshot.*}` available in `create` scope.
- Scans the resulting directory (excluding `manifest.yml` and `workspace/`), streaming sha256 per file. Symlinks inside the snapshot directory are rejected.
...
- Backs up the current `workspace/local.yml` and `.dwe/deploy/state.yml` into `.dwe/snapshots/.pre-restore-backup/` atomically. The previous backup is overwritten.
- Restores workspace files from `<snap>/workspace/` over the working copies.
...
`dwe validate snapshot [<name>] [--verify]` exposes static checks:

| Validator | Severity | Trigger |
|---|---|---|
| `snapshot.config_loadable` | error | `workspace/snapshot.yml` exists but does not parse. Absent file is silent. |
| `snapshot.create_defined` | info | `create:` block missing — `dwe snapshot create` will refuse to run. |
...
See [commands/types.md](commands/types.md#type-workflow) for the `WorkflowStep` shape reused by snapshot workflows, and [state/index.md](state/index.md) for the deploy state journal that snapshots back up alongside `workspace/local.yml`.
