...
> **Note** — only deploy pipelines have a state file. [`type: daemon`](../commands/types.md#type-daemon) commands have **no on-disk registry**: `docker ps` (filtered on the standard `dwe.project` / `dwe.daemon.id` / `dwe.daemon.params` labels) is the single source of truth for which daemons are running. There is no journal to drift, lock, or invalidate; a `docker stop` issued outside devbox is reflected immediately on the next `devbox status daemons` read.
