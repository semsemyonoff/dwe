...
The match is by **effective name**: the volume's explicit `name:` after the `-f` chain collapses (a later overlay's `name:` wins), or the compose map key when no file sets one, compared against the shared `docker.yml` volume's own name (a `shared: true` volume resolves to its `name:` verbatim, without the project prefix). A mismatch is not acknowledged, and neither is `shared: false`. Both the `external_volume` and the `named_volume` finding for that volume are marked together.
