...
The scan behind this runs on the scenario's **view of the project**: the working tree with the scenario's `env:` applied and the per-developer `compose.extra` overlays removed — as the copy's seeded `local.yml` removes them — evaluated before the copy is made. `dwe validate tests` and `dwe test list --output json` build the same view, so a compose file reached only through a `compose.extra` overlay is invisible to all three: the copy never runs it.
