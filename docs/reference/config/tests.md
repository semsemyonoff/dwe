...
**Interpolated host ports are remapped only through what they read.** A compose host port written as a variable (`"${VALKEY_PORT:-6379}:6379"`) moves only when that variable carries a remapped value: an `exports.env` rule `from: services.<name>.ports.<x>` for a service the runner remaps in this scenario, or `from: vars.<path>` with `env.vars: { <path>: auto }` in the scenario. Anything else — a `vars:` path without `auto`, a rule whose `when:` is falsy, a variable from the host environment or a hand-written `.env` — binds the original port in the copy and collides with the working environment. The runner does not remap such a port; the [scanner](#interpolated-host-ports) warns about it.
...
| A host port that is exactly one variable (`"${VALKEY_PORT:-6379}:6379"`, `published: "${PORT}"`) | `interpolated_host_port` | Warning — collides only in a scenario that does not remap the variable; see [Interpolated host ports](#interpolated-host-ports) |
...
The runner remaps a host port only through the value it writes into the copy, so a port written as a variable (`"${VALKEY_PORT:-6379}:6379"`, `$PORT`, `${PORT?}` …) moves only when that variable is exported from a remapped value. The scanner follows the variable to its `exports.env` rule — the last one with that `name:` whose `when:` is empty or truthy, exactly as `.env` is rendered — and the finding depends on what the rule reads:
