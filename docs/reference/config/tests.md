...
2. Scan the scenario's view of the project for [compose isolation](#compose-isolation-scanner) hazards and plan the host ports the copy will allocate. A blocking finding fails the scenario here, before anything is created.
3. Copy the project into an isolated tree.
4. Generate the copy's `local.yml` (seed + this scenario's `env:` + the planned ports + a fresh identity) and a docker identity file, then write a durable run manifest.
5. Run `dwe validate` in the copy (cheap fail-fast).
6. Run `dwe deploy run --silent` in the copy — the real deploy pipeline, not a re-composed approximation.
7. Run `steps:` in-process against the copy's config.
...
2. this scenario's `env.vars` / `env.services`, plus every port the runner allocates for this copy — the remapped `services.<name>.ports` and every `vars:` path it remaps, **including traced paths the scenario never mentions** (below);
3. identity: `project: { prefix: <compose project name> }` and `update: { mode: "off" }` (no self-update prompts inside a disposable test run).

**Automatic host-port isolation.** Every host port declared under `services.<name>.ports` by a service that will be enabled in the copy is remapped to a freshly allocated free port, written into the generated `local.yml` as a `services.<name>.ports.<x>` override (the original port's scheme is preserved). Because `ports_free` preflight reads that same field — and a project that sources its compose host bindings from `services.<name>.ports` (directly, or via an `exports.env` entry `from: services.<name>.ports.<x>`) binds from it too — the preflight and the actual bind move together, so a scenario runs alongside the working environment with no port config. Every `vars:` path the copy remaps — the traced ones below and any `env.vars: { …: auto }` — is allocated in the same batch. All ports come from one allocation pass (all listeners opened before any is closed, guaranteeing intra-batch uniqueness); the copy's `ports_free` preflight still catches host-level races, and on a deploy failure that looks like a host-port bind conflict (with any allocated port present) the runner re-allocates every port and retries the deploy **exactly once** before failing the scenario.

**Interpolated host ports are remapped through the variable's `exports.env` rule.** A compose host port written as a variable (`"${VALKEY_PORT:-6379}:6379"`) moves when the scanner can trace that variable to a value the copy controls — the last `exports.env` rule with that `name:` whose `when:` is empty or truthy, exactly as `.env` is rendered:

- `from: vars.<path>` → **always remapped**, with no scenario config: the runner allocates a free port for `<path>` in the same batch as everything else and writes it into the generated `local.yml`. A scenario that pins the path to a concrete number (`env.vars: { ports.valkey: 6380 }`) keeps that number instead — the author's decision about which port the copy binds, not a collision.
- `from: services.<name>.ports.<x>` → remapped in the scenarios that remap `<name>`: enabled in the copy and not listed under `env.services.disable` (a `required: true` service stays enabled, so it stays covered).
- anything else — another path, a `.port` sub-path, an undeclared port, a falsy `when:`, a variable from the host environment or a hand-written `.env` — is **not** remapped. The variable keeps whatever value it would have had outside a test run: an expression carrying a default falls back to it (`"${VALKEY_PORT:-6379}:6379"` → `6379`) and a value coming from the host environment or a hand-written `.env` survives, both of which make the copy bind the working environment's port; a variable with neither resolves to the empty string, and compose then publishes no host port for that entry at all. The [scanner](#interpolated-host-ports) warns in every one of these cases.

The scan behind this runs on the scenario's **view of the project**: the working tree with the scenario's `env:` applied and the per-developer `compose.extra` overlays removed — as the copy's seeded `local.yml` removes them — evaluated before the copy is made. `dwe test list --output json` scans the same view, so a compose file reached only through a `compose.extra` overlay is invisible to everything it reports: the copy never runs it. `dwe validate tests` builds the view per scenario too, but only to decide [coverage](#interpolated-host-ports) — its own scan is project-wide, so a finding of any other kind in such an overlay still warns there.
...
| Host ports a compose file reads from a `vars:` path through `exports.env` | a free port from the same batch, written as that `vars:` path in the generated `local.yml` |
...
When a scenario does **not** pass (deploy failure, step failure, or timeout) and `--keep` was not used, the runner **attempts to collect** a failure report into `.dwe/tests/reports/<scenario>/` **before** teardown destroys the environment — so the debugging material survives. The directory is cleared and rewritten on every non-passing run that got as far as creating the environment (the latest failure is what you debug); a passing scenario or a `--keep` run never touches it. A scenario stopped by the [isolation gate](#compose-isolation-scanner) is the one other exception: it fails before anything is created, so no collection is attempted and a report left by an **earlier** failure of that scenario stays in place — check the run's own warnings, not the directory's timestamp.
...
`dwe test list --output json` carries a `cost_profile` object per scenario — what running that scenario would cost, and how far its isolation reaches. Every chain-derived number is computed on the scenario's [view of the project](#isolation-model), so the per-developer `compose.extra` overlays are excluded: a compose service that exists only in `workspace/local.yml` counts toward nothing here, because the copy never receives it.
...
| `isolation_findings` | The **non-blocking** findings of the [compose isolation scanner](#compose-isolation-scanner) — `named_volume` / `external_volume` / `named_network` / `external_network`, i.e. resources shared with the working environment, plus `interpolated_host_port`, a host port interpolated from a variable this scenario's copy does not remap. The blocking kinds (`container_name`, `raw_host_port`) are omitted: they abort the scenario before the copy is even made. A volume [acknowledged](#volumes-acknowledged-by-shared-true) by a `docker.yml` `shared: true` declaration stays listed and carries `"shared": true` (the key is omitted otherwise) — the profile reports facts, not verdicts, so a caller that only wants unacknowledged hazards filters on it. Full messages come from `dwe validate tests`. |
...
| A host port that is exactly one variable (`"${VALKEY_PORT:-6379}:6379"`, `published: "${PORT}"`) | `interpolated_host_port` | Warning when the variable is untraced, or reads a service port the scenario does not remap; a variable traced to a `vars:` path is remapped automatically and never warns. See [Interpolated host ports](#interpolated-host-ports) |
...
Container-port-only entries (random host port) are not flagged, and neither is a host-port token that mixes a variable with other text (`80${X}`, `${A}${B}`) — only a literal port number or range, or a token that is exactly one variable. IPv6-bracketed hosts (`[::1]:8080:80`) are out of scope and are not flagged (rare in dev compose). A `container_name:` finding is always emitted regardless of which compose service it's on — mapping a compose service back to a dwe service key isn't reliable, so false positives are cleared with `--skip-isolation-check` rather than suppressed at the source.
...
- **`dwe test run`** — runs the scan on the scenario's [view of the project](#isolation-model) **before the copy is made** (the same scan decides which `vars:` paths get an allocated port). Every finding that is not [acknowledged as shared](#volumes-acknowledged-by-shared-true), and not an [interpolated host port](#interpolated-host-ports) the scenario covers, is printed as a warning. A **blocking** finding fails the scenario immediately, with nothing created: no copy, no compose project, no teardown and no [failure report](#failure-reports) collected for that run (one left by an **earlier** failure of the same scenario stays in place) — the scenario is reported `failed` (exit code 1), and under `--keep` there is no kept copy to print. `--skip-isolation-check` downgrades every finding — blocking or not — to a warning and the run proceeds. See the [ports prerequisite](../../guides/integration-tests.md#ports-are-isolated-automatically) for how to avoid a `raw_host_port` finding in the first place: model the port under `services.<name>.ports`.
- **`dwe validate tests`** — emits every finding that is not acknowledged as shared as a warning (an interpolated host port only while some scenario leaves it uncovered), regardless of `Blocking`; static validation never fails a build over an isolation hazard, it only surfaces it early.
- **`dwe test list --output json`** — lists the non-blocking findings the scenario does not cover under `cost_profile.isolation_findings`, acknowledged ones included and marked `"shared": true`.

### Interpolated host ports

A host port written as a variable (`"${VALKEY_PORT:-6379}:6379"`, `$PORT`, `${PORT?}` …) moves only when the copy controls the variable's value. The scanner follows the variable to its `exports.env` rule — the last one with that `name:` whose `when:` is empty or truthy, exactly as `.env` is rendered — and what the rule reads decides the rest:

| The variable's rule | Covered in a scenario when | Message |
|---|---|---|
| `from: vars.<path>` | **always** — the runner allocates a free port for `<path>` in every copy, and a scenario that sets `env.vars: { <path>: <number> }` pins its own port instead | states the fact: the variable is remapped through `vars.<path>` |
...
| anything else — another path, a `.port` sub-path, an undeclared port, a falsy `when:`, no rule at all (host environment, a hand-written `.env`, the `:-` default) | only when its compose file is not in the scenario's stack (see below) | names the two routing fixes: export the port from a declared service port (`from: services.<name>.ports.<port>`), or from a `vars.<path>` through an `exports.env` rule, which dwe test then remaps automatically |
...
- `dwe test run` drops it when the scenario being run covers it — always, for a `vars:`-traced variable — and prints it as a warning otherwise.
- `dwe test list --output json` does the same per scenario, so `cost_profile.isolation_findings` never lists a `vars:`-traced variable; `when:` conditions are evaluated with the scenario's service toggles applied.
- `dwe validate tests` has no scenario of its own, so it drops the finding only when every scenario covers it, and otherwise appends `(not covered in scenarios: a, b)`. A `vars:`-traced variable is therefore silent in a project whose scenarios all load, and a scenario file that fails to load covers nothing. A finding of the last table row warns without a scenario list, unless no scenario's stack includes its compose file. Here `when:` is evaluated on the project's own config, so a rule gated on a service that is off by default but enabled by every scenario still warns.

Because `dwe test run` runs `dwe validate` inside the copy, and the copy carries every scenario file, `dwe test run a` still streams this project-wide warning while scenario `b` leaves an untraced variable uncovered. It does not block the run.
...
- **Host ports dwe cannot trace aren't isolated.** The automatic remap covers ports declared under `services.<name>.ports` and `vars:` paths an `exports.env` rule routes into a compose interpolation; a host port hardcoded straight in a raw compose file (`8080:8080`), or interpolated from a variable no active rule traces, bypasses it — and `ports_free` preflight sees neither. Declare the port under `services.<name>.ports`, or export the variable from a `vars.<path>` through an `exports.env` rule. The isolation scanner flags a literal as a **blocking** `raw_host_port` finding and an untraced variable as a non-blocking `interpolated_host_port` warning — only a warning, so a scenario that ignores it still binds the original port.
