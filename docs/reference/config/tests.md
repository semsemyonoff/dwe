...
- anything else — another path, a `.port` sub-path, an undeclared port, a falsy `when:`, a variable from the host environment or a hand-written `.env` — is **not** remapped. Compose falls back to the literal default (`:-6379`), the copy binds the working environment's port, and the [scanner](#interpolated-host-ports) warns about it.
