...
- **Live view (TTY)**: the reporter drives a `LiveLine` sticky footer (`bubbles/v2/spinner` + `[N/M] <step>: <last-output-line>`) for the whole pipeline, and switches to a `LiveBlock` while a parallel group is active (one row per sub-step, per-row spinner replaced by ✓/✗/◎ when the sub-step settles). The live view is rendered via the bubbles `Model.View()` API plus a private `time.Ticker` — `tea.NewProgram` is NOT used (so the terminal stays in cooked mode, Ctrl+C still raises SIGINT via VINTR, and no capability queries or kitty-keyboard sequences are emitted).
- **Frame-aware parser**: each parallel sub-step gets a PTY so children (curl, docker compose, ...) emit normal TTY progress. A `\r`-aware `lineTee` parses the stream into `(frame, final)` callbacks; only the latest frame is shown on the live row, and `\r` frames are normalised to one-frame-per-line in log files via `logSanitizer` (ANSI stripped, `\r`→`\n`).
...
- **Per-sub-step log files**: `.devbox/logs/parallel/<pipeline>/<group>/<sub>.log` captures the sub-step's full output (ANSI stripped, `\r`→`\n`). The global pipeline log (`.devbox/logs/<pipeline>.log`) receives every status line and every final-frame child line exactly once.
- **Prompt handoff**: every `ui.Run*` huh prompt fires package-level hooks registered by `NewPlainReporter` (`ui.SetHuhHooks(live.Pause, live.Resume)`), so the footer is erased before the prompt renders and repainted after it returns. The executor no longer wraps step bodies in coarse Suspend/Resume calls.
...
- **`childIO`** never writes to `os.Stdout` / `os.Stderr` directly in parallel mode (`ActionContext.Parallel = true`); all child output is routed through the reporter's `StepOutput` callback so the single-mutex single-copy guarantee holds.
