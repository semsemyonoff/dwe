...
| `slices` | `first`, `last`, `slice`, `join`, `reverse`, `uniq`, `has` | List/array operations |
| `maps` | `keys`, `values`, `hasKey`, `pick`, `omit` | Map/object operations |
...
**The value goes last.** Nearly every sprout function takes the map, list or string it operates on as the *last* argument (`merge`, `mergeOverwrite` and `concat` take the destination first), so they read naturally in a pipe: `{{ $m | pick "a" }}`, `{{ $list | append "x" }}`, `{{ $name | regexReplaceAll "-" "_" }}`. Templates written for Sprig's argument order need rewriting — see [Upgrading DWE](../guides/upgrading.md).
