...
How Devbox remembers what has already been deployed, how it serialises concurrent runs, how it recovers from a crash mid-pipeline, and how pending state defers work between `devbox services enable` and the next `devbox deploy run`.
...
Devbox achieves this with one on-disk file per project, `.devbox/deploy/state.yml`, and two cooperating file locks, `.devbox/deploy/deploy.lock` and `.devbox/snapshots/snapshot.lock`. The journal records what ran and what each step's body looked like; the locks serialise concurrent mutators so the journal can never be written by two processes at once.

> The journal is for the deploy pipeline only. `devbox run`, `devbox stop`, `devbox restart`, and `devbox reset run` execute every reachable step on every invocation — there is no "this already ran" optimisation outside deploy. Daemons (`type: daemon` commands) have no journal either: `docker ps` filtered on the standard `devbox.*` labels is the single source of truth for which daemons are running.

## The state journal

`.devbox/deploy/state.yml` is created and maintained by `devbox deploy run`. It is not checked into version control (the `.devbox/` prefix is in the project's `.gitignore`). Its shape is:
...
Field-level reference: [`config/state/schema.md`](../config/state/schema.md). Inspect with `devbox deploy state show`, clear with `devbox deploy state clear`, recompute aggregates with `devbox deploy state repair` ([`config/state/management.md`](../config/state/management.md)).
...
- **`config_hash`** comes in two scopes. The **service** hash covers `devbox/services/<name>/service.yml` + `devbox/services/<name>/deploy.yml` and invalidates every step under that service when it changes. The **project** hash covers tracked services + `devbox/deploy.yml` + per-service `deploy.yml` files for tracked services, and invalidates project-level steps the same way. "Tracked" means the service appears in the resolved plan (enabled + inlined by a `deploy_services: true` phase). Tools are never tracked.
...
`status` values for phases, services, and the project itself aggregate from the step level. `devbox deploy state repair` recomputes those aggregates from per-step records when something drifts.
...
Read-only commands (`devbox status`, `devbox docs ...`, `devbox info`, `devbox validate`) take no project locks. The docs subsystem in particular is explicitly read-only and never runs preflight, so opening docs while a deploy is running is always safe.
...
6. The skip table treats `failed` / `partial` / `in_progress` as "run (resume)", so `devbox deploy run --resume` picks up at the first non-`ok` step. In a TTY the runner offers a choice (resume / re-run all / cancel); in non-interactive mode, `--resume` or `--force` is required.

Force a clean slate with `devbox deploy run --force` (clears `state.yml` and re-runs every step) or wipe both journal and pending state with `devbox reset run`.

## Pending state

The `pending` block in `state.yml` captures work that has been queued but not yet applied. The canonical producer is `devbox services enable / disable` without `--apply`: the local override is written to `devbox/local.yml` immediately, but the restart or deploy that would actually realise the change is deferred.
...
| Event | Effect |
|-------|--------|
| `devbox restart` success | clears the `restart` op; any `deploy` op survives |
| `devbox deploy run` (full project) success | clears the `deploy` op; any `restart` op survives |
| `devbox deploy run --service <name>` success | removes `<name>` from the `deploy` op; removes the op when empty |
| `devbox reset run` (project-wide) success | clears all pending (full journal wipe) |
| `devbox reset run --service <name>` success | writes `{kind: deploy, services: [<name>]}` |
...
While `pending` is non-nil, `devbox status` (and its subcommands) prints a warning banner at the top of the output naming the pending action and the command to run. The banner disappears once the corresponding consumer succeeds. Schema and lifecycle reference: [`config/state/schema.md`](../config/state/schema.md#pending-state).
...
- [`config/state/management.md`](../config/state/management.md) — `devbox deploy state show / clear / repair`, the `--force` and `--resume` flags, non-interactive behaviour.
- [Pipelines](pipelines.md) — how the journal-skip decision fits into the per-step execution flow.
- [Reset](../config/reset.md) — what `devbox reset run` clears, and the always-on baseline that returns a project to a known clean state.
