...
| `secrets` | key material and writes to tracked layer files are host-only. Container *reads* of already-decrypted values stay open by design — `dwe vars get` and `render config` return plaintext, the same exposure the container already has through its rendered `.env`; see [secrets → Container behavior](../config/secrets.md#container-behavior) |
