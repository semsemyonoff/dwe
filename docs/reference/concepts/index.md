...
- [Docker integration](docker.md) — how DWE drives Docker Compose: project-name derivation, the compose file list assembled from base, tool/infra/app overlays, `compose_after` patches, and local, environment propagation, volume conventions, and why some lifecycle commands bypass compose and call `docker stop` / `docker rm` directly.
