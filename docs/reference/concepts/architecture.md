...
The CLI itself is a single static binary with docs and pipelines embedded. There is no DWE daemon, no companion service, no plugin loader — every invocation is short-lived and stateless except for what it writes under `.dwe/`.
...
- **DWE does not run as a daemon.** No background process, no socket, no tray app. Every command starts fresh, reads config, does its work, exits.
