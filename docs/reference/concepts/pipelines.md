...
The phase → step → condition execution model that `devbox deploy`, `devbox run`, `devbox stop`, and `devbox reset` all share. One grammar, one runner, one journal.
...
|---------|------|---------------------|
| `devbox deploy run` | `devbox/deploy.yml` + per-service `devbox/services/<svc>/deploy.yml` | Built-in `services → start → post-deploy` pipeline. |
| `devbox run` | `lifecycle.yml` → `run:` | Built-in `start` phase that calls `docker up --wait`. |
| `devbox stop` | `lifecycle.yml` → `stop:` | Built-in `stop` phase that calls `docker down`. |
| `devbox restart` | `lifecycle.yml` (both sections) | Runs `stop` then `run --no-update`. |
| `devbox reset run` | `devbox/reset.yml` (+ per-service `reset.yml`) | Built-in `pre → stop → cleanup` pipeline. |
...
- **`deploy_services: true`** on a phase is the orchestrator marker that inlines per-service deploy pipelines. It is allowed in `devbox/deploy.yml` only — rejected at load time in `lifecycle.yml` and `reset.yml`.
...
| `devbox` | Recursive call into the devbox binary | Compose passthroughs (`docker up`, `docker down`), render commands, `info`, anything reachable as a devbox subcommand. |
| `command` | Declarative command from `devbox/commands/` | Workflow / service-exec / service-run / builtin / daemon / script / shell / devbox commands declared in the registry. |
...
- **`when:`** — pre-condition. Evaluated before the body runs. Falsy → step skipped (not failed). Typed as `builtin` (filesystem predicates like `dir-empty`), `shell` (hardcoded `sh -c`), or `template` (Go template against the merged `DevboxConfig`).
- **`check:`** — post-action. Evaluated after the body succeeds. Failure → step is reported as failed. Same `type:` shape as a step body (`shell` / `devbox` / `command` / `builtin`), but its return value gates the step's success status rather than producing user-visible output.
...
- The journal-skip decision belongs to `devbox deploy run` only. `devbox run` / `stop` / `restart` and `devbox reset run` execute every reachable step on every invocation — there is no "this already ran" optimization outside deploy.
...
The reporter swaps the LiveLine footer for a LiveBlock during a parallel group, with one row per sub-step, the pipeline-wide `[N/M]` index per row, and a frame-aware parser that normalises `\r` progress lines into one frame per visible row. Full output goes to `.devbox/logs/parallel/<pipeline>/<group>/<sub>.log`.
...
- [Step execution types](../config/deploy/steps.md) — `shell`, `devbox`, `command`, `builtin`; `cmd: shell` builtin vs `type: shell` step.
