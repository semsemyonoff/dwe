...
- Сгенерированный `.env` — как зарезервированную системную переменную [`COMPOSE_PROJECT_NAME`](../render/env.md#системные-переменные); именно поэтому сырой `docker compose`, запущенный из корня проекта, попадает в тот же проект, что и DWE, — поверх любого верхнеуровневого `name:` в compose-файле.
...
5. Файлы `compose_after:` включённых сервисов, один проход по именам сервисов среди всех типов — патчи, которые должны побеждать все группы выше (см. [`compose_after` в справочнике полей `service.yml`](../config/services/fields.md#поля-сервиса-верхнего-уровня)).
6. Сгенерированный bridge-оверлей, `.dwe/compose.bridge.yml`, если он существует.
7. Общепроектный `compose.extra` из `local.yml`, всегда последним.

Порядок типов сервисов важен: сначала tools, потом infra, потом apps, потом патчи `compose_after`. Внутри группы — алфавитная сортировка по ключу сервиса (имени директории в `workspace/services/<name>/`). Явная сортировка делает список файлов детерминированным, так что `docker compose` всегда видит оверлеи в одном и том же порядке merge.

`dwe docker pull --all` и `dwe docker build --all` работают с тем же упорядоченным списком, но игнорируют флаг `enabled`, чтобы разработчик мог тянуть или собирать образы для оверлеев, отключённых локально, — без правки `workspace/local.yml`. Это касается и файлов `compose_after` отключённых сервисов.
