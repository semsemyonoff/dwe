...
- Сгенерированный `.env` — как зарезервированную системную переменную [`COMPOSE_PROJECT_NAME`](../render/env.md#system-variables); именно поэтому сырой `docker compose`, запущенный из корня проекта, попадает в тот же проект, что и DWE, — поверх любого верхнеуровневого `name:` в compose-файле.
