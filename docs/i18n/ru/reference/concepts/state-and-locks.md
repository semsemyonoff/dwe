...
Пока `pending` не nil, `dwe status` (и его подкоманды) печатает предупреждающий баннер в верху вывода, называя отложенное действие и команду для его запуска. Баннер исчезает, как только соответствующий потребитель успешно отработал. Справочник по схеме и lifecycle: [`config/state/schema.md`](../config/state/schema.md#состояние-pending).
