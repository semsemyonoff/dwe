...
- Изменение `when:` или `files_gate:` не отражается в `action_hash`. Они вычисляются на каждом запуске независимо от журнала. Журнал лишь укорачивает выполнение тела.
...
- Если блокировка удерживается живым процессом, вызов возвращает `ProjectLockHeldError` с PID держателя и exit-кодом 2. CLI показывает это как `deploy operation in progress: pid 12345`.
...
2. При успехе статус переключается на `ok`, с `finished_at`, `action_hash` и `duration_ms`, и пишется снова.
3. Если процесс убит (`Ctrl+C`, `SIGTERM`, `kill -9`, паника, OOM), файл на диске всё ещё отражает последнюю запись — шаг, который выполнялся, остаётся записан как `in_progress` или `failed`.
4. На следующем старте `RepairProjectStatus` проходит журнал: любые записи `in_progress` переводятся в `failed` (процесса, владевшего ими, уже нет), и агрегаты пересчитываются.
