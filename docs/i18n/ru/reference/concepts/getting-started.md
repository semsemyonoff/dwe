> Translated from: reference/concepts/getting-started.md @ 3e4a21eb6fcd

# Начало работы

Первое прохождение Devbox: собрать бинарник, войти в проект, запустить пайплайн деплоя, поднять стек и прочитать информационную панель.
...
Devbox поставляется как один Go-бинарник. Собирайте из этого репозитория:

```sh
make build
```

`make build` запускает `go mod tidy`, синхронизирует `docs/` во встроенное дерево, перегенерирует `internal/core/docs/content_hashes_gen.go`, собирает `./cmd/devbox` и пишет `bin/devbox`. Бинарник самодостаточен: документация, переводы и дефолтные пайплайны встроены внутрь.
...
Для использования из любой директории скопируйте бинарник в `$PATH`:

```sh
install -m 0755 bin/devbox /usr/local/bin/devbox
```

Devbox требует `docker` (и `docker compose`) на пути; `git` и `bash` используются для рендеринга хуков и shell-шагов. См. [`devbox.yml`](../config/devbox.md#binary-overrides) для переопределения путей к этим инструментам, если они лежат в нестандартных местах.
