...
Список compose-файлов завершается отслеживаемыми патчами `compose_after:` (по сервисам, эмитятся после всех групп сервисов — для оверлея, который должен побеждать собственный оверлей другого сервиса) и затем общепроектным `compose.extra` из `workspace/local.yml`, так что переопределения на разработчика (альтернативные образы, debug-порты, дополнительные тома) накладываются поверх отслеживаемых оверлеев без их редактирования. `workspace/docker.local.yml` — это **политика** compose (имя проекта, аргументы подкоманд, окружение процесса), а не файл в цепочке `-f`. Полная сборка — в [`docker.yml`](../config/docker.md) и [Интеграции с Docker](docker.md).
