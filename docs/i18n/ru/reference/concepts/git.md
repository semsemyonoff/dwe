...
Типичный `.gitignore` проекта исключает три дерева, управляемых DWE:
...
- **`volumes/`** содержит цели bind-mount, в которые пишут контейнеры (директории данных БД, кэши сборки, сгенерированные артефакты). Отслеживание этой папки связало бы репозиторий с выводом контейнеров.
