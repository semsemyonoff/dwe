> Translated from: reference/concepts/architecture.md @ eb472f58e59e
...
- **Детерминированный argv.** Список compose-файлов отсортирован (tools → infra → apps → патчи `compose_after`, по алфавиту внутри каждой группы). Имя проекта шаблонизируется один раз и переиспользуется. Два вызова `dwe run` на одинаковом конфиге дают побайтово идентичные команды `docker compose`.
