...
| `secrets` | ключевой материал и запись в отслеживаемые файлы слоёв — только на хосте. Контейнерное *чтение* уже расшифрованных значений остаётся открытым по решению: `dwe vars get` и `render config` возвращают открытый текст — ровно та же экспозиция, что у контейнера уже есть через отрендеренный `.env`; см. [секреты → Поведение в контейнере](../config/secrets.md#поведение-в-контейнере) |
