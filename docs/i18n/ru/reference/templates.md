> Translated from: reference/templates.md @ db899535a11f
...
- [Render-контекст по местам использования](#render-контекст-по-местам-использования)
  - [Паки ide / ai / git никогда не видят расшифрованный секрет](#паки-ide--ai--git-никогда-не-видят-расшифрованный-секрет)
...
### Паки ide / ai / git никогда не видят расшифрованный секрет

Поскольку их выводы обычно отслеживаются git, эти три рендерера грузят **санитизированный** конфиг: та же трёхслойная сборка, но **вообще без прохода расшифровки**. Каждое поле, доступное шаблону — `.Raw`, `.Cfg`, `.Project`, `.Runtime`, `.Services`, `.ServiceCfg`, — несёт закоммиченный маркер `ENC[age:…]` там, где настоящий рантайм-конфиг несёт открытый текст.

Поэтому шаблон, читающий [зашифрованное значение](config/secrets.md), выдаёт шифротекст (уже закоммиченный, безвредный), но не открытый текст — без учёта путей и без неоднозначности для последовательностей и точечных ключей. Это структурная гарантия, а не allowlist, — именно поэтому совет «избегайте секретных ключей» выше про *шум в диффах*, а не про утечку.

Конфиг-паки (`render config`) — противоположный случай: они рендерятся в hub-директорию сервиса из gitignore по **настоящему** конфигу и вдобавок поддерживают целые источники `.age` — см. [`render config`](render/config.md#зашифрованные-источники-age).
