> Translated from: reference/render/ide.md @ dd6ac0c25336
...
> **Manifest обязателен.** Каждый IDE-пакет обязан содержать `manifest.yml` в корне, перечисляющий каждый рендерящийся файл. Отсутствие manifest — жёсткая ошибка. Схема разделяется с `render ai` и `render git` — см. [Общая схема manifest](index.md#общая-схема-manifest). Пофайловые [локальные оверрайды](index.md#локальные-оверрайды) через соседний `<pack>.local/` shadow-tree применяются ко всем трём рендерерам одинаково.
...
Каждый IDE-пакет обязан содержать `manifest.yml` в корне по [общей схеме manifest](index.md#общая-схема-manifest):
...
| `.Commands` / `.CommandGroups` | **объявленные** команды проекта и авторские группы команд | А также `.ServiceCommands` / `.ServiceCommandGroups` для контейнера этого сервиса. Поля и правило «объявленное, а не живое»: [Индекс объявленных команд](ai.md#индекс-объявленных-команд) |
...
- [блок `services.<name>.render.ide`](../config/services/fields.md#блок-renderide) — `enabled`, `template`, наследование через `extends`
