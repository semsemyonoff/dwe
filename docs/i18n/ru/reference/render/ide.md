...
- **Implicit-цепочка** (когда `render.ide.template` не задан): `<service-name>` → `default`. Провал-дальше происходит только если кандидатный каталог отсутствует; любая другая ошибка файловой системы — жёсткая. Если implicit-цепочка исчерпана без пакета, рендер пропускается с предупреждением.
