...
- [Разрешение значения](#разрешение-значения)
...
| `default` | string | нет | fallback, когда `from` не разрешается или разрешается в ложную строку (см. [Разрешение значения](#разрешение-значения)) |
| `required` | bool | нет | если `true`, `from` отсутствует и `default` пустой — рендер падает с ошибкой |
...
3. **Выбрать значение** — см. [Разрешение значения](#разрешение-значения).
