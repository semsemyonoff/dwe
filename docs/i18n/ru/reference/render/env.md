...
- [Правила экспорта](#export-rules)
  ...
  - [Нерезолвящиеся пути](#нерезолвящиеся-пути)
...
Правило, чей `from:` не резолвится, без `default:` и без `required: true`, рендерится голым `NAME=` — пустым значением, которое доходит до каждого контейнера так, будто его так и объявили. `dwe render env` сообщает об этом — по одной строке на правило, в **stderr**, перед записью содержимого:

```text
warning: exports.env[DB_PASSWORD]: from "vars.db.passwrod" does not resolve — rendered empty
```

stderr оставляет `dwe render env > .env` побайтово прежним, а `--output json` подавляет предупреждение. Про две другие формы здесь не предупреждают: при `default:` отрендеренное значение — это собственный фолбэк автора, а `required: true` роняет рендер сразу. `dwe validate` репортит все три формы как предупреждения `config.exports` (см. [`validate.md`](../config/validate.md)), включая нерезолвящийся `when:`, который это предупреждение покрыть не может — такое правило пропускается и не рендерит строку вовсе.
