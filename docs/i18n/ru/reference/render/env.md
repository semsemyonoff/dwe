> Translated from: reference/render/env.md @ 656a4cf82c2b
...
Проверка покрывает каждое выдаваемое значение — три системные переменные
(`PROJECT` берётся из `project.name`) **и** каждое правило экспорта, включая
