> Translated from: reference/render/env.md @ e31d8a346016
...
Список правил экспорта живёт под `exports.env` в `devbox/defaults.yml` (см. [справочник по exports.env](../config/devbox.md)). Порядок правил в YAML задаёт порядок строк на выходе.
...
|------------|----------|-----------|
| `PROJECT` | `project.name` из `devbox.yml` | используется Docker-метками, именем проекта Compose и Make-целями |
...
Имена `PROJECT`, `UID` и `GID` **зарезервированы**: любое правило экспорта, пытающееся использовать одно из них как `name`, отвергается на этапе загрузки конфигурации с понятной ошибкой. Это касается любой команды, загружающей проектную конфигурацию, а не только `devbox render env` — поэтому опечатка ловится при первом же запуске после правки `defaults.yml`.
...
```text
# Generated by devbox — do not edit manually
...
- Путь трактуется относительно текущего рабочего каталога, не корня проекта. Указывайте абсолютный путь, если нужно детерминированное расположение независимо от того, откуда вызвана команда (например, при запуске с `-c /path/to/devbox.yml` из другого каталога).
...
`devbox render env` на macOS произведёт:
...
- **Правка `.env` вручную** — файл перегенерируется через `devbox render env --out .env` и lifecycle-хуки. Правьте `exports` в `devbox/defaults.yml` или оверрайды в `devbox/local.yml`.
- **У `--out` нет короткой формы.** Флаг называется `--out`; алиаса `-o` нет. Используйте `devbox render env --out .env`.
...
- [схема правил `exports.env`](../config/devbox.md) — полный справочник по полям и форматам
- [Разрешение dot-path](../config/devbox.md) — как пути `from` и `when` ходят по объединённой конфигурации
- CLI-справочник: [`devbox render env`](../cli/devbox_render_env.md)
