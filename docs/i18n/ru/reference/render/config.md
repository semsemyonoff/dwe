> Translated from: reference/render/config.md @ e800b0572942
...
- **Произвольные значения** живут под `vars:` — ссылайтесь на них как
  `${vars.<путь>}` (например, `${vars.databases.main}`). Голый dot-путь
  верхнего уровня без префикса `vars.` не резолвится: корень объединённой
  конфигурации — это строгий allowlist (`project`, `services`, `vars`, …),
  поэтому произвольный ключ вроде `databases` не может появиться там напрямую.
