> Translated from: reference/render/index.md @ 833bb97c47b9
...
2. Выбирает цели:
   ...
   - `ide` / `ai` / `git` / `config` — итерирует сервисы, применяет политику выборки, опционально сужает до одного сервиса через аргумент `[service]`. Без аргумента каждая из них обходит **все** сервисы и учитывает соответствующее поле opt-in каждого сервиса, поэтому в конвейере должен быть один шаг без аргумента (например, `cmd: "render ai"`), а не по шагу на сервис; аргумент `[service]` — для разовых запусков по одному hub'у.
