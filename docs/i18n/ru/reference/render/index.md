> Translated from: reference/render/index.md @ e2d0ffb704b8
...
- [Общий конвейер](#общий-конвейер)
- [Вход и выход одним взглядом](#вход-и-выход-одним-взглядом)
- [Страницы](#страницы)
...
2. Выбирает цели:
   ...
   - `ide` / `ai` / `git` — итерируют сервисы, применяют политику выборки, опционально сужают до одного сервиса через аргумент `[service]`. Без аргумента каждая из них обходит **все** сервисы и учитывает поле opt-in каждого сервиса (`render.<pack>.enabled`), поэтому в конвейере должен быть один шаг без аргумента (например, `cmd: "render ai"`), а не по шагу на сервис; аргумент `[service]` — для разовых запусков по одному hub'у.
   - `config` — тоже итерирует сервисы и принимает `[service]`, но выборка у неё другая: только app-сервисы, и разрешается она через config-пак, а не через поле opt-in у сервиса.
