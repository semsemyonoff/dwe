...
| `.Commands` | **объявленные** публичные команды всего проекта (команды с `private: true` не входят), отсортированные по id — см. [Индекс объявленных команд](#индекс-объявленных-команд). Пусто, если проект не объявляет команд или его файлы команд не загружаются. |
...
**Объявленные, а не текущие.** Индекс строится только по файлам команд; условия `hide:` **не** вычисляются. `hide:` может зависеть от запущенного стека, а отслеживаемый файл, меняющийся при каждом старте или остановке контейнера, давал бы бессмысленные диффы. Поэтому `.Count` — объявленное число: пока действует `hide:`, `dwe commands list <group>` печатает меньше команд, чем сказано в файле, или ни одной. Формулируйте сгенерированный текст соответственно — «N declared», а не обещание того, что вернёт листинг. Текущий аналог — `dwe docs llms-txt`: он вычисляет `hide:` при каждом вызове. Отрендеренный файл точно так же остаётся снимком файлов команд: после изменений в `workspace/commands/` перезапустите `dwe render ai` (и `ide` / `git`, если их пакеты читают индекс).

**Если файлы команд не загружаются**, `dwe render ai|ide|git` всё равно рендерит: печатает одно предупреждение с ошибкой загрузки, а `.Commands` / `.CommandGroups` пусты для всех сервисов в этом запуске. Саму ошибку сообщает `dwe validate commands`; `dwe validate templates` прогоняет пакеты вхолостую на том же, в этом случае пустом, индексе и повторно ошибку не сообщает.
