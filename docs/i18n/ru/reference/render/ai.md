...
Пофайловые [локальные оверрайды](index.md#local-overrides) через соседний `<pack>.local/` shadow-tree применяются к AI-пакетам так же, как к IDE и git — положите файл в `workspace/templates/ai/<pack>.local/<rel>`, чтобы подменить им канонический без правки отслеживаемого пакета.
...
Аргумент `[service]` необязателен. Без него `dwe render ai` обходит все сервисы объединённой конфигурации и рендерит каждый, прошедший [гейт активации](#activation-gate) ниже — при этом учитывается `render.ai.enabled` каждого сервиса, так что отказавшиеся пропускаются без предупреждения.
...
| `.Cfg` | объединённый `DweConfig` (продвинутое). `.Cfg.Raw` — мапа после слияния и нормализации DWE (`services.*` подставляется из per-service `service.yml`) — см. [Шаблоны](../templates.md#render-context-per-site). Для обычных случаев предпочитайте выделенные поля выше. |
...
- [блок `services.<name>.render.ai`](../config/services/fields.md#renderai-block) — `enabled`, `template`, наследование через `extends`
