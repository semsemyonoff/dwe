> Translated from: reference/render/ai.md @ 6b191bf82ec3
...
| `.Description` | авторское `description:`, **на английском** — пакеты никогда не локализуются, поэтому отрендеренный файл побайтово одинаков при любой активной локали |
...
| `.Title` | `title:` из заголовка группы, иначе последний сегмент идентификатора |
...
**Если файлы команд не загружаются**, `dwe render ai|ide|git` всё равно рендерит: печатает одно предупреждение с именем файла, а `.Commands` / `.CommandGroups` пусты для всех сервисов в этом запуске. Саму ошибку сообщает `dwe validate commands`.
