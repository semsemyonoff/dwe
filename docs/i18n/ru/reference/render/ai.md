> Translated from: reference/render/ai.md @ f373ae86052b
...
Пофайловые [локальные оверрайды](index.md#локальные-оверрайды) через соседний `<pack>.local/` shadow-tree применяются к AI-пакетам так же, как к IDE и git — положите файл в `workspace/templates/ai/<pack>.local/<rel>`, чтобы подменить им канонический без правки отслеживаемого пакета.

## Содержание

- [Конвейер](#конвейер)
- [Выборка сервисов](#выборка-сервисов)
  - [Гейт активации](#гейт-активации)
  - [Нормализация каталога](#нормализация-каталога)
  - [Разрешение коллизий: выигрывает поверхностнейший](#разрешение-коллизий-выигрывает-поверхностнейший)
  - [Явный аргумент `[service]`](#явный-аргумент-service)
- [Разрешение пакета шаблонов](#разрешение-пакета-шаблонов)
- [Схема manifest](#схема-manifest)
  - [Записи `render`](#записи-render)
  - [Записи `symlinks`](#записи-symlinks)
  - [Правила валидации manifest](#правила-валидации-manifest)
- [Пофайловый рендер](#пофайловый-рендер)
  - [Переменные шаблона](#переменные-шаблона)
  - [Индекс объявленных команд](#индекс-объявленных-команд)
  - [Гарды безопасности путей](#гарды-безопасности-путей)
- [Создание симлинков](#создание-симлинков)
- [Проработанный пример](#проработанный-пример)
- [Сообщения вывода](#сообщения-вывода)
...
Аргумент `[service]` необязателен. Без него `dwe render ai` обходит все сервисы объединённой конфигурации и рендерит каждый, прошедший [гейт активации](#гейт-активации) ниже — при этом учитывается `render.ai.enabled` каждого сервиса, так что отказавшиеся пропускаются без предупреждения.

Именно эта форма без аргумента предназначена для конвейера: один шаг на уровне проекта (`type: dwe`, `cmd: "render ai"` в `workspace/deploy.yml`) обновляет все hub'ы и не требует правок при добавлении, удалении или переключении сервисов. Отдельный шаг `render ai <name>` в каждом `workspace/services/<name>/deploy.yml` лишь повторяет ту же работу по разу на сервис. Автоматически ничего не рендерится — во встроенном deploy-конвейере по умолчанию нет фазы рендера, поэтому такой шаг всегда добавляется явно.

Явный аргумент `[service]` оставьте для разовых запусков по одному hub'у.
...
3. Выполняет шаблон по [переменным шаблона](#переменные-шаблона).
4. Разрешает назначение и прогоняет [гарды безопасности путей](#гарды-безопасности-путей).
...
| `.Commands` | **объявленные** команды всего проекта, отсортированные по id — см. [Индекс объявленных команд](#индекс-объявленных-команд). Пусто, если проект не объявляет команд или его файлы команд не загружаются. |
| `.CommandGroups` | авторские группы команд с объявленным числом команд, отсортированные по id — см. [Индекс объявленных команд](#индекс-объявленных-команд). |
| `.ServiceCommands` | метод: элементы `.Commands`, у которых `service:` равен `.ServiceCfg.Container`. |
...
### Индекс объявленных команд

Каждый AI-, IDE- и git-пакет получает реестр команд проекта как простые данные. Элементы `.Commands`:
...
| `.Service` | объявленный `service:` (имя compose-сервиса / контейнера). Может быть неотрендеренным выражением вроде `app-${param.service}` |

Элементы `.CommandGroups`:
...
Группы, которые никто не объявлял, не попадают в список: точечный идентификатор вроде `services.magento` неявно создаёт узел `services`, и без заголовка `group:` или собственных команд такому узлу нечего вывести, поэтому он не появляется.

**Привязка команд к hub.** `.ServiceCommands` сравнивает `service:` команды с `.ServiceCfg.Container` — не с `.Service` и не с `.Resolved`. Команда называет compose-сервис, в котором выполняется, а к сервису, у которого `service.yml` задаёт `container: app-magento` под ключом `magento`, обращаются как к `app-magento`. Команда без `service:` или с шаблонным значением не принадлежит ни одному hub. `.ServiceCommandGroups` оставляет группы, содержащие хотя бы одну команду этого hub, и отбрасывает такую группу, если она вложена в другую оставленную, — `dwe commands list <родитель>` уже перечисляет вложенную. Родитель поглощает вложенные группы, только если ни одна команда под ним не нацелена на контейнер сервиса с другим каталогом hub. Команды для контейнеров без собственного hub — вспомогательные и не учитываются: `services.magento.config.set` в magento нацелена на `db`, и группа всё равно сворачивается; не учитывается и `extends`-наследник, разделяющий hub (`magento-debug`). Авторская группа `services`, охватывающая `services.magento` и `services.node`, не заменяет `services.magento` в hub magento и сама попадает в список, только если напрямую содержит команду magento, не покрытую вложенной группой.

**Объявленные, а не текущие.** Индекс строится только по файлам команд; условия `hide:` **не** вычисляются. `hide:` может зависеть от запущенного стека, а отслеживаемый файл, меняющийся при каждом старте или остановке контейнера, давал бы бессмысленные диффы. Поэтому `.Count` — объявленное число: пока действует `hide:`, `dwe commands list <group>` печатает меньше команд, чем сказано в файле, или ни одной. Формулируйте сгенерированный текст соответственно — «N declared», а не обещание того, что вернёт листинг. Текущий аналог — `dwe docs llms-txt`: он вычисляет `hide:` при каждом вызове.

**Если файлы команд не загружаются**, `dwe render ai|ide|git` всё равно рендерит: печатает одно предупреждение с ошибкой загрузки, а `.Commands` / `.CommandGroups` пусты для всех сервисов в этом запуске. Саму ошибку сообщает `dwe validate commands`.

#### Блок `Declared commands` из поставки

Пакет `default`, который создаёт `dwe init`, рендерит в `AGENTS.md` каждого hub блок ниже. Hub, чьему сервису не принадлежит ни одна группа команд, блока не получает вовсе. Пакеты живут в проекте, поэтому проекты, созданные до появления блока, его не получают — чтобы его добавить, вставьте сниппет в `workspace/templates/ai/<pack>/AGENTS.md.tmpl`:
...
Строка группы печатает описание группы, а при его отсутствии — последний сегмент идентификатора. Блок намеренно перечисляет группы и вызов, а не таблицу команд: `dwe commands list <group> --output json` отдаёт те же данные по запросу, а таблица загружается в каждую сессию агента независимо от того, нужна ли она. Проекту, которому таблица нужна, `.ServiceCommands` позволяет построить её в собственном пакете.

Именно проза заставляет агентов пользоваться командами: она говорит, когда проверять (задача подпадает под описание группы), что приоритетнее (объявленная команда), чего не делать до проверки (запускать нижележащий инструмент напрямую) и во что проверка обходится (один раз за сессию). При правке сохраняйте все четыре части.
...
| warning | Файлы команд не загрузились — печатается один раз за запуск, а не на каждый сервис; `.Commands` и `.CommandGroups` рендерятся пустыми. |
...
- [блок `services.<name>.render.ai`](../config/services/fields.md#блок-renderai) — `enabled`, `template`, наследование через `extends`
