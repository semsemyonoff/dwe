...
Git-хуки рендерятся под `<svc.Dir>/src/.git/hooks/` (gitignored), поэтому значения из `local.yml` в `.Cfg.Raw` дают пер-разработческую вариацию хуков, которая не коммитится, — это и есть здесь целевой кейс, в отличие от `render ide` / `render ai`, которые пишут отслеживаемые файлы.
