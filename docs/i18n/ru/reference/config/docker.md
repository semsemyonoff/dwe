...
**Advisory-поведение, никогда не жёсткий сбой:** каждый шаг prepull (вычисление ссылок из `compose config` и Dockerfile'ов сервисов, проверка наличия базового образа локально, pull) выполняется по принципу best-effort. Любая внутренняя ошибка понижается до предупреждения в stderr, а обычные `compose build`/`compose up` всё равно выполняются — включение `prepull_bases` не может сделать сборку *хуже*, чем без него. Единственный «громкий» случай — подтверждённо отсутствующий базовый образ, который не удалось притянуть: предупреждение называет ссылку и сообщает, что сборка, скорее всего, провалится, поскольку этот сбой теперь предсказуем.
