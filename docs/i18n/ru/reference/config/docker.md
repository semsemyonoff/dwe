...
Имя Docker Compose-проекта, передаваемое как `-p <name>` в каждый compose-вызов. Поддерживает `${dot.path}` lookup'ы по смерженному DWE-конфигу (см. [Шаблоны](../templates.md) — пространства имён `${...}`), разрешаемый по отдельному, более строгому правилу: **любой** dot-path в `Raw` резолвится здесь (без whitelist'а по неймспейсам), но неразрешённый путь — это жёсткая **ошибка**, а не буквальный `${...}` — сломанное имя проекта должно упасть громко, а не молча дойти как `${...}` до `docker compose -p`. Имя, которое разрешается в нерасшифрованный [маркер секрета](secrets.md), отклоняется по той же причине (`resolves to an undecrypted secret — see 'dwe secrets status'`): область проекта, заданная шифротекстом, осиротила бы все созданные под ней контейнеры, сети и тома. По умолчанию разрешается в `dwe-laravel`.
...
**Итоговое имя также пишется в `.env`** как зарезервированная системная переменная [`COMPOSE_PROJECT_NAME`](../render/env.md#system-variables), перегенерируемая на каждом `dwe run` и `dwe render env`. Поскольку `.env` лежит в каталоге compose-проекта, Docker Compose подхватывает её сам: «сырой» `docker compose …` из корня проекта — и шаг пайплайна `type: shell`, который сам вызывает compose, — работает в том же проекте, что и `dwe`, **поверх** любого верхнего `name:` в compose-файле. Шаг пайплайна `type: shell` (и его shell-`check:`) вдобавок получает имя напрямую как `COMPOSE_PROJECT_NAME`, поверх значения из вашего shell, — см. [типы исполнения шагов](deploy/steps.md#type-shell). Скрипты, которые собирали имя из `PROJECT` вручную, могут читать переменную напрямую.

Если ваша compose-цепочка объявляет `name:`, расходящееся с этим резолвером (`dwe validate` сообщает об этом как `config.compose_project_name`), «сырой» compose перестаёт видеть контейнеры, сети и тома, созданные под старым именем. Либо задайте здесь `project_name` равным старому значению — это переключит и сам `dwe`, поэтому один раз передеплойте, — либо впишите в compose-файл `name: ${COMPOSE_PROJECT_NAME}`, которое теперь резолвится из `.env`.
