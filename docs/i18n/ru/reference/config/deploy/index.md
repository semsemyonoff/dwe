> Translated from: reference/config/deploy/index.md @ c53a748916e5
...
Единственное исключение — строковые листья `with:`: они рендерятся, если содержат `${...}` с известным head'ом **или** литеральный `{{`, потому что значение `with:` и раньше всегда проходило через шаблонный движок перед попаданием в команду. Поэтому `with: {host: '{{ resolve .Raw "vars.db.host" }}'}` по-прежнему резолвится, а ограничение на смешанную форму выше распространяется и на листья `with:`.
