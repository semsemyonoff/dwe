> Translated from: reference/config/deploy/index.md @ fb63af418d04
...
**Дефолтный пайплайн деплоя** (срабатывает, когда `workspace/deploy.yml` отсутствует или есть, но не объявляет ни одной фазы — пустой, из одних комментариев, `phases: []`):

Фазы: `services` (запускает `deploy_services: true` для встраивания пайплайнов включённых сервисов) → `start` (`type: dwe`, `cmd: "docker up --wait"`, с `check: {type: builtin, cmd: containers_running}`) → `post-deploy` (отображение info + сообщение об успехе). Благодаря `check:` шаг `up` выполняется на каждом деплое, поэтому стек, остановленный после прошлого деплоя, снова поднимается, а встроенный пайплайн никогда не завершается с `already up-to-date` — см. [Идемпотентный деплой и состояние](#идемпотентный-деплой-и-состояние).
...
|------|-----|--------------|----------|
| `log` | bool | `deploy.yml`: `true`; `reset.yml`: `false` | Дублировать статусные сообщения DWE и stdout/stderr дочерних процессов в `.dwe/logs/<pipeline>.log`. В файл попадает по одной строке на каждую **закоммиченную** строку, ANSI-коды вырезаются: серия перерисовок на месте через одиночный `\r` (прогресс git clone, `curl`) сворачивается до последнего кадра вместо строки на кадр, а серия, закончившаяся голым `\r` без завершающего перевода строки, всё равно отдаёт свой последний кадр в конце шага. Сворачивание — не эмуляция терминала: `abc\rX\n` на настоящем терминале выглядит как `Xbc`, а в лог пишется `X`. Плата за это: кадры перерисовки больше не доходят до файла по мере появления, поэтому `tail -f .dwe/logs/deploy.log` больше не показывает живой прогресс клонирования — только закоммиченные строки. |
...
| `check` | typed action, либо скаляр `auto` | Постусловие, вычисляемое после успеха шага; пайплайн прерывается, если действие провалилось. Пропускается, когда `continue_on_error: true` и шаг провалился. Шаг с `check:` никогда не пропускается журналом, а план, в котором такой шаг есть, никогда не завершается с `already up-to-date` (см. [Идемпотентный деплой и состояние](#идемпотентный-деплой-и-состояние)). См. [Условия](conditions.md). |
...
Шаг, который выполняется всегда, заодно не даёт всему деплою досрочно завершиться с `already up-to-date`. На этом держится встроенный пайплайн: его шаг `up` несёт `check: {type: builtin, cmd: containers_running}`, поэтому каждый деплой выполняет `docker up --wait` (на запущенном стеке это no-op), а затем проверяет, что каждый не-oneoff контейнер активного compose-сервиса запущен или завершился с кодом 0 ([режим всего проекта](builtins.md#режим-всего-проекта)). Шаг `docker up` без `check:` в извлечённом через eject или написанном вручную `deploy.yml` записывается в журнал после первого успеха и с тех пор пропускается, так что стек, остановленный после прошлого деплоя, остаётся лежать — добавьте ему такой же `check:`.
...
- `dwe deploy eject [--out PATH] [--force]` — выдать **встроенный дефолтный** пайплайн деплоя в виде закомментированного, готового к правке `deploy.yml`. Это константа, а не эффективный план проекта: ничего не рендерится, пайплайны сервисов не встраиваются, фильтра `--service` нет (для разрешённого экземпляра используйте `dwe deploy plan`). Без `--out` (или с `--out -`) документ уходит в stdout и ничего не записывается; с `--out PATH` он пишется в этот файл и **отказывается перезаписывать существующий без `--force`** — в отказе называется файл, и отдельно говорится, что файл инертен (только комментарии / пустой либо без фаз), то есть ровно тот файл, про который `dwe validate` пишет `has no active content …`. Неявного пути по умолчанию нет: канонический адресат `workspace/deploy.yml` передаётся явно. Под `--output json` путь через stdout выдаёт сырой документ без конверта, а путь через `--out` — `{"path": "…", "pipeline": "deploy"}` вместо строки подтверждения. Помните, что активный `workspace/deploy.yml` **заменяет** встроенный пайплайн целиком.
- `dwe reset eject [--out PATH] [--force]` — то же самое для встроенного дефолтного пайплайна сброса и `workspace/reset.yml`; см. [reset.md](../reset.md).
- `dwe reset plan` — показать пайплайн сброса
- `dwe reset run [--yes]` — выполнить пайплайн сброса
- См. также [lifecycle.yml](../lifecycle.md) — пайплайны `run` / `stop` переиспользуют ту же грамматику фаз/шагов с опциональной пробой обновлений и хук-фазами. Команды `lifecycle eject` намеренно **нет**: эффективный пайплайн `stop` всегда несёт синтетическую фазу движка `_auto_reap_daemons`, а пользовательская фаза с именем на `_` отвергается при загрузке — выданный `lifecycle.yml` был бы файлом, который сам dwe отказывается загружать.
