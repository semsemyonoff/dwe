...
Git запускается с отключёнными промптами — `GIT_TERMINAL_PROMPT=0`, обнулённые `GIT_ASKPASS`/`SSH_ASKPASS` и `GIT_SSH_COMMAND=ssh -o BatchMode=yes`, когда окружение его ещё не задало — поэтому отсутствующие учётные данные проваливают шаг вместо того, чтобы подвесить деплой на неотвечаемом промпте. Явно заданный `GIT_SSH_COMMAND` (кастомный файл ключа, порт) уважается как есть. Команда не читает stdin.
