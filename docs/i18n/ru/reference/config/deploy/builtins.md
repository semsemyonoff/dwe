> Translated from: reference/config/deploy/builtins.md @ 3af29352bb50
...
Гейт `when:` по-прежнему применяется, как обычно — шаг-предикат с `when:`, вычисляющимся в false, пропускается без утверждения (условное утверждение остаётся условным). Однако сам гейт не принимает эти билтины: `when: {type: builtin}` разрешается по отдельному реестру предикатов (`dir-exists`, `file-missing`, …), а не по билтинам с этой страницы — это [непересекающиеся реестры](../conditions.md#два-регистра-type-builtin).
...
Эта возможность чисто разрешающая: предикаты, ранее легальные только внутри `check:`, теперь легальны и как тела шагов, в любом пайплайне (`deploy.yml`, `reset.yml`, `lifecycle.yml` и тест-сценарии) и как пользовательские команды `type: builtin`. Существующие конфиги не затрагиваются.
...
| `containers_running` | Быстрая проверка «запущен» для названных сервисов или всего проекта (без поллинга, таймаута и обязательного healthcheck) |
...
«Собрать, а не сгенерировать»: собственный генератор сервиса (например, `php artisan key:generate`) пишет секрет; DWE только считывает его обратно и воспроизводит при последующих рендерах. Запись-если-отсутствует означает, что значение, уже находящееся в хранилище, сохраняется, поэтому повторный деплой — это no-op. Сервис без полей `generated:` — это no-op. Отсутствующий файл, паттерн, не совпавший ни с одной строкой, паттерн без группы захвата или захваченное пустое значение выдаются как ошибки — никогда не пропускаются молча. См. [блок `generated`](../services/fields.md#блок-generated) и [render config](../../render/config.md).
...
Без `services` (поле отсутствует или `[]`) билтин проверяет compose-проект целиком. Именно этот `check:` несёт шаг `up` [встроенного пайплайна деплоя](index.md#назначение):
...
Проверка проходит, когда каждый не-one-off контейнер проекта запущен или завершился с кодом 0 (отработавший one-shot для init или миграций). Эта терпимость рассчитана на one-shot, к которому другие сервисы обращаются через `depends_on: {condition: service_completed_successfully}` или который отмасштабирован в 0, — one-shot, от которого никто не зависит, валит сам `docker compose up --wait` ещё до этой проверки. Контейнеры `compose run` — в том числе контейнеры команд [`type: daemon`](../commands/types.md#тип-daemon) — не оцениваются.

- **Ожидаемый набор — это существующие контейнеры**, а не список сервисов compose-конфига. У сервиса со `scale: 0` / `deploy.replicas: 0` контейнера нет, и он никогда не проваливает проверку; проект без единого контейнера проходит — ничего запущено быть не должно.
- **Учитываются только сервисы, активные в текущем compose-конфиге.** Контейнер, который не запущен и не завершился с кодом 0 (вышел с ненулевым кодом, застрял в `created` или `restarting`), проваливает шаг с `containers not running: <comma-separated container names>`, если его сервис есть в `docker compose config --services`. Остановленный контейнер сервиса неактивного профиля, который `up --remove-orphans` не удаляет, и контейнер-сирота игнорируются.
- **Имя проекта.** Фильтр по лейблу использует имя, которое dwe передаёт в `-p` (`project.name` или `project_name` в `docker.yml`). Если ни одно не задано, имя, которое compose выбирает сам (`COMPOSE_PROJECT_NAME`, top-level `name:`, имя директории), читается из `docker compose config --format json`; если и оно пустое, шаг падает с `containers_running: cannot determine the compose project name — set project.name or docker.yml project_name`.

Известные граничные случаи — задокументированы, а не обработаны:

- **podman-compose без лейбла `com.docker.compose.oneoff`.** Билтин распознаёт бэкенд, который не ставит этот лейбл, и оценивает все контейнеры проекта без фильтра, поэтому остановленный контейнер `compose run` (остановленный демон) активного сервиса может быть назван незапущенным.
- **Бэкенд без `docker compose config --format json` или `config --services`.** Шаг падает с ошибкой бэкенда, когда имя проекта не задано или когда найден контейнер, который не запущен и не завершился с кодом 0.
- **`args.global` в эти вызовы compose не передаётся.** С `--project-directory` или `--env-file` в `args.global` запасное имя может отличаться от того, с которым работал `up`, — задайте `project.name`. С `--profile` там сервисы этого профиля выглядят неактивными, поэтому упавший контейнер такого сервиса проходит проверку вместо провала (но никогда наоборот).
