...
Проверка проходит, когда каждый не-one-off контейнер проекта запущен или завершился с кодом 0 (отработавший one-shot для init или миграций). Контейнеры `compose run` — в том числе контейнеры команд [`type: daemon`](../commands/types.md#type-daemon) — не оцениваются.
