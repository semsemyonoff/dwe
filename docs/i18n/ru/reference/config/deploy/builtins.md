> Translated from: reference/config/deploy/builtins.md @ 6f436ad2f32d
...
**Предикат** — билтин, отвечающий на вопрос «да/нет» о состоянии мира (`file_exists`, `tcp_reachable`, `http_check`, `containers_running`, `env_keys_present`, `config_keys_present` и билтин `shell`) — может использоваться прямо как тело шага, а не только внутри блока `check:`/`when:`. Как тело шага предикат становится **утверждением**:
