> Translated from: reference/config/deploy/examples.md @ 52c97866f03f
...
- **Маршрутизация sequential vs parallel**:
  - Последовательные тела шагов приостанавливают LiveLine через `Reporter.SuspendForExec`, и дочерний процесс пишет в host-терминал напрямую (с PTY, когда stdout — TTY). Цвета, позиционирование курсора и интерактивный UX работают как в обычном shell; tee, обёрнутый `frameLogWriter`, фиксирует ANSI-очищенную копию со свёрнутыми кадрами в on-disk лог и сбрасывается перед финишной строкой самого шага, поэтому оба писателя в этот файл остаются в порядке. `ResumeAfterExec` перерисовывает футер после выхода ребёнка.
...
- **Frame-aware парсер**: `\r`-aware `lineTee` парсит поток ребёнка в коллбеки `(frame, final)` — напрямую для параллельных под-шагов и через `frameLogWriter` для последовательных тел шагов. На живой строке показывается только последний фрейм. В лог-файлах не-финальный (завершённый `\r`) фрейм становится *pending*: следующий фрейм вытесняет его, поэтому пишется только последний кадр серии перерисовок, и `50%\r100%\n` даёт одну строку `100%`, а не две. Серию, закончившуюся голым `\r`, отдаёт `Flush()` писателя в конце шага. `\r\n`, попавший в два разных чтения, не сворачивается внутри одного разбора: сам `lineTee` придерживает кадр, закрытый `\r`, и отдаёт его вместо пустого кадра, который иначе породил бы `\n`, — поэтому строка переживает границу чтения на обоих маршрутах. Это сворачивание кадров, а не эмуляция терминала: `abc\rX\n` пишется как `X`, тогда как терминал показывает `Xbc`; `foo\r\x1b[K\n` пишется как `foo`, тогда как терминал показывает пустую строку; а полнокадровые перерисовки через cursor-up-последовательности (блок `[+] up 2/3` у compose) вне области и по-прежнему попадают в лог на каждую перерисовку.
...
- **Лог-файлы под-шагов**: `.dwe/logs/parallel/<pipeline>/<group>/<sub>.log` фиксирует закоммиченные строки под-шага (ANSI вырезается, не-финальные `\r`-фреймы отбрасываются — кроме случая, когда закрывающий строку кадр не несёт текста, как у `\r\n`, разорванного между двумя чтениями: тогда коммитится придержанный содержательный кадр, а не пустая строка). Хвост, который ребёнок оставил без закрывающего перевода строки — завершённый `\r` или нет, — в этот файл не пишется; он всё равно доходит до глобального пайплайнового лога, потому что тот же фрейм уходит и в `Reporter.StepOutput`, который коммитит хвост при завершении под-шага. Глобальный пайплайновый лог (`.dwe/logs/<pipeline>.log`) получает каждую статусную строку и каждую закоммиченную дочернюю строку ровно один раз.
