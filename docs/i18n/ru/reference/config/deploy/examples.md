> Translated from: reference/config/deploy/examples.md @ 071a1c9eed08
...
- **Лог-файлы под-шагов**: `.dwe/logs/parallel/<pipeline>/<group>/<sub>.log` фиксирует закоммиченные строки под-шага (ANSI вырезается, не-финальные `\r`-фреймы отбрасываются). Хвост, завершённый `\r`, в этот файл не пишется — он всё равно доходит до глобального пайплайнового лога, потому что тот же фрейм уходит и в `Reporter.StepOutput`, который коммитит хвост при завершении под-шага. Глобальный пайплайновый лог (`.dwe/logs/<pipeline>.log`) получает каждую статусную строку и каждую закоммиченную дочернюю строку ровно один раз.
