> Translated from: reference/config/deploy/steps.md @ 7861cd89508b
...
**Таймаут.** Билтин `cmd: shell` принимает опциональный `timeout:` (по умолчанию `10s`). `timeout: "0"` означает **без ограничения**, как и в соглашении о собственном `timeout:` шага — именно это позволяет выведенному [`check: auto`](conditions.md#check-auto-инверсия-when) сохранить неограниченную позицию `when:`, который он инвертирует. У тел шагов `type: shell` собственного встроенного таймаута нет; ограничивайте их полем `timeout:` уровня шага.
