> Translated from: reference/config/deploy/steps.md @ e26ccf4dc66e
...
Большинство билтинов — действия (они что-то делают). Некоторые — **предикаты**, отвечающие на вопрос «да/нет» о состоянии мира (`file_exists`, `tcp_reachable`, `http_check`, `containers_running`, `env_keys_present`, `config_keys_present` и билтин `shell`). Предикат может использоваться как тело шага, где ведёт себя как **утверждение**:
