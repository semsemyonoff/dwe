> Translated from: reference/config/services/index.md @ 79c4c6f5dfe1
...
Загружается через `LoadServices(baseDir)`, который перечисляет подпапки `workspace/services/*/`, для каждой папки вызывает `LoadServiceFolder(baseDir, name)` (обязанности: предварительная валидация сырой формы, обязательное поле `type`, разрешённые поля для конкретного типа, защита от `extends:` вне app, валидация формы `ports`/`hosts`, строгий декод с `KnownFields`), затем применяет топологическую сортировку межсервисных `extends:` и слияние наследования. Отсутствующий каталог `workspace/services/` возвращает пустую карту (не ошибка). Переключатели уровня разработчика (`enabled:`, `ports:`, `hosts:`) живут в трёхслойном оверлее; структурные поля принадлежат исключительно `workspace/services/<name>/service.yml`.
...
- Каждый `workspace/services/<name>/service.yml` декодируется строго — неизвестные и не разрешённые для данного типа поля являются фатальными ошибками. `LoadServices` собирает ошибки со всех папок через `errors.Join`, поэтому сразу выявляются все сломанные папки, а не только первая.
...
Каждый сервис живёт в собственной папке под `workspace/services/`. Имя папки становится ключом сервиса.
