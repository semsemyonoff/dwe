> Translated from: reference/config/services/fields.md @ 9bc627ddfedb

# Справочник полей сервиса

Каждое поле, допустимое в `workspace/services/<name>/service.yml`, плюс вложенные блоки (`ports`, `hosts`, `icon`, `info`, `configs`, `dirs`, `cli`, `status`, `render`).
...
**URL'ы через reverse-proxy (`port_via`)** намеренно НЕ читают `info.scheme` сервиса-прокси. Используется только per-port переопределение на выбранном порту прокси (обычно `http` / `https`) плюс глобальный фолбэк `runtime.use_https`. `info.scheme` у самого прокси влияет только на его собственную строку в `dwe info`, но не на приложения, маршрутизируемые через него.
