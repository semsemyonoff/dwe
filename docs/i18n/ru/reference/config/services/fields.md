...
| `compose` | list | нет | все | Дополнительные файлы оверлея compose, активируемые при включении сервиса. Относительные пути внутри оверлея (bind-маунты, `build.context`, `env_file`) разрешаются от каталога первого файла в цепочке `-f` — [`compose.base`](../workspace.md#compose), то есть от корня проекта при обычной раскладке, — а не от каталога самого оверлея: `dwe` не передаёт `--project-directory`, поэтому действует правило Compose для нескольких файлов. Сами пути в списке задаются относительно корня проекта; `dwe validate` предупреждает ([`config.compose_files`](../validate.md#домены-валидации)), если такого файла нет, независимо от того, включён ли сервис. |
| `compose_after` | list | нет | все | Файлы оверлея compose, добавляемые после всех групп сервисов (tool → infra → app) и перед сгенерированным bridge-оверлеем и общепроектным `compose.extra` из `local.yml` — тот же гейт `enabled`, что у `compose`, один проход по именам сервисов среди всех типов, порядок из списка внутри сервиса сохраняется. Используйте это для патча, который должен побеждать собственный оверлей приложения (полные значения ключей вроде `command:`, `healthcheck:` или записи `environment:`, которые задают оба файла), а не проигрывать ему только из-за того, что группа app эмитится позже. Если два файла `compose_after` трогают один и тот же ключ, побеждает более поздний по алфавиту сервис — не полагайтесь на это, патчите непересекающиеся ключи. Это фиксирует **порядок** эмиссии, а не **наличие** патчимого сервиса: блок патча для приложения, которое сейчас отключено, всё равно заставит Compose отклонить весь проект целиком. Недоступно из слоёв-оверлеев (`workspace.yml`, `defaults.yml`, `local.yml`) — место этого поля в `service.yml`, как и у `compose`. Пути разрешаются так же, как у `compose`. В отличие от `compose`, это поле намеренно не отражается в смерженном конфиге — нет ни `${services.<name>.compose_after}`, ни `from:` в `exports.env`, ни `default_from:` команды, которые могли бы его прочитать. |
...
| `depends_on` | list | нет | app / infra | Упорядоченная зависимость от других сервисов (влияет на порядок deploy, а также расширяет область [`env.ports_free`](../validate.md#домены-валидации) при по-сервисном deploy до сервисов, которые compose поднимает вместе с запрошенным). Цель `type: tool` отклоняется при загрузке. |
| `status` | list | нет | все | Пользовательские колонки для таблицы `dwe status apps` / `tools` / `infra` по типу — см. [Блок `status`](#блок-status). |
| `on_enable` | block | нет | app / tool / infra | Хуки жизненного цикла при включении сервиса. См. [Примеры — жизненный цикл переключения](examples.md#жизненный-цикл-переключения). |
...
В отличие от `render.ide` / `ai` / `git`, у `render.config` **нет флага `enabled`** — рендер конфигов гейтится исключительно тем, разрешается ли пак (opt-in: нет пака → нет рендера). Шаблоны конфигов используют сокращение `${...}` (например, `${services.main.ports.http}`, `${vars.databases.main}`, `${generated.app_key}`) — намеренное расхождение с сырым субстратом `{{ }}`, используемым другими видами рендера. См. [render config](../../render/config.md) для полного справочника, субстрата, схемы манифеста и потока harvest/replay.
