> Translated from: reference/config/services/fields.md @ 844ef624c342
...
| `compose` | list | нет | все | Дополнительные файлы оверлея compose, активируемые при включении сервиса. Относительные пути внутри оверлея (bind-маунты, `build.context`, `env_file`) разрешаются от каталога первого файла в цепочке `-f` — [`compose.base`](../workspace.md#compose), то есть от корня проекта при обычной раскладке, — а не от каталога самого оверлея: `dwe` не передаёт `--project-directory`, поэтому действует правило Compose для нескольких файлов. |
