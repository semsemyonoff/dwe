> Translated from: reference/config/services/extends.md @ 086cf31da014
...
- `render.config` — наследуется целиком, когда потомок не объявляет собственного блока `render.config:` (блок родителя клонируется); потомок с собственным `render.config:` сохраняет его и не сливается с родительским.
- `generated` — наследуется, когда потомок не объявляет собственной карты `generated:` (карта родителя клонируется); потомок с собственной картой `generated:` полностью заменяет родительскую, не сливается.
...
- `compose_after` — наследуется независимо от `compose`, по тому же правилу: клонируется от родителя, когда потомок не объявляет собственного списка `compose_after:`; собственный список потомка полностью заменяет родительский, не сливается. Если и родитель, и потомок включены, унаследованный файл эмитится дважды — без дедупликации, точно так же, как унаследованный список `compose:`.
