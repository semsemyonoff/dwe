> Translated from: reference/config/services/examples.md @ b84d6e3b1375
...
```yaml
on_enable:
  requires: none | restart | deploy | deploy-or-restart  # что запускать после записи local.yml
...
|-------|---------|-------------|
| `requires` | `restart` | Что должно произойти, чтобы изменение вступило в силу. `none` → только запись local.yml; `restart` → запустить `dwe restart`; `deploy` → запустить `dwe deploy run --service <name>`; `deploy-or-restart` → deploy, когда сервис ещё ни разу не разворачивался, иначе restart (только для `on_enable`). `deploy` и `deploy-or-restart` запрещены в `on_disable`. |
...
- **Забытый `depends_on:` у потомка** — не наследуется. Потомок, нуждающийся в зависимости, должен объявить её явно. (`compose:` наследуется от родителя, когда потомок его опускает — см. [Правила разрешения наследования](extends.md#правила-разрешения).)
