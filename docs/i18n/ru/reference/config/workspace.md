...
Этот блок управляет только тем, *что контейнеру можно писать*; он отличается от пер-сервисного блока [`services.<name>.bridge`](services/fields.md#блок-bridge), который управляет тем, *подключён ли сервис к бриджу вообще*. Полная поверхность контейнера — в разделе [Хост-бридж → политика команд](../concepts/bridge.md#политика-команд-внутри-контейнеров).
...
#### Второй потребитель: изоляция портов в `dwe test`

Эти же правила позволяют `dwe test` увести копию сценария с хост-портов рабочего окружения. Хост-порт compose, записанный одной переменной (`"${VALKEY_PORT:-6379}:6379"`), переназначается в каждой копии сценария, когда стоящее за этой переменной правило читает `from: vars.<path>` (переназначается всегда, если сценарий не зафиксировал путь) или `from: services.<name>.ports.<port>` (переназначается, пока переназначается сам сервис). Любой другой `from:` — или `when:`, ложный в этом сценарии, — оставляет копию на порту вашего собственного окружения, о чём сообщается предупреждением об изоляции. Поэтому смена `from:` у правила или добавление `when:` меняет не только `.env`, но и изоляцию тестовой копии; см. [Интерполированные host-порты](tests.md#интерполированные-host-порты).
