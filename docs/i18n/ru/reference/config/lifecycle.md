> Translated from: reference/config/lifecycle.md @ f22496d48e77
...
В файл попадает по одной строке на каждую **закоммиченную** строку — ровно как для `deploy.yml`: серия перерисовок на месте через одиночный `\r` сворачивается до последнего кадра вместо строки на кадр, а серия, закончившаяся голым `\r`, всё равно отдаёт свой последний кадр в конце шага. Плата та же (живого прогресса перерисовки под `tail -f` больше нет, и сворачивание — не эмуляция терминала) — см. [`deploy.yml` → `log`](deploy/index.md#поля-верхнего-уровня).
