...
Сопоставление идёт по **эффективному имени**: явное `name:` volume'а после схлопывания цепочки `-f` (побеждает `name:` из более позднего overlay) либо ключ в compose-карте, если ни один файл его не задал, — оно сравнивается с именем общего volume'а из `docker.yml` (volume с `shared: true` резолвится в своё `name:` дословно, без префикса проекта). Несовпадение не подтверждается, как и `shared: false`. Находки `external_volume` и `named_volume` по этому volume'у помечаются вместе.
