...
Путь отчёта отображается в текстовом выводе `dwe test run` (строка `report: <dir>` под непройденным сценарием) и в JSON-выводе как `report_dir` (пусто для успешного сценария или запуска с `--keep`).
