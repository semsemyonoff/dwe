> Translated from: reference/config/tests.md @ f50ce17761d7
...
| `isolation_findings` | **Неблокирующие** находки [сканера изоляции compose](#сканер-изоляции-compose) — `named_volume` / `external_volume` / `named_network` / `external_network`, то есть ресурсы, разделяемые с рабочим окружением, плюс `interpolated_host_port` — host-порт, подставленный из переменной, которую копия этого сценария не переназначает. Блокирующие виды (`container_name`, `raw_host_port`) не включаются: они и так прерывают сценарий до деплоя. Volume, [подтверждённый](#volumeы-подтверждённые-через-shared-true) объявлением `shared: true` в `docker.yml`, остаётся в списке и несёт `"shared": true` (иначе ключ опускается) — профиль сообщает факты, а не вердикты, так что вызывающий, которому нужны только неподтверждённые риски, фильтрует по нему. Полные сообщения даёт `dwe validate tests`. |
