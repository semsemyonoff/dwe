> Translated from: reference/config/tests.md @ 5534e0278f61
...
- **загрузка** — `LoadScenario` (строгий `KnownFields(true)`, пустой файл отвергается); это также покрывает **нормализацию имени** (`ValidateScenarioName` по базовому имени файла), так что некорректное имя файла всплывает здесь.
