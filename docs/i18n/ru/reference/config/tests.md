> Translated from: reference/config/tests.md @ fcb10077fc94
...
| `shell_steps` | Шаги `type: shell`, которые запустит сценарий: его собственные плюс деплой-пайплайн, который он вызывает (проектный и включённых сервисов), с заходом внутрь групп `parallel:`. Побочные эффекты shell-шага на хосте [не изолируются](#документированные-ограничения). |
