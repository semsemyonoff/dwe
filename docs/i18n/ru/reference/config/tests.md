> Translated from: reference/config/tests.md @ 20149146bc7b
...
| `from: services.<name>.ports.<x>`, где `<x>` объявлен под `services.<name>.ports` | раннер переназначает порты `<name>` — сервис включён в сценарии и не указан в `env.services.disable` | говорит, что порты `<name>` переназначаются, только пока он включён |
