...
- `argv`, отличный от `[<shell>, -c, …]`, например `[bash, -lc, …]`, — любой другой `argv` запускается без шелла;
...
Значение имеет членство в переназначении, а не то, включён ли сервис в итоге: сервис с `required: true`, указанный в `env.services.disable`, остаётся включённым в копии, но его порты не переназначаются, поэтому находка остаётся.
