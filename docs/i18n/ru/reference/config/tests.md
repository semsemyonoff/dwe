> Translated from: reference/config/tests.md @ 23553963e4b8
...
| что угодно другое — другой путь, подпуть `.port`, необъявленный порт, ложный `when:`, отсутствие правила (окружение хоста, написанный вручную `.env`, значение по умолчанию после `:-`) | никогда | называет оба способа исправления |
...
- У `dwe validate tests` нет своего сценария, поэтому он отбрасывает находку, только когда её покрывает каждый сценарий, а иначе дописывает `(not covered in scenarios: a, b)`. Файл сценария, который не загрузился, ничего не покрывает; находка из последней строки таблицы предупреждает всегда. Здесь `when:` вычисляется на собственной конфигурации проекта, поэтому правило, привязанное к сервису, выключенному по умолчанию, но включённому каждым сценарием, всё равно предупреждает.
