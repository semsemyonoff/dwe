> Translated from: reference/config/tests.md @ efac1ec9a0c3
...
**Предпосылка — порты должны быть проведены через vars.** `auto` может переписать только те порты, которые проект уже проводит через vars (`${...}` в compose-файлах или в `ports:` dwe, подставляемых из vars). Проект с захардкоженным буквальным host-портом не может быть протестирован параллельно с работающим рабочим окружением: preflight-проверка `ports_free` в копии сразу падает с подсказкой перенести порт на var. Это стоит подчеркнуть особо, потому что это единственная предпосылка, которая нужна каждому проекту, прежде чем его деплой вообще можно протестировать сценарием.
...
**Порты.** `auto`-vars выделяются пакетом (все listener'ы открываются до закрытия любого из них, гарантируя уникальность внутри пакета) и записываются как конкретные числа. Собственная preflight-проверка `ports_free` копии всё равно выполняется и ловит гонки выделения; при провале деплоя из-за конфликта портов при наличии `auto`-vars runner переаллоцирует и повторяет деплой **ровно один раз**, прежде чем провалить сценарий.
...
| `.dwe/tests/reports/<scenario>/` | Зарезервировано для артефактов сбоя stage 2 (пока не заполняется) |
...
`--keep` пропускает все шаги выше, оставляет манифест и копию на месте и печатает имя compose-проекта, путь копии и подсказку по очистке. Последующий `dwe test run` **того же** имени сценария сразу проваливается (манифест сохранённого запуска ещё существует), а не тихо удаляет сохранённое окружение из-под вас — очистите вручную, либо дождитесь `dwe test clean` из stage 2.
...
| `1` | Хотя бы один сценарий провалился (сбой деплоя, сбой шага, таймаут) |
| `2` | Сценарий — или сам запуск — не удалось даже подготовить (неизвестное имя сценария, ошибка загрузки/парсинга сценария, flock захвачен параллельным запуском, манифест сохранённого предыдущего запуска всё ещё существует) |
...
`status` — одно из `passed`, `failed`, `error` (`error` = сценарий не удалось подготовить — сбой копии/конфига/манифеста/validate; отличается от сбоя деплоя или шага, который отражается как `failed`). `report_dir` зарезервировано для артефактов сбоя stage 2 и в stage 1 остаётся пустым. Как и в любой другой read-only/отчётной поверхности, живой вывод пайплайна и строка сводки в JSON-режиме молчат — файловый лог под `.dwe/logs/` всё равно фиксирует всё.
...
- **Именованные compose-ресурсы обходят изоляцию.** `container_name:`, явно именованные сети/volume'ы и `external: true` в сырых compose-файлах игнорируют разграничение по имени compose-проекта и могут столкнуться — или прикрепиться — к рабочему окружению. Именно поэтому teardown никогда не использует `compose down -v`.
- **Host-порты вне системы vars не покрываются.** `ports_free` видит только порты, объявленные через `services.<name>.ports`; буквальный host-порт в сыром compose-файле обходит и `auto`-выделение, и preflight-проверку.
