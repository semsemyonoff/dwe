...
| `from: services.<name>.ports.<x>`, где `<x>` объявлен под `services.<name>.ports` | раннер переназначает порты `<name>` — сервис включён в сценарии и не указан в `env.services.disable` | говорит, что порты `<name>` переназначаются только в сценариях, где он включён и не указан в `env.services.disable` |
...
Проект, исправивший каждый сценарий, остаётся зелёным под `dwe validate --strict`.
