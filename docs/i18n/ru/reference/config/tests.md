> Translated from: reference/config/tests.md @ ec3721f51bd7
...
Конкретное число вместо `auto`, наоборот, **фиксирует** путь: копия биндит именно этот порт, а автоматическое переназначение отступает. `null`, `""` и отсутствующий ключ не фиксируют — экспорт откатился бы к собственному дефолту и забиндил порт живого стека, поэтому такие пути всё равно получают выделенный порт. Не фиксируют и map или список по этому пути: это структура, а не порт, и выделенный порт заменяет её.
...
- всё остальное — другой путь, под-путь `.port`, необъявленный порт, ложный `when:`, переменная из окружения хоста или написанного вручную `.env` — **не** переназначается. Переменная сохраняет то значение, которое имела бы и вне тестового запуска: выражение с дефолтом откатывается к нему (`"${VALKEY_PORT:-6379}:6379"` → `6379`), а значение из окружения хоста или написанного вручную `.env` остаётся как есть — в обоих случаях копия биндит порт рабочего окружения; переменная без того и другого разворачивается в пустую строку, и compose тогда вовсе не публикует host-порт для этой записи. [Сканер](#интерполированные-host-порты) предупреждает в каждом из этих случаев.
...
`status` — одно из `passed`, `failed`, `error` (`error` = сценарий не удалось подготовить — сбой копии/конфига/манифеста/validate; отличается от сбоя деплоя или шага, который отражается как `failed`). `failed_step` и `report_dir` опускаются, когда пусты (у успешного сценария нет ни того, ни другого). `report_dir` — директория [отчёта об ошибке](#отчёты-об-ошибках) для непройденного сценария; опускается для успешного сценария, запуска с `--keep`, сценария, остановленного [гейтом изоляции](#сканер-изоляции-compose) до создания копии (сбор не запускается), или когда сбор не смог создать директорию отчёта. Как и в любой другой read-only/отчётной поверхности, живой вывод пайплайна и строка сводки в JSON-режиме молчат — файловый лог под `.dwe/logs/` всё равно фиксирует всё. Исключение — гейт изоляции: он срабатывает раньше, чем появляется лог запуска, поэтому под `--output json` остановленный им сценарий виден только как `failed` без `failed_step` и `report_dir`, а сообщения о находках нигде не сохраняются — чтобы их прочитать, перезапустите без `--output json` или выполните `dwe validate tests`.
