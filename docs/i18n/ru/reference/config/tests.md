...
Сканирование, стоящее за этим, выполняется по **представлению проекта глазами сценария**: рабочее дерево с применённым `env:` сценария и вырезанными персональными overlay'ями `compose.extra` — так же, как их вырезает seed-`local.yml` копии — и вычисляется до того, как копия создана. `dwe validate tests` и `dwe test list --output json` строят то же представление, поэтому compose-файл, доступный только через overlay `compose.extra`, невидим для всех трёх: копия его никогда не выполняет.
