> Translated from: reference/config/tests.md @ 3e1de61e4555
...
| `isolation_findings` | **Неблокирующие** находки [сканера изоляции compose](#сканер-изоляции-compose) — `named_volume` / `external_volume` / `named_network` / `external_network`, то есть ресурсы, разделяемые с рабочим окружением. Блокирующие виды (`container_name`, `raw_host_port`) не включаются: они и так прерывают сценарий до деплоя. Volume, [подтверждённый](#volumeы-подтверждённые-через-shared-true) объявлением `shared: true` в `docker.yml`, остаётся в списке и несёт `"shared": true` (иначе ключ опускается) — профиль сообщает факты, а не вердикты, так что вызывающий, которому нужны только неподтверждённые риски, фильтрует по нему. Полные сообщения даёт `dwe validate tests`. |
...
- **изоляция compose** — см. [Сканер изоляции compose](#сканер-изоляции-compose) ниже; находки, не подтверждённые как общие, выводятся один раз на проект как предупреждения (никогда как ошибки — многоуровневая политика fail/warn применяется только к `dwe test run`, а не к статической валидации).
...
- **`dwe test run`** — запускает скан на копии прямо перед подпроцессом `dwe validate`. Каждая находка, не [подтверждённая как общая](#volumeы-подтверждённые-через-shared-true), печатается как предупреждение. **Блокирующая** находка немедленно проваливает сценарий (teardown всё равно выполняется; подпроцесс деплоя не запускается), если не передан `--skip-isolation-check` — в этом случае каждая находка, блокирующая или нет, становится только предупреждением, и запуск продолжается. См. [предпосылку по портам](../../guides/integration-tests.md#порты-изолируются-автоматически), как избежать находки `raw_host_port` в принципе: смоделируйте порт через `services.<name>.ports`.
- **`dwe validate tests`** — выводит каждую находку, не подтверждённую как общая, как предупреждение, независимо от `Blocking`; статическая валидация никогда не проваливает сборку из-за риска изоляции, а только заранее его показывает.
- **`dwe test list --output json`** — перечисляет неблокирующие находки в `cost_profile.isolation_findings`, включая подтверждённые, с пометкой `"shared": true`.
