> Translated from: reference/config/tests.md @ 675a1001e1af
...
**Интерполированные host-порты переназначаются только через то, что они читают.** Host-порт compose, записанный переменной (`"${VALKEY_PORT:-6379}:6379"`), сдвигается, только когда эта переменная несёт переназначенное значение: правило `exports.env` `from: services.<name>.ports.<x>` для сервиса, который раннер переназначает в этом сценарии, или `from: vars.<path>` с `env.vars: { <path>: auto }` в сценарии. Всё остальное — путь `vars:` без `auto`, правило, чей `when:` ложен, переменная из окружения хоста или написанного вручную `.env` — биндит в копии исходный порт и конфликтует с рабочим окружением. Раннер такой порт не переназначает; [сканер](#интерполированные-host-порты) предупреждает о нём.
...
| Host-порт, который целиком одна переменная (`"${VALKEY_PORT:-6379}:6379"`, `published: "${PORT}"`) | `interpolated_host_port` | Предупреждение — сталкивается только в сценарии, который не переназначает переменную; см. [Интерполированные host-порты](#интерполированные-host-порты) |
...
Раннер переназначает host-порт только через значение, которое он записывает в копию, поэтому порт, записанный переменной (`"${VALKEY_PORT:-6379}:6379"`, `$PORT`, `${PORT?}` …), сдвигается, только если эта переменная экспортируется из переназначенного значения. Сканер прослеживает переменную до её правила `exports.env` — последнего с этим `name:`, у которого `when:` пуст или истинен, ровно как рендерится `.env`, — и находка зависит от того, что читает правило:

| Правило переменной | Покрыто в сценарии, когда | Сообщение |
|---|---|---|
| `from: vars.<path>` | сценарий задаёт `env.vars: { <path>: auto }` | называет compose-файл, сервис и переменную и даёт строку исправления `env.vars: { <path>: auto }` |
...
| что угодно другое — другой путь, подпуть `.port`, необъявленный порт, ложный `when:`, отсутствие правила (окружение хоста, написанный вручную `.env`, значение по умолчанию после `:-`) | только когда её compose-файла нет в стеке сценария (см. ниже) | называет оба способа исправления |

Сервис с `required: true`, указанный в `env.services.disable`, остаётся включённым в копии — ровно так загрузчик конфигурации разрешает сгенерированный `local.yml`, — поэтому его порты по-прежнему переназначаются и находка покрыта.

При любом правиле сценарий покрывает находку и тогда, когда compose-файла, публикующего порт, нет в его стеке: собственные compose-файлы сервиса (`compose:` в его `service.yml` и его оверлеи из `local.yml`) выпадают из цепочки, когда сценарий выключает сервис, поэтому в этой копии ничто из них порт не занимает. Сервис с `required: true` остаётся включённым, и его файлы остаются в цепочке; сервис, объявленный в корневом compose-файле, из неё не выпадает никогда.

Находка никогда не блокирующая. Каждый вызывающий фильтрует её по-своему:

- `dwe test run` отбрасывает её, когда запускаемый сценарий её покрывает, а иначе печатает как предупреждение.
- `dwe test list --output json` делает то же для каждого сценария; условия `when:` вычисляются с применёнными переключателями сервисов сценария.
- У `dwe validate tests` нет своего сценария, поэтому он отбрасывает находку, только когда её покрывает каждый сценарий, а иначе дописывает `(not covered in scenarios: a, b)`. Файл сценария, который не загрузился, ничего не покрывает; находка из последней строки таблицы предупреждает без списка сценариев, если только её compose-файла нет в стеке ни одного сценария. Здесь `when:` вычисляется на собственной конфигурации проекта, поэтому правило, привязанное к сервису, выключенному по умолчанию, но включённому каждым сценарием, всё равно предупреждает.

Поскольку `dwe test run` запускает `dwe validate` внутри копии, а копия несёт все файлы сценариев, `dwe test run a` всё равно выводит это общепроектное предупреждение, пока сценарию `b` не хватает исправления. Запуск оно не блокирует.
