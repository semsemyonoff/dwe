...
`workspace/snapshot.yml` в корне проекта. Файл опционален — подкоманды только для чтения (`list`, `current`, `inspect`) работают без него; `unpack` (как и `pack`) тоже работает без файла, но захватывает блокировки проекта и пишет на диск. `pack` тоже работает без него (он архивирует существующую на диске директорию снапшота, откатываясь к умолчанию `./snapshots/<name>`). Изменяющие подкоманды (`create`, `restore`, `rollback`, `remove`) падают с ошибкой, если он отсутствует или нужный workflow-блок не задан.
...
- `./snapshots/` ПО умолчанию в gitignore (сгенерированный `dwe init` `.gitignore` включает `snapshots/`); проект, который хочет коммитить небольшие dev-фикстуры, должен переопределить это правило игнорирования (например, `!snapshots/fixtures/`).
...
Когда любая из блокировок уже удерживается другим живым процессом, операция выходит с кодом 2 с понятным сообщением `"<operation> in progress: pid N"`.
...
| 2 | Блокировка удерживается другим живым процессом |
