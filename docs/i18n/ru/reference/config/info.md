> Translated from: reference/config/info.md @ 5d8346773a0a
...
| `icon` | Опциональная эмодзи или символ, подставляемый перед значением. Предпочитайте кодпойнты с `Emoji_Presentation=Yes` (например, `📦`, `🐳`, `💾`); text-default кодпойнты вроде `🛢`, `🗄`, `⚙` помечаются `dwe validate` и отбрасываются во время рендера, чтобы сохранить выравнивание колонок — полный комментарий см. в [поле `icon`](services/fields.md#поле-icon) в справочнике сервисов. |
