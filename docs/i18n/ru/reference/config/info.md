...
Загружается отдельно. Не сливается с 3-слойным конфигом.
...
> **Замечание о рендеринге `auto-urls` и `auto-hosts`:** элементы `auto-urls` и `auto-hosts` разворачиваются во время рендера (когда выполняется `dwe info`), а не во время загрузки YAML. Разворот консультируется с текущим конфигом и итерирует включённые сервисы в deploy-порядке, так что дашборд всегда отражает свежие определения и состояние сервисов.
...
Все поля `text`, `value` и `when` поддерживают синтаксис Go template, вычисляемый против разрешённой конфигурации проекта.
...
- **Использование конфигурационных ключей, не выставленных на верхнем уровне** — как прямые шаблонные пути вроде `.Project.Name` доступны только поля, поверхнутые разрешённой конфигурацией проекта (см. таблицу выше). Кастомные ключи, добавленные в `defaults.yml`, лежат под `.Cfg.Raw` и достаются через `index` или точечные пути к `Raw`.
