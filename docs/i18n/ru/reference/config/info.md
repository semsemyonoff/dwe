> Translated from: reference/config/info.md @ ba433a5e285d
...
`devbox/info.yml` объявляет содержимое дашборда `devbox info`: секции, элементы, условную видимость и шаблонные выражения. Рендерится через `render.Info()` (в `internal/core/ui/render/`) с использованием Lipgloss.
...
> **Замечание о рендеринге `auto-urls` и `auto-hosts`:** элементы `auto-urls` и `auto-hosts` разворачиваются во время рендера (когда выполняется `devbox info`), а не во время загрузки YAML. Разворот происходит внутри info-рендерера через консультацию с текущим конфигом и итерацию сервисов в deploy-порядке. Этот дизайн гарантирует, что дашборд всегда отражает свежие определения и состояние сервисов. Детали архитектуры о render-time развороте через паттерн `Source*Spec` см. в [`docs/internals/packages.md`](../../internals/packages.md).
...
| `icon` | Опциональная эмодзи или символ, подставляемый перед значением. Предпочитайте кодпойнты с `Emoji_Presentation=Yes` (например, `📦`, `🐳`, `💾`); text-default кодпойнты вроде `🛢`, `🗄`, `⚙` помечаются `devbox validate` и выбрасываются во время рендера, чтобы сохранить выравнивание колонок — полный комментарий см. в [поле `icon`](services/fields.md#icon-field) в справочнике сервисов. |
...
Динамически генерирует список сервисных URL из настроенных сервисов проекта. Сервисы объявляют свои хосты и порты в `devbox/services/<name>/service.yml`; `auto-urls` рендерит их с опциональной фильтрацией и кастомизацией.
...
| `ip` | string | `127.0.0.1` | IP-адрес, ассоциируемый со всеми именами хостов. Значения здесь не валидируются на формат IP; `devbox validate` эмитит предупреждение, если парсинг падает. |
...
Все поля `text`, `value` и `when` поддерживают синтаксис Go template, вычисляемый против `DevboxConfig`.
...
Info-шаблонам доступна стандартная поверхность шаблонов devbox: доменный хелпер `appURL` плюс реестры sprout (`std`, `strings`, `numeric`, `slices`, `maps`, `regexp`, `conversion`, `time`, `filesystem`, `semver`). Полный справочник хелперов — в [Шаблонах](../templates.md).
...
Когда `devbox/info.yml` не существует, используется встроенная дефолтная конфигурация. Она рендерит две секции:
...
Это позволяет проектам без `info.yml` сразу увидеть осмысленный дашборд, показывающий связность всех сервисов, целиком построенный из определений сервисов в `devbox/services/*/service.yml`. Сервисы вносят детали через свои блоки `info:` (title, paths, ключи host/port). Никакого редактирования `info.yml` для старта не требуется.

Чтобы кастомизировать дашборд, создайте `devbox/info.yml` со своими `sections` и `items`. Встроенный дефолт не используется, если файл существует, даже если в нём нет элементов `auto-urls` или `auto-hosts`.
...
- **Использование конфигурационных ключей, отсутствующих в структуре DevboxConfig** — в шаблонах доступны только поля, экспонированные на типизированной структуре `DevboxConfig`. Кастомные ключи, добавленные в `defaults.yml`, лежат в `Raw`, но не доступны в данных шаблона, если не экспонированы явно.
...
- `devbox info` — рендер полного дашборда
- `devbox` (без аргументов) — показывает компактную сводку (не из `info.yml`, использует `render.Summary`)
