> Translated from: reference/config/commands/validation.md @ 8908ac774e75
...
- **Не шеллите в `./bin/devbox`** — используйте `type: devbox` или `$DWE_BIN` (в скриптах). Любая из этих форм подхватывает запущенный бинарник, даже когда путь сборки меняется.
