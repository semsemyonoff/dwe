> Translated from: reference/config/commands/directives.md @ e4273b23670a
...
это по-прежнему ничего не может, но аргументы разобьются по пробелам.

Два места молча теряют аргументы, потому что у `"$@"` есть область видимости:

- **внутри тела shell-функции** — там `$@` это аргументы *функции*, поэтому слот
  развернётся в пустоту. Держите слот на верхнем уровне скрипта.
