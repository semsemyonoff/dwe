...
**Выражение обязано завершаться с кодом 0, когда список законно пуст.**
Ненулевой код — это сломанное выражение, и оно проваливает команду: намеренно не
трактуется как «нечего делать», потому что опечатка в команде не должна выглядеть
как чистый пропуск. Это важно при фильтрации через `grep`, который возвращает 1,
когда совпадений нет: используйте pathspec (`git diff … -- '*.py'`, код 0 при
пустом результате) или добавьте `|| true`.
