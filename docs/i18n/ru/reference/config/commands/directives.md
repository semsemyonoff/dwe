...
В строке `cmd:` аргументы **экранируются для shell и склеиваются**, потому что
строка исполняется через `sh -c`. Имя файла с пробелом остаётся одним
аргументом, а `;` или `$(…)` внутри аргумента остаются буквальным текстом:
...
Элемент, который лишь *содержит* `${args}` (`--filter=${args}`), рендерится как
обычное шаблонное выражение — там действует склейка с экранированием.
