> Translated from: reference/config/commands/directives.md @ 4d44dccd71fa
...
Пишите слот **без кавычек** — `${args}`, а не `"${args}"`. Он уже разворачивается
в корректно закавыченное `"$@"`; собственные кавычки дадут `""$@""` — исполнить
это по-прежнему ничего не может, но защиту, которую даёт квотирование, снимет:
аргументы разобьются по пробелам, а содержащий `*` или `?` попадёт под развёртку
имён файлов. `-- '*.txt'` дойдёт до команды подходящими именами файлов, а не
буквальным шаблоном.
