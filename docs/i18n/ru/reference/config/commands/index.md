...
2. **Разрешение контекста** — каждый точечный путь `context.<key>.from` читается из объединённого конфига.
   ...
   `dwe validate` предупреждает (домен `commands`), когда один из этих dot-путей не резолвится в собранном конфиге. Последствия у полей разные: неразрешимый `params.<name>.default_from` уходит в `default:` либо валит параметр, если тот `required:` и другого значения нет; неразрешимый `params.<name>.options.from` даёт **пустой список опций**, и форма выбора не предлагает ничего; неразрешимый `context.<name>.from` рендерится пустой строкой. `dwe validate --strict` превращает предупреждение в ошибку.
...
Когда `dwe commands` вызывается без точного идентификатора команды на интерактивном терминале — и без `--inspect`, которому идентификатор обязателен, — открывается интерактивный двухпанельный браузер команд; см. [Интерактивный браузер](#интерактивный-браузер).
...
| `service` | опускается, если команда его не объявляет. Это **объявленное** значение, а не отрендеренное — шаблонный `service: app-${param.service}` печатается как есть, так что это не всегда буквальное имя контейнера |
| `private` | только с `--all` |
| `params` | опускается, если параметров нет; полное описание параметров — в `dwe commands -i <id> --output json` |

`description` и `service` позволяют агенту выбрать команду по одному листингу: чтобы найти то, что выполняется в контейнере, фильтруйте по `service`; чтобы понять назначение команды, читайте `description`. Команды, скрытые активным `hide:`, отсутствуют — как и в текстовом листинге.
...
Он не конфигурируется: группы верхнего уровня раскрыты на входе, пустые поддеревья сворачиваются при fuzzy-фильтрации, бэйджи типов включены. Это описание полного двухпанельного фрейма; на терминале уже 100 колонок бэйджи и счётчики групп отключаются, а ниже 80 колонок или 15 строк браузер деградирует до плоского списка — см. [Fallback-лестница](#fallback-лестница).
