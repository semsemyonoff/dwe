> Translated from: reference/config/commands/index.md @ cc3ff5d88802
...
2. **Разрешение контекста** — каждый точечный путь `context.<key>.from` читается из объединённого конфига.
   ...
   `dwe validate` предупреждает (домен `commands`), когда `params.<name>.default_from`, `params.<name>.options.from` или `context.<name>.from` не резолвится в собранном конфиге: неразрешимый путь рендерится пустым и молча уходит в `default:` либо валит параметр с `required:`. `dwe validate --strict` превращает это предупреждение в ошибку.
