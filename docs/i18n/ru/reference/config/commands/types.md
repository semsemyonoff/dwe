...
`COMPOSE_PROJECT_NAME` берётся из того же резолвера, что `dwe render env` пишет в `.env` как [зарезервированную системную переменную](../../render/env.md#системные-переменные).
...
| `mode` | опционально | `exec-or-run` (по умолчанию), `exec-or-fail`, `exec` или `run` — см. [разрешение mode](#разрешение-mode) |
...
|------|---------------------------|-------------------------------|
| `exec-or-run` (по умолчанию) | запускается через `docker compose exec` | переключается на `docker compose run --rm`; выводит жёлтое предупреждение, чтобы поведение с эфемерным контейнером было видно |
| `exec-or-fail` | запускается через `docker compose exec` | отказывает с ясной ошибкой DWE, предлагающей `dwe docker up <svc>` |
...
Если `mode:` опущен, вы получаете `exec-or-run`: команда работает с живым контейнером, когда стек поднят, и всё равно выполняется на остановленном.

Объявляйте `mode: exec-or-fail` для инструментов, которые **зависят от персистентного состояния контейнера** и не должны создавать контейнер, — клиент базы данных, консоль сервера приложений, всё, чья ценность берётся из памяти, сокетов и накопленных файлов запущенного экземпляра. Для них остановленный контейнер должен проявиться как actionable ошибка, а не как свежий эфемерный контейнер, который ничего этого состояния не видит. `exec` и `run` остаются двумя безусловными выходами: `exec` — когда нужна собственная ошибка docker, `run` — когда свежий контейнер и есть цель.

Два режима `exec-or-*` различаются ровно **одним** наблюдаемым состоянием: проба running-контейнера отработала и сообщила, что контейнер остановлен. Когда сама проба падает — Docker недоступен, демон не поднят — оба режима заканчиваются обычным `compose exec`, и вы получаете один и тот же сырой сбой compose, что до смены значения по умолчанию, что после.

`runner.mode` подчиняется тому же enum и тем же правилам приоритета, что и `runner.user`.

> **Если вы пишете `check:` типа `type: command`.** `check:` шага — это полноценный action, диспетчеризуемый тем же исполнителем, так что он может указывать на пользовательскую команду любого типа, включая `service_exec`. Такой check наследует это значение по умолчанию, а значит при остановленном сервисе он *создаёт* контейнер вместо того, чтобы упасть, — при том что check должен быть постусловием без побочных эффектов. Объявляйте `mode: exec-or-fail` на любой команде, на которую ссылаетесь из `check:`.
...
`compose_args` — это список дополнительных флагов, вставляемых **перед** генерируемыми раннером флагами `--user` / `--workdir` / `-e`. Используйте его для флагов, которыми раннер не управляет сам: `-d`, `--name`, `--rm`, `--no-deps` и любых других флагов `docker compose exec` / `run`.

`-T` больше не входит в их число. Раннер сам решает вопрос о TTY внутри контейнера (см. [TTY контейнера](#tty-контейнера) ниже) и добавляет `-T`, когда терминал не нужен, так что написанный вручную `-T` «чтобы можно было пайпить» теперь избыточен.
...
Раннер выделяет TTY внутри контейнера только для запуска, который пользователь инициировал сам, и только когда терминал есть с обеих сторон:

- **Получает TTY контейнера** — верхнеуровневый `dwe commands <id>` (алиас `dwe cmd <id>`, а также запуск той же команды из браузера команд), у которого собственные stdin *и* stdout являются терминалом; плюс та же команда, запущенная через host-мост изнутри контейнера, где мост сам создаёт терминал.
- **Получает `-T`** — всё остальное. Шаг `type: command` внутри `dwe deploy run` или любого другого пайплайна; под-шаг блока `parallel:` в workflow; проба `check:`; запуск, вывод которого уходит в пайп или редирект (`dwe cmd foo | grep …`); и вложенный повторный вход в dwe — шаг `type: dwe` либо шаг `type: shell` / `type: script`, вызывающий dwe обратно через `$DWE_BIN`. Единственная команда, запускающая шаг пайплайна *как* пользовательский вызов, — `dwe reset step`, который вы набрали сами: за телом шага терминал сохраняется, за его `check:` — нет.

Подавление TTY не лишает вывод цвета: когда раннер забирает терминал у процесса, вывод которого всё равно доходит до вашего, он пробрасывает в контейнер `CLICOLOR_FORCE=1` / `FORCE_COLOR=1` / `COLORTERM=truecolor`, чтобы инструменты, ориентирующиеся на isatty, продолжали выдавать ANSI. Отсоединённый процесс (`-d`) намеренно исключён — его вывод идёт в логи Docker, где принудительный цвет навсегда впечатал бы туда escape-последовательности.

#### Как переопределить решение

Явный флаг TTY в любом месте **эффективного** вектора флагов побеждает, и автоопределение полностью отключается. «Эффективный» означает значения по умолчанию из `args.exec` / `args.run` в `docker.yml` **плюс** отрендеренный `compose_args` самой команды — флаг, объявленный в любом из этих мест, засчитывается. Распознаются формы `-T`, `-T=<значение>`, `--no-tty` со значением или без, а также `T` внутри связки коротких флагов вида `-dT`. Длинную форму compose пишет строчными буквами, и сравнение для неё регистронезависимое — чтобы автор, написавший `--no-TTY`, был услышан; короткий `-T` сравнивается с учётом регистра, потому что флага `-t` у compose нет.

Управление передаётся автору по самому *наличию* такого флага, независимо от его значения. Именно это и позволяет принудительно включить TTY: отдельного поля схемы для этого нет, поэтому

```yaml
compose_args:
  - "--no-tty=false"        # запросить TTY контейнера там, где раннер его не дал бы
```

— это явный и намеренно неуклюжий способ попросить терминал внутри пайплайна.

Посторонние флаги автоопределение **не** подавляют. `-d`, `--name`, `--rm` и им подобные ничего не говорят о терминале, поэтому команда с ними всё равно получит `-T`, если запуск не пользовательский.
...
Объединённые stdout/stderr каждого подшага захватываются в `.dwe/logs/parallel/workflow/<workflow-id>/<sub-command>.log`. В файл лога пишется только кадр, закрывающий строку: промежуточные прогресс-кадры с carriage-return остаются в live-строке, поэтому `50%\r100%\n` даёт одну строку `100%`, и файл остаётся читаемым без спама `\r`. Кадр, закрытый `\r`, *попадает* в лог, когда закрывающий строку кадр не несёт текста: и `\r\n`, разорванный между двумя чтениями, и идиома «стереть строку и перевести» `\r\x1b[K\n` записывают содержательную строку. Второй случай — осознанное приближение: это сворачивание кадров, а не эмуляция терминала, поэтому там, где настоящий терминал показывает пустую строку, лог сохраняет содержимое (см. [deploy → Репортер и логирование](../deploy/examples.md#репортер-и-логирование)).

Единственное исключение — последняя строка вывода: если дочерний процесс завершился без финального перевода строки (`printf 'error: x'; exit 1`), эта строка всё равно пишется в файл лога и воспроизводится в дампе при сбое. Финальный прогресс-кадр с `\r` по-прежнему отбрасывается, как и хвост, состоящий только из ANSI escape-последовательностей (сброс цвета, показ курсора).
