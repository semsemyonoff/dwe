> Translated from: reference/config/commands/types.md @ bed7d209fbf6
...
| `argv_append_from` | опционально | Shell-выражение на хосте, строки stdout которого дописываются в `argv` отдельными элементами — см. [Вычисляемые аргументы](directives.md#вычисляемые-аргументы-argv_append_from). Требует `argv`; пустой вывод пропускает команду |
...
| `argv_append_from` | опционально | Shell-выражение, строки stdout которого дописываются в `argv`. Выполняется на **хосте**, а не в контейнере — см. [Вычисляемые аргументы](directives.md#вычисляемые-аргументы-argv_append_from) |
...
| `workdir` | опционально | Контейнерный workdir; рендерится через шаблоны. Если опущен — откатывается к workdir целевого сервиса; `internal` отключает fallback целиком. См. [Разрешение workdir](#разрешение-workdir) |
...
Для `workdir` и `user` это теперь верно буквально — daemon выполняет те же резолверы, что и раннеры сервисов:

- **`workdir`** проходит полную [цепочку из семи ступеней](#цепочка-раннеров-сервисов), включая сентинел `internal` и сервисный fallback `cli.workdir` → `work_dir_internal` → `dir_internal`. Daemon, не объявляющий `workdir`, стартует в той же директории, в которую попадает `dwe shell <svc>`, а не в `WORKDIR` образа.
- **`user`** откатывается к `services.<svc>.cli.user` целевого сервиса, если команда не задала `user:`, а `user: internal` отключает этот fallback — см. [Разрешение user](#разрешение-user).

Вместе с этим выравниванием изменились два специфичных для daemon поведения, и оба видны снаружи:

- **`workdir_from` теперь побеждает литеральный `workdir`.** Раньше daemon делал обратное (побеждал литерал, а `workdir_from` читался только при пустом `workdir`), что противоречило приоритету, задокументированному для раннеров сервисов. Daemon, задающий **оба** поля, теперь стартует в директории из `workdir_from`.
- **Точечный путь `workdir_from`, не разрешающийся ни во что, больше не роняет daemon.** Раньше `.start` прерывался с `workdir_from "…": path not found in config`; теперь разрешение проваливается на следующие ступени. Жёсткая ошибка превращается в *успех в другой директории* — если ваш daemon полагался на эту ошибку, чтобы поймать опечатку или переименованный точечный путь, теперь об этом никто не сообщит. Нестроковое значение по точечному пути по-прежнему остаётся жёсткой ошибкой.
...
`argv_append_from` тоже отвергается, хотя `daemon` и строит `argv`: разворачивание упаковывает этот `argv` в синтетическую команду `.start`, где задокументированное «пустой вывод → пропуск» читалось бы как молчаливый отказ запустить демона. См. [Вычисляемые аргументы](directives.md#вычисляемые-аргументы-argv_append_from).
...
Когда заданы и `workdir`, и `workdir_from`, побеждает `workdir_from` — тот же паттерн «конфиг побеждает, литерал — страховочная сетка», что и у `params.*.default_from`. Внутри блока `runner:` действует то же правило между `runner.workdir_from` и `runner.workdir`. Единственное исключение — сентинел `internal`, описанный ниже.

### Цепочка раннеров сервисов

`service_exec`, `service_run` и `daemon` разрешают контейнерный workdir по одной цепочке из семи ступеней. Побеждает первая ступень, давшая непустое значение:

| № | Ступень | Результат |
|---|---------|-----------|
| 1 | `workdir: internal` (или `runner.workdir: internal`) | Флаг `--workdir` не передаётся, **и** весь fallback пропускается. Стоп. |
| 2 | `runner.workdir_from` → `workdir_from` | Строка, в которую разрешается точечный путь |
| 3 | `runner.workdir` → `workdir` | Литеральный путь |
| 4 | `services.<svc>.cli.workdir` | CLI-workdir, объявленный на сервисе |
| 5 | `services.<svc>.work_dir_internal` | Внутренняя рабочая директория сервиса |
...
`workdir_from` по-прежнему жёстко падает, если точечный путь разрешается в нестроковое значение (баг конфигурации). Точечный путь, которого в объединённом конфиге просто **нет**, ошибкой не является: он не даёт значения, и разрешение проваливается на следующую ступень.

### `workdir: internal` перекрывает `workdir_from`

Для этого одного значения опубликованное правило «побеждает `workdir_from`» **инвертируется**. `internal` — это не путь, а явное «не передавай этой команде `--workdir` и не подбирай его сам», и выразить это иначе нечем, поэтому резолвер останавливается на ступени 1 даже тогда, когда `workdir_from` задан и разрешается во вполне корректную строку. Для любого другого литерального значения `workdir` приоритет остаётся обычным: `workdir_from` побеждает `workdir`.

Это намеренная симметрия с [`user: internal`](#разрешение-user), который точно так же не передаёт флаг `--user` и пропускает fallback к `services.<svc>.cli.user`. Одно слово-сентинел, один смысл — на обоих полях.

Порядок разрешения:

```mermaid
flowchart TD
    S{workdir равен internal} -- да --> N[без флага --workdir]
    S -- нет --> F[workdir_from]
    F -- нестрока --> X[ошибка]
    F -- строковое значение --> U[использовать]
    F -- пусто/отсутствует --> W[workdir]
    W -- непусто --> U
    W -- пусто --> A[cli.workdir]
    A -- непусто --> U
    A -- пусто --> B[work_dir_internal]
    B -- непусто --> U
    B -- пусто --> C[dir_internal]
    C -- непусто --> U
    C -- пусто --> N
```

У хостовых раннеров (`type: shell`, `type: script`) нет целевого сервиса, к которому можно откатиться, и `workdir_from` они отвергают на этапе загрузки: применяется только литеральный `workdir`, а незаданный `workdir` означает корень проекта.
