...
Объединённые stdout/stderr каждого подшага захватываются в `.dwe/logs/parallel/workflow/<workflow-id>/<sub-command>.log`. Только кадры, завершённые переводом строки, пишутся в файл лога (прогресс-кадры с carriage-return остаются в live-строке и отбрасываются из логов), так что файл остаётся читаемым без спама `\r`. Единственное исключение — последняя строка вывода: если дочерний процесс завершился без финального перевода строки (`printf 'error: x'; exit 1`), эта строка всё равно пишется в файл лога и воспроизводится в дампе при сбое. Финальный прогресс-кадр с `\r` по-прежнему отбрасывается, как и хвост, состоящий только из ANSI escape-последовательностей (сброс цвета, показ курсора).
