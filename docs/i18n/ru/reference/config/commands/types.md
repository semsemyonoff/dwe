...
Если `mode:` опущен, вы получаете `exec-or-run`: команда работает с живым контейнером, когда стек поднят, и делает что-то осмысленное на остановленном стеке вместо того, чтобы заставлять автора выбирать одно из четырёх написаний. Именно это почти каждый проект и объявлял вручную.
