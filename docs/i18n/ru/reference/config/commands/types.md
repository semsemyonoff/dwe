> Translated from: reference/config/commands/types.md @ 878118712c34
...
- **Получает `-T`** — всё остальное. Шаг `type: command` внутри `dwe deploy run` или любого другого пайплайна; под-шаг блока `parallel:` в workflow; проба `check:`; запуск, вывод которого уходит в пайп или редирект (`dwe cmd foo | grep …`); и любой вложенный повторный вход в dwe — шаг `type: dwe` либо шаг `type: shell` / `type: script`, вызывающий dwe обратно через `$DWE_BIN`.
