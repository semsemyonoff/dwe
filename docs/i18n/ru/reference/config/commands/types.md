...
`COMPOSE_PROJECT_NAME` берётся из того же резолвера, что `dwe render env` пишет в `.env` как [зарезервированную системную переменную](../../render/env.md#system-variables).
