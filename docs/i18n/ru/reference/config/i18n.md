> Translated from: reference/config/i18n.md @ ec4f57457d87
...
# Через userconfig (~/.config/workspace/config.yml или .dwe/config.yml)
# language: de
dwe commands list
# → Выбирает немецкий
...
`DWE_LANGUAGE` инжектится в загрузчик userconfig на старте; он подменяет любое значение `language`, которое живёт в `~/.config/workspace/config.yml` или в локальном конфиге проекта. Стоит **ниже** явного флага `--lang` и **выше** системной переменной `$LANG`:
