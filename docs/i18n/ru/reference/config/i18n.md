> Translated from: reference/config/i18n.md @ 8d2b141fa736
...
DWE_LANGUAGE=fr devbox commands list
...
`DWE_LANGUAGE` инжектится в загрузчик userconfig на старте; он подменяет любое значение `language`, которое живёт в `~/.config/devbox/config.yml` или в локальном конфиге проекта. Стоит **ниже** явного флага `--lang` и **выше** системной переменной `$LANG`:

```bash
# Переопределить язык для одной команды (env побеждает userconfig и $LANG)
DWE_LANGUAGE=ru devbox commands list
...
DWE_LANGUAGE=de devbox docs show config/services/fields --lang ru
