> Translated from: reference/config/validate.md @ 2702066f02fe
...
Тот же домен проверяет цель команд `service_exec`, `service_run` и `daemon`. Runtime передаёт `service:` (или `runner.service:`) прямо в `docker compose exec` / `run`, поэтому там должно стоять имя **compose-сервиса**, а не ключ dwe-сервиса. Проверка даёт предупреждение `service: "<name>" is not a service in any compose overlay, disabled services included`, если ни один файл полной цепочки оверлеев не объявляет такой сервис. Учитываются все настроенные оверлеи, поэтому команда, нацеленная на compose-only сервис выключенного сейчас инструмента, не считается ошибкой. Подсказка предлагает ближайшее compose-имя, а если значение является ключом dwe-сервиса, то compose-сервис, под которым этот ключ запускается. Проверка пропускается, если значение шаблонное (`${param.*}`, `{{ … }}`), если конфиг не загрузился и если какой-то файл цепочки не читается или подключает другие файлы через `include:` верхнего уровня. Отсутствующий compose-файл сообщает `config.compose_files`.
