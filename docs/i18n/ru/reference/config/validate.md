> Translated from: reference/config/validate.md @ b5268a7dc511
...
Тот же домен проверяет `hide:` на командах и в блоках `group:`. Помимо синтаксиса шаблона, выражение без предикатов рендерится против смерженного конфига — того же контекста, что использует runtime-видимость, — и ошибка рендера даёт предупреждение `hide: expression does not evaluate: …`. Runtime-вычисление fail-open, поэтому иначе такое выражение оставляет команду видимой, оставив лишь строку в логе. Выражения с `cmd:` и builtin-предикатами `dwe validate` не исполняет и не рендерит; при неудачной загрузке конфига проверка рендера пропускается. См. [Условие hide](commands/directives.md#условие-hide).
