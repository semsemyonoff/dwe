> Translated from: reference/config/validate.md @ 2fe7f6adf5ec
...
`config.exports` предупреждает о правиле `exports.env`, чей dot-path в `from:` или `when:` не резолвится в смерженном конфиге. Это голые dot-path'ы, а не ссылки `${...}`, поэтому `config.template_refs` их не видит: `from: vars.db.passwrod` — корректная строка, загрузчик её принимает, а [`dwe render env`](../render/env.md) рендерит `DB_PASSWORD=` — пустое значение, которое доходит до каждого контейнера так, будто его так и объявили. Нерезолвящийся `when:` ещё тише: он ложный, поэтому правило пропускается и переменная не появляется вовсе. Подсказка называет последствие для конкретной формы правила — переменная рендерится пустой, всегда используется `default:`, или `dwe render env` падает на правиле с `required:`. Это предупреждение, а не ошибка: `from:` вместе с `default:` — легитимный паттерн необязательного пути, а сам путь может жить в `local.yml`, которого нет на этой машине. Путь, который резолвится в существующий ключ со значением `null`, **не** репортится — ключ есть, значит пустой рендер осознан автором. Находка привязывается к слою, объявившему правило (`workspace.yml`, `workspace/defaults.yml` или `workspace/local.yml`).
