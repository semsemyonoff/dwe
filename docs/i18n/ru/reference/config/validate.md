> Translated from: reference/config/validate.md @ 40a7523635c1
...
`secrets.shadowed` отвечает на вопрос, ответом на который два других валидатора *читают*, но который они не задают. Маркер, перебитый плейнтекстом из вышележащего слоя, отлично расшифровывается и числится читаемым, а проект при этом читает плейнтекст — то есть пара ключей не является тем, через что это значение шарится, и об этом никто не говорит. Это **предупреждение**, а не ошибка: временно перебить общий секрет локальным значением — законный приём, ломать его дороже, чем стоит полученная видимость. По той же причине валидатор не попадает в preflight.
