> Translated from: reference/config/validate.md @ c521b90199dd
...
`config.template_refs` предупреждает о ссылке `${head.path}`, чей head является известным корневым ключом смерженного конфига (то есть реально лежит в `cfg.Raw`), но оставшийся путь не резолвится — почти всегда это опечатка, поскольку `vars:` — единственный корневой ключ с полностью свободным содержимым (`${vars.opechatka}`, когда объявлен только `vars.source.repo`). Проверка намеренно молчит на нераспознанном head'е (`${HOME}`, случайный знак доллара) и на специальных неймспейсах, которые никогда не живут в `Raw` (`param`, `context`, `files`, `host`, `snapshot`, `args`, `generated`) — см. [Два синтаксиса: shorthand и полные шаблоны](../templates.md#два-синтаксиса-shorthand-и-полные-шаблоны) в справочнике по шаблонам.

Область проверки — все ссылки `${...}`, которые рантайм действительно рендерит, а не только шаги пайплайна: шаблонизируемые поля всего workspace-YAML (`cmd` / `with` / `check` / shell `when:` пайплайна, `text`/`value`/`title` в `info.yml`, `cmd`/`argv`/`env`/`workdir`/`messages.*` команд, `project_name` в `docker.yml`) плюс тела рендер-шаблонов под `workspace/templates/config/**`. Проверяется только shorthand-форма `${...}` — Go-шаблонная форма (`{{ resolve .Raw "vars.x" }}`) не проверяется.

`config.container_name` предупреждает, когда `container_name:` compose-сервиса расходится с именем, которое dwe для него выводит (`<project>-<service>`). Дефект — само расхождение, а не регистр: несовпадающий `container_name` — это грабли для голого использования `docker`/`docker compose`, скриптов и документации, которые полагаются на выводимое имя. Проверка молчит, когда объявленное значение уже совпадает с выводимым именем, или когда это интерполированное значение `${...}`, которое нельзя сравнить без разрешения окружения.

`config.ports_exports` предупреждает, когда сервис объявляет `services.<name>.ports.<key>`, но ни одно правило `exports.env` не читает `services.<name>.ports.<key>`. Такой порт — только для отображения: его переопределение в `local.yml` никак не сдвинет реальную привязку контейнера, и автоматическая изоляция host-портов `dwe test` (см. [`tests.md`](tests.md)) на него тоже молча не распространяется.
