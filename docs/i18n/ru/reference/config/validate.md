> Translated from: reference/config/validate.md @ 659c175b787d
...
`secrets.unresolved` — **второе исключение** из правила, что в preflight попадают только `env.*` и `checks.*` (первое — `config.validate`). Это вопрос готовности — «может ли эта машина реально деплоить прямо сейчас», — а не контента, поэтому `dwe run` / `dwe restart` / `dwe deploy run` / `dwe reset run` и pre-flight-гейт мастера деплоя останавливаются с одной и той же понятной подсказкой, а `dwe status`, `dwe docs`, `dwe validate` и `dwe prompt` продолжают работать. Нерасшифрованные значения группируются **по причине**: одна диагностика на причину со списком отсортированных путей, чтобы единственное осмысленное действие не было похоронено под одинаковыми строками на каждый маркер. Причины: `no_identity` (источника нет нигде), `invalid_identity` (источник **задан**, но ключа age не содержит — обрезанный `DWE_AGE_KEY`, keyfile, потерявший строку с ключом; сообщение называет источник и составлено из фиксированных формулировок, потому что ошибка парсера `age` повторяет байты приватного ключа, на которых он споткнулся), `wrong_identity` (читаемый identity другого recipient) и `corrupt` (повреждённый payload — проблема не в ключе). В интерактивном `dwe run` / `dwe restart` / `dwe deploy` *отсутствующий* identity [предлагается ввести](secrets.md#предложение-в-dwe-deploy-dwe-run-и-dwe-restart) до preflight, поэтому сюда доходит только то, что предложение исправить не могло: отказ пользователя, неинтерактивный запуск или сломанный источник, который импорт ключа всё равно не починит.
