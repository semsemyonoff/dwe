> Translated from: reference/config/validate.md @ cc914939311b
...
Для `deploy.yml` и `reset.yml` у этих двух Info-состояний есть прямое лекарство: `dwe deploy eject --out workspace/deploy.yml --force` (или `dwe reset eject`) записывает встроенный дефолт поверх инертного файла, превращая его в настоящий редактируемый пайплайн. `eject` отказывается перезаписывать *авторский* файл без `--force`, и его отказ опирается ровно на те же два условия, что и этот валидатор, — так что обе команды всегда согласны в том, какие файлы инертны. У `lifecycle.yml` аналога eject нет — см. [`deploy/index.md`](deploy/index.md).
