...
Builtin-условия тестируют состояние файловой системы через **регистр предикатов**. Предикаты отличаются от engine-билтинов (вроде `service_configs_copy`) — они живут в отдельном неймспейсе и не могут использоваться в действиях `check:`.
...
**Портируемость:** эвалюатор предикатов использует захардкоженный `sh -c` (а не сконфигурированный shell-бинарь проекта), чтобы обеспечить POSIX-портируемость и согласованность независимо от выбора шелла в проекте.
...
1. **Предикаты** — используются в `when: type: builtin`. Тесты состояния файловой системы, например, `dir-empty`, `file-exists`.
...
Условия workflow'ов классифицируются по ведущему префиксу (`{{ ... }}` → template, `cmd: ...` → shell-команда, иначе → предикат). Полную грамматику workflow'ов см. в [commands/](commands/index.md).
