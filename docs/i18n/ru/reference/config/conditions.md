...
**Условия** (`when:`) — это **предусловия**, вычисляемые до запуска фазы или шага. Они возвращают булево значение: true = идти дальше, false = пропустить.

**Действия** — это **payload'ы**, исполняющие код. Когда используются как `check:` (post-action), их успех/падение определяет, прошёл шаг или упал.

Система пайплайнов использует **типизированные** формы для обоих — поле `type:` диспетчеризует в разные эвалюаторы. Шаги workflow'ов (внутри определений команд) используют отдельную строковую форму `when:`, документированную в [commands/](commands/index.md).
...
Builtin-условия тестируют состояние файловой системы через **регистр предикатов** в `internal/core/execution/condition`. Предикаты отличаются от engine-билтинов (вроде `service_configs_copy`) — они живут в отдельном неймспейсе и не могут использоваться в действиях `check:`.
...
**Портируемость:** эвалюатор предикатов использует захардкоженный `sh -c` (а не сконфигурированный `ShellBin` проекта), чтобы обеспечить POSIX-портируемость и согласованность независимо от выбора шелла в проекте.

### `type: shell` — shell-команды

Shell-условия выполняют команду и тестируют её exit-код: exit 0 = true, не-ноль = false.
...
Полная shell-семантика применима: пайпы, редиректы, операторы и т.д. Как и предикаты, shell-условия используют захардкоженный `sh -c` для портируемости.
...
Система пайплайнов имеет **два отдельных неймспейса `type: builtin`**, различаемых по позиции в YAML:

1. **Предикаты** — используются в `when: type: builtin`. Живут в `internal/core/execution/condition`, например, `dir-empty`, `file-exists`.
2. **Engine-билтины** — используются в телах шагов и в `check: type: builtin`. Живут в `internal/core/execution/builtin`, например, `service_configs_copy`, `service_configs_check`, `message`.
...
Шаги workflow'ов используют отдельный строковый мини-язык условий. Полная грамматика документирована в [commands/](commands/index.md); эта секция лишь набрасывает поверхность для контекста.
...
Условия workflow'ов вычисляются `internal/shared/tpl/render_command.go` и string-классификационной логикой в `internal/core/execution/condition`. Полную грамматику workflow'ов см. в [commands/](commands/index.md).
