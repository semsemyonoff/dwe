> Translated from: reference/config/setup.md @ 2e27f5b952f3
...
`devbox/setup.yml` определяет интерактивные промпты, запускающиеся, когда разработчик впервые заходит в свежий проект (без `devbox/local.yml` или с пустым). Визард собирает ответы, пишет их в `devbox/local.yml` как смерженные настройки и затем переходит к деплою.

Используйте setup-вопросы для одноразовой per-developer конфигурации:
- API-ключи или секреты (хранятся в `local.yml`, который gitignored)
- Тумблеры сервисов (какие опциональные инструменты разработчик хочет включить)
- Переопределения портов (когда есть конфликты локальных портов)
- Кастомные пути или имена хостов

Setup-визард — часть более широкого потока `devbox deploy` — запуск `devbox deploy` без подкоманды открывает интерактивное меню, включающее опцию Wizard, когда setup-вопросы присутствуют.

## Как это работает

1. Разработчик запускает `devbox deploy` в интерактивном терминале на свежем проекте.
2. CLI пробует на конфликты портов и загружает `devbox/setup.yml` (если есть).
3. Если оба пусты (ни вопросов, ни конфликтов), визард не запускается — переходим сразу к деплою.
4. Если в чём-то есть содержимое, меню открывается с опцией **Wizard**.
5. Визард выполняется:
   ...
   - Наконец, оба набора ответов deep-merge'атся в `devbox/local.yml` и атомарно записываются.
6. Конфиг перезагружается из обновлённого `local.yml`, запускается preflight, и деплой идёт нормально.

Если разработчик отменяет на любом шаге визарда (Ctrl-C), `local.yml` остаётся нетронутым — никаких частичных записей.
...
Файл опционален. Когда отсутствует, визард (если вызван) обрабатывает только port-конфликты.
...
Схемные правила, форсируемые валидацией (запускается через `devbox validate`):
...
Паттерн должен компилироваться как валидный Go-regex. Невалидные паттерны ловятся `devbox validate` до того, как визард вообще запустится.

## Правила области записи

Поле `writes:` — это dot-path, определяющий, где в `devbox/local.yml` сохраняется ответ. Не все пути разрешены — визард форсит правила, чтобы ответы безопасно сливались со схемой конфига.
...
- `devbox deploy` — открывает меню визарда на свежих проектах
- `devbox validate` — проверяет схему `devbox/setup.yml` и пути записи
- `devbox validate setup` — валидирует только setup-домен
