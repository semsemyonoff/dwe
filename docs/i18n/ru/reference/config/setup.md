...
`workspace/setup.yml` определяет интерактивные промпты, запускающиеся, когда разработчик впервые заходит в свежий проект (без `workspace/local.yml` или с пустым). Визард собирает ответы, пишет их в `workspace/local.yml` как смерженные настройки и затем переходит к деплою.
...
Setup-визард — часть более широкого потока `dwe deploy` — запуск `dwe deploy` без подкоманды открывает интерактивное меню, включающее опцию Wizard, когда setup-вопросы присутствуют.

## Как это работает

1. Разработчик запускает `dwe deploy` в интерактивном терминале на свежем проекте.
2. CLI пробует на конфликты портов и загружает `workspace/setup.yml` (если есть).
...
5. Визард выполняется:
   ...
   - Наконец, оба набора ответов deep-merge'атся в `workspace/local.yml` и атомарно записываются.
