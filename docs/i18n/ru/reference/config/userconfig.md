...
В той же директории `~/.config/dwe/` лежит и `keys/<recipient>.key` — приватный age-identity одного проекта, права `0600` в директории `0700`; его пишут `dwe secrets init` / `key import` / `rekey`, а переопределяют `DWE_AGE_KEY` / `DWE_AGE_KEY_FILE`. К схеме этого файла он не относится — см. [секреты](secrets.md#ключи-где-живёт-identity).
