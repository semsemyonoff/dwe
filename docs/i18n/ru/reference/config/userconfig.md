> Translated from: reference/config/userconfig.md @ 5d61f17860b3
...
1. **Глобальный пользовательский конфиг** в `~/.config/dwe/config` на каждой поддерживаемой ОС (macOS и Linux; под Windows запускайте dwe внутри WSL2 — там путь принадлежит дистрибутиву). Один путь везде — никаких platform-native локаций, никакого XDG-фолбэка. Отсутствие файла трактуется как пустой. Если DWE когда-либо пишет этот файл, права — `0600`.
...
В той же директории `~/.config/dwe/` лежит и `keys/<recipient>.key` — приватный age-identity одного проекта, права `0600` в директории `0700`; его пишут `dwe secrets init` / `key import` / `rekey`, а переопределяют `DWE_AGE_KEY` / `DWE_AGE_KEY_FILE`. К схеме этого файла он не относится — см. [секреты](secrets.md#keys-where-the-identity-lives).
