> Translated from: reference/config/notifications.md @ c59fb8be7882
...
1. **Глобальный пользовательский конфиг** в `~/.config/dwe/config` на каждой поддерживаемой ОС (macOS и Linux; под Windows запускайте dwe внутри WSL2 — там путь принадлежит дистрибутиву). Никакого нативного для платформы расположения, никакого XDG-отката — один путь везде. Отсутствие файла молча трактуется как пустота. Если DWE когда-нибудь его пишет, режим — `0600`.
...
Пути для Windows нет: dwe поддерживает macOS и Linux, а под Windows работает внутри WSL2, где уведомления идут по Linux-ветке (в дистрибутиве должен быть доступен демон libnotify).
...
Linux (libnotify) соблюдает встроенную иконку и имя приложения без дополнительной настройки.
