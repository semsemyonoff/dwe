> Translated from: reference/config/notifications.md @ ec343c919f75
...
- [Зарезервированные ключи (пока не подключены)](#зарезервированные-ключи-пока-не-подключены)
...
- Неизвестные ключи — предупреждения (forward-compat с зарезервированными ключами).
...
## Зарезервированные ключи (пока не подключены)

Парсер принимает эти ключи без ошибки, чтобы добавление соответствующего backend'а в будущем релизе не требовало миграции конфига. Они декодируются, но **не используются** в MVP:

- `notify_telegram_token`
- `notify_telegram_chat`
- `notify_webhook_urls` (список)

Установка `notify_channels = telegram` или `notify_channels = webhook` в MVP откатывается на no-op (неизвестный канал логируется на debug).
