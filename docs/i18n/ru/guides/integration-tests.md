> Translated from: guides/integration-tests.md @ c8ffd39850fb
...
Случай, который это *не* покрывает, — host-порт, который dwe отследить не может: захардкоженный прямо в сыром compose-файле (`8080:8080`) или подставленный из переменной, которую ни одно активное правило `exports.env` не направляет к порту. Он обходит и переназначение, и preflight `ports_free`, и копия биндит тот же порт, что и ваше рабочее окружение. Объявите порт под `services.<name>.ports` либо экспортируйте переменную из `vars.<path>` через правило `exports.env`. Если это пропустить, `dwe test run` и `dwe validate` предупредят находкой `interpolated_host_port`, которая называет переменную и оба способа маршрутизации.
