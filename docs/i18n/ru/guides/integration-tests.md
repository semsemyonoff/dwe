...
Host-порт compose, записанный переменной (`"${VALKEY_PORT:-6379}:6379"`), тоже покрыт — если dwe может отследить переменную: правило `exports.env` `from: vars.<path>` заставляет раннер выделить свободный порт для `<path>` и записать его в `vars:` копии — снова без какой-либо конфигурации сценария. Шаг читает его как `${vars.<path>}`. Сценарий, которому нужен конкретный порт, фиксирует его строкой `env.vars: { <path>: 6380 }`.

Вне переназначения остаются две формы, и ведут они себя по-разному. Host-порт, захардкоженный прямо в сыром compose-файле (`8080:8080`), — это **блокирующая** находка `raw_host_port`: `dwe test run` отказывается выполнять сценарий ещё до создания копии, так что порт вашего рабочего окружения никто не биндит. Порт, подставленный из переменной, которую ни одно активное правило `exports.env` не направляет к порту, **не** блокирует: он обходит и переназначение, и preflight `ports_free`, копия биндит тот же порт, что и ваше рабочее окружение, а `dwe test run` и `dwe validate` лишь предупреждают находкой `interpolated_host_port`, которая называет переменную и оба способа маршрутизации. Исправление для обеих форм одно: объявите порт под `services.<name>.ports` либо экспортируйте переменную из `vars.<path>` через правило `exports.env`.
...
`dwe test run` сканирует compose-файлы, которые будет выполнять копия сценария, — файлы самого проекта с применённым `env:` сценария — на конструкции, которые обходят разграничение compose по имени проекта. `container_name:` и буквальные (нешаблонизированные) host-порты **блокирующие**; volume'ы и сети с `external:`/явным `name:` — только предупреждения. Скан выполняется до создания копии, поэтому блокирующая находка проваливает сценарий, ничего не создав — ни копии, ни контейнеров, ни teardown, и отчёт об ошибке для этого запуска не собирается (отчёт, оставшийся от **более раннего** провала того же сценария, остаётся на месте), — с сообщением, называющим проблемную конструкцию:
...
- **Буквальный host-порт** (`8080:8080` в сыром compose-файле) — перенесите его в `services.<name>.ports`, чтобы `dwe test` мог увидеть его и переназначить автоматически (см. [Порты изолируются автоматически](#порты-изолируются-автоматически) выше), либо подставляйте его из переменной, которую правило `exports.env` экспортирует `from: vars.<path>`, — тогда `dwe test` переназначит его сам.
...
## Host-скрипты и имя проекта

Внутри `dwe test` копия работает под собственным именем compose-проекта, и dwe передаёт это имя запускаемым shell-шагам и host-командам как `COMPOSE_PROJECT_NAME` (см. [кто его получает](../reference/config/tests.md#модель-изоляции)). Host-скрипт, который сам вычисляет `-p` из префикса и имени проекта, его игнорирует и обращается к вашему **рабочему** стеку — сценарий проходит, пока скрипт наполняет, дампит или очищает живую базу.

Берите имя из `$COMPOSE_PROJECT_NAME`. Для скрипта, который вы запускаете и вручную, оставьте старое значение как fallback:
...
Для скрипта, который запускает только dwe, сделайте отсутствие имени фатальным, а не откатом на живой стек:

```sh
PROJECT="${COMPOSE_PROJECT_NAME:?run this through dwe}"
```

Форма `:?` покрывает и места, куда dwe имя не подставляет: shell-предикат `when:` или билтин-проба `shell` в сценарии вообще не видят `COMPOSE_PROJECT_NAME`, так что fallback через `:-` там резолвится в живой стек.

`dwe validate tests` ловит типичную ошибку — значение `-p` / `--project-name` или `COMPOSE_PROJECT_NAME=`, собранное без `$COMPOSE_PROJECT_NAME`, в host-командах, скриптах, shell-шагах пайплайнов и сценариев и в скриптах, которые они вызывают, на один уровень вглубь. Он никогда не угадывает, поэтому не видит имя, переданное как `$1`, прочитанное через `read`, возвращённое подстановкой команды, заданное в скрипте двумя файлами глубже, или вручную собранное имя контейнера (`docker exec myproj-db-1 …`) — это остаётся на вас. Полное правило — в [Host-скриптах, которые сами собирают имя compose-проекта](../reference/config/tests.md#host-скрипты-которые-сами-собирают-имя-compose-проекта).
