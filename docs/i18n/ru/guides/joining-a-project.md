> Translated from: guides/joining-a-project.md @ e7431d81c708
...
0. **Онбординг ключа** (только если у проекта есть зашифрованные секреты, а на этой машине нет identity для них). В терминале `dwe deploy` предлагает принять ключ раньше всего остального; при согласии открывается скрытый ввод, identity сохраняется в `~/.config/dwe/keys/<recipient>.key`, и деплой продолжается в том же вызове. Отказ — как и запуск без терминала, с `--output json` или под `DWE_NONINTERACTIVE=1` — предложение пропускает, и деплой тогда останавливается на preflight-гейте `secrets.unresolved`, который называет нечитаемые значения. `dwe run` и `dwe restart` делают то же предложение. См. [Зашифрованные секреты](../reference/config/secrets.md).
