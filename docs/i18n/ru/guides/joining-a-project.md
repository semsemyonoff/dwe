> Translated from: guides/joining-a-project.md @ 3115e030d8f9
...
0. **Онбординг ключа** (только если у проекта есть зашифрованные секреты, а на этой машине нет identity для них). В терминале `dwe deploy` предлагает принять ключ раньше всего остального; при согласии открывается скрытый ввод, identity сохраняется в `~/.config/dwe/keys/<recipient>.key`, и деплой продолжается в том же вызове. Отказ — как и запуск неинтерактивно, с `--yes` или с `--output json` — предложение пропускает, и деплой тогда останавливается на preflight-гейте `secrets.unresolved`, который называет нечитаемые значения. `dwe run` и `dwe restart` делают то же предложение. См. [Зашифрованные секреты](../reference/config/secrets.md).
