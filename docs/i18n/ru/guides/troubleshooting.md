> Translated from: guides/troubleshooting.md @ 3dbfae6701f3
...
dwe compose raw --all -- config    # цепочка вместе с оверлеями выключенных сервисов
```

`dwe compose raw` — это низкоуровневый прямой вызов `docker compose`: DWE определяет список compose-файлов и имя проекта, а остальные аргументы передаёт в `docker compose` без изменений. Никаких policy-аргументов, никаких оверлеев сверх тех, что уже на диске. `--all` (есть и у `compose files`, и у `compose argv`) добавляет оверлеи выключенных сервисов — только для просмотра: они могут конфликтовать, и итоговая цепочка не обязательно валидна; флаг ставится до первого аргумента `docker compose`, поэтому в `dwe compose raw -- ps --all` `--all` уходит в сам `docker compose`. Используйте как диагностику, не как повседневный инструмент — высокоуровневые команды `dwe` существуют не зря — но это правильный инструмент, когда вы дебажите сам DWE или воспроизводите проблему напрямую через compose CLI. Справочник: [`../reference/config/docker.md`](../reference/config/docker.md).
...
`-v, --verbose` выводит эхом команды, которые выполняет DWE (lifecycle docker/compose, прямые `docker stop/restart/rm`, `sh -c …`, вложенные `dwe …`, git, а также дочерний процесс пользовательской команды — `docker compose exec`/`run` команды `service_exec`/`service_run`, хостовый процесс команды `shell`/`script`/`dwe`, выражение `argv_append_from`, вызовы `docker` за `.start`/`.stop`/`.restart`/`.logs` демона и процессы, которые запускают builtin-ы — `shell`, уборка демонов в `dwe stop`, `docker_remove_project_volumes`) — каждую отдельной копируемой строкой `$ …` (значения, которые команда передаёт через окружение — её `env:`, параметры с `env:`, — не показываются: в строке `service_exec`/`service_run` есть только `-e KEY`, а значение уходит в окружение дочернего процесса, поэтому для ручного повтора строки эти переменные сначала нужно экспортировать), — плюс ключевые **решения** пайплайна: какой шаг выполнился или был пропущен и почему (результаты `when:`, фазовые гейты, `state: already deployed`, files-gate), а также сводку preflight (прошёл/не прошёл).
...
- **Read-only пробы не попадают в verbose.** `dwe status -v` *не* засыпает выводом `docker compose ps` — эти пробы только на уровне Debug, как и те, что решают, что именно запустит команда (проверка демона «запущен ли» через `docker ps`, выбор exec или run у `service_exec`, `docker volume ls`). Если нужно их увидеть, используйте `--debug`.
