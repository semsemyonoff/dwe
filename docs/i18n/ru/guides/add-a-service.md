> Translated from: guides/add-a-service.md @ 0ba33903849c
...
Оверлей не ограничен своим сервисом: оверлей tool-сервиса может заодно пропатчить соседний app-сервис (добавить `environment:`, `volumes:`), и тогда один переключатель включает и выключает целую фичу. Эта техника разобрана в руководстве [Наблюдаемость с OpenTelemetry](observability-otel.md#2-оверлей) — что Compose сливает, что заменяет и почему порядок файлов решает, чьё значение победит. Патч, который должен целиком заменить значение, заданное собственным оверлеем приложения (`command:`, `healthcheck:`, ключ `environment:`), кладите в список [`compose_after:`](../reference/config/services/fields.md#поля-сервиса-верхнего-уровня) tool-сервиса, а не в `compose:` — он выводится после всех групп сервисов и поэтому побеждает.
