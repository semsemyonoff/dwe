> Translated from: guides/author-project-commands.md @ 3f1bb72f6c19

# Авторство проектных команд

В проекте всё длиннее становится README со снадобьями: «запусти это, чтобы засидить базу», «запусти это, чтобы пересобрать фронт», «запусти это, когда очередь шалит». Это руководство заменяет такие сниппеты на полноценные команды `dwe <id>`, которые вся команда находит через `dwe commands`, запускает через `dwe cmd <id>` и компонует в большие пайплайны.
...
Команды живут под `workspace/commands/`. Путь каталога становится ID команды, через точку. Базовое имя файла — это листовой сегмент, а каждый ключ в `commands:` файла добавляет ещё одну точку.
...
Правило большого пальца: основные команды группы — в один файл по имени группы; делите в поддиректорию только когда команд накапливается на логические подгруппы.
...
Каждый подпроцесс `type: shell` наследует три экспортированных переменных, чтобы дотянуться до того же compose-проекта, которым рулит DWE, без перевычисления:
...
- **`mode:`** — что делать, если контейнер не запущен:
  - `exec-or-fail` (по умолчанию) — отказ с действенной ошибкой и подсказкой `dwe run`.
...
- **`command: <id>`** — вызов другой команды. `with:` переопределяет её params; `when:` пропускает шаг, когда выражение falsy; `continue_on_error: true` превращает падение в предупреждение вместо abort.
...
`confirmation_text:` поддерживает `${...}`-шаблонизатор, чтобы можно было показать значения, на которые пользователь сейчас подействует. Промт обходится в трёх случаях:
...
Используйте для долгих «запустил-переключился в другое окно» команд (большие импорты, полные bootstrap-ы, pack/unpack снапшота). Для subsecond-команд опускайте — десктоп-попапы по мгновенным операциям только шум.
