...
- `header.font` принимает стандартные имена FIGlet-шрифтов: `doom`, `banner`, `big`, `block`, `slant` и похожие. По умолчанию — `doom`.
...
Распространённые альтернативы: `"·"` (точка по центру, по умолчанию), `"—"` (длинное тире), `":"` (двоеточие — лаконично, но воспринимается как разрыв между заголовком и значением). Выбрали один раз — и забыли.
...
- `.Project`, `.Services`, `.Runtime` и `.State` доступны на верхнем уровне. Собственные ключи, которые вы добавили в `defaults.yml`, лежат под `.Cfg.Raw`.
