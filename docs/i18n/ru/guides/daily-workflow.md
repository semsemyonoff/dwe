> Translated from: guides/daily-workflow.md @ f5be01c30cf7
...
Без аргумента `dwe logs` показывает логи всего стека — каждый включённый сервис, с мультиплексом и префиксами, как `docker compose logs`. С именем сервиса показывает контейнер одного сервиса. По умолчанию печатает последние 50 строк и выходит. Чтобы следить:
