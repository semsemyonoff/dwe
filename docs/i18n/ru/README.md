> Translated from: README.md @ b1cb9db1336a
...
Перейдите в директорию проекта, содержащую `workspace.yml`, и запустите любую команду — DWE поднимается вверх от рабочей директории, пока не найдёт корень проекта.
...
- Каждый вызов короткоживущий и stateless: нет демона DWE, нет загрузчика плагинов, нет сети на штатном пути. После удаления DWE compose-файлы в `compose/` остаются валидным самостоятельным вводом для `docker compose`.
