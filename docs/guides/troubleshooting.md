...
`-v, --verbose` echoes the commands DWE executes (docker/compose lifecycle, raw `docker stop/restart/rm`, `sh -c …`, nested `dwe …`, git, and the child a user command spawns — the `docker compose exec`/`run` of a `service_exec`/`service_run` command, the host process of a `shell`/`script`/`dwe` command, an `argv_append_from` expression) each as a copy-pasteable `$ …` line (values a command passes through the environment — its `env:`, params with `env:` — are not shown: a `service_exec`/`service_run` line carries only `-e KEY`, and the value travels in the child's environment, so re-running the line by hand needs those variables exported first), plus the key pipeline **decisions**: which step ran or was skipped and why (`when:` results, phase gates, `state: already deployed`, files-gate), and the preflight pass/fail summary.
