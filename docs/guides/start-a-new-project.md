...
- **Two files ship active because they cannot ship commented.** A template-pack `manifest.yml` is strict-decoded and must declare at least one entry; the test-scenario loader rejects an empty document outright. `workspace/tests/smoke.yml` is therefore a real scenario with a `description:` and `steps: []` — deliberately assertion-free, because on a fresh scaffold `compose.yaml` declares no services and "the stack deploys and reports healthy" would pass vacuously. Add steps once your service exists in compose.
